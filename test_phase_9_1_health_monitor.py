...
def test_enhanced_health_monitor():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "health_monitor", 
            "c:/dix_vision_v42.2/execution_unified/health/health_monitor.py"
        )
    ...
    try:
        ...
        print(f"[PASS] World context created: {world_context.market_regime} regime, {world_context.volatility_regime} volatility")
    except Exception as e:
    ...
    try:
        ...
        print(f"[PASS] Enhanced health check created with confidence interval: {enhanced_check.confidence_interval}")
    ...
    try:
        ...
        print(f"[PASS] World integration available: {monitor._world_integration_bridge is not None}")
    ...
    try:
        ...
        print(f"[PASS] High volatility interval: {high_vol_interval}ms (expected ~{monitor._base_check_interval_ms * 0.2}ms)")
        ...
        print(f"[PASS] Standard conditions interval: {standard_interval}ms (expected {monitor._base_check_interval_ms}ms)")
    except Exception as e:
