...
class WalletBehaviorProfile:
    ...
    Produced by analyzing on-chain actions via Solscan / Alchemy.
    Used to classify wallets as smart money, sniper, whale, etc.
    """

    ts_ns: int
    wallet_address: str
    chain: str
    label: str  # "smart_money" | "sniper" | "whale" | "degen" | "bot" | "unknown"
    trade_count_30d: int = 0
    win_rate: float = 0.0  # [0, 1]
    avg_hold_time_sec: int = 0
    total_pnl_usd: float = 0.0
    token_diversity: int = 0
    is_copy_worthy: bool = False

    def __post_init__(self) -> None:
        if self.ts_ns <= 0:
            raise ValueError("WalletBehaviorProfile.ts_ns must be positive")
        if not self.wallet_address:
            raise ValueError("WalletBehaviorProfile.wallet_address must be non-empty")
        if not self.chain:
            raise ValueError("WalletBehaviorProfile.chain must be non-empty")
