...
class SharedInfrastructureAdapter:
            ...
            with self._lock:
                ...
                if self._config.use_new_signal_processing:
                    try:
                        ...
                        logger.warning(
                            f"[SHARED_INFRA] Failed to initialize signal processing: {e}"
                        )
                        self._signal_processing = None
    ...
    def create_plan(
        ...
        try:
            ...
            return self._fallback_create_plan(
                plan_type, goal, constraints, requester, start_time_ms
            )
    ...
    def _try_create_plan(
        self, plan_type: str, goal: str, constraints: Optional[Dict], requester: str
    ) -> Optional[Dict]:
    ...
    def _fallback_create_plan(
        ...
        return {
            ...
            "plan_type": plan_type,
            "goal": goal,
            "steps": [f"Analyze: {goal}", "Execute plan for {plan_type}", "Validate results"],
            "latency_ms": latency_ms,
            "integration_mode": "fallback",
        }

    def process_signals(
        self, signals: List[Dict[str, Any]], filters: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if (
                self._config.use_new_signal_processing
                and self._signal_processing
                and self._signal_healthy
            ):
    ...
    def _try_process_signals(
        self, signals: List[Dict[str, Any]], filters: Optional[Dict]
    ) -> Optional[Dict]:
        ...
        try:
            if self._signal_processing and hasattr(self._signal_processing, "process_signals"):
                result = self._signal_processing.process_signals(
                    signals=signals, filters=filters or {}
                )
    ...
    def _fallback_process_signals(
        self, signals: List[Dict[str, Any]], filters: Optional[Dict], start_time_ms: float
    ) -> Dict:
        """Fallback to simple signal processing."""
        end_time_ms = time.time() * 1000
        latency_ms = end_time_ms - start_time_ms
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        avg_planning_latency = (
            sum(self._latencies["planning"]) / len(self._latencies["planning"])
            if self._latencies["planning"]
            else 0.0
        )
        avg_signal_latency = (
            sum(self._latencies["signal"]) / len(self._latencies["signal"])
            if self._latencies["signal"]
            else 0.0
        )
