...
class Plan:
    ...
    def update_progress(self) -> None:
        ...
        if self.total_goals > 0 and self.total_actions > 0:
            self.progress = (self.achieved_goals / self.total_goals) * 0.4 + (
...
class PlanningEngineInterface(ABC):
    ...
    @abstractmethod
    def adjust_plan(self, plan_id: str, adjustments: Dict[str, Any]) -> bool:
...
class PlanningEngine(PlanningEngineInterface):
    ...
    def _generate_actions_from_goals(
        self, goals: List[PlanningGoal], context: Dict[str, Any]
    ) -> List[PlanningAction]:
        ...
        for i, goal in enumerate(goals):
            ...
            if goal.goal_type == "trading":
                ...
                action = PlanningAction(
                    ...
                    parameters={"goal_id": goal.goal_id, "operation": goal.description},
                ...
                action = PlanningAction(
                    ...
                    parameters={"goal_id": goal.goal_id, "analysis_type": goal.description},
                ...
                action = PlanningAction(
                    action_id=f"action_{goal.goal_id}",
                    action_type="custom",
                    parameters={"goal_id": goal.goal_id, "description": goal.description},
                    scheduled_time=datetime.utcnow() + timedelta(minutes=i * 5),
                    depends_on=[],
                )
    ...
    def _validate_constraints(
        self, constraints: List[PlanningConstraint], context: Dict[str, Any]
    ...
    def _calculate_success_probability(
        ...
        satisfied_ratio = (
            sum(1 for c in constraints if c.satisfied) / len(constraints) if constraints else 1.0
        )
    ...
    def monitor_plan(self, plan_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "goals": {"total": plan.total_goals, "achieved": plan.achieved_goals},
                "actions": {
                    "total": plan.total_actions,
                    "completed": plan.completed_actions,
                    "current_action": (
                        plan.get_next_action().action_id if plan.get_next_action() else None
                    ),
                },
