...
class SignalProcessingService(SignalProcessingServiceInterface):
    ...
    def _majority_vote_aggregation(self, signals: List[SignalEvent]) -> SignalEvent:
        ...
        aggregated_value = (
            sum(s.value for s in winning_signals) / len(winning_signals) if winning_signals else 0.0
        )
        ...
        with self._lock:
            ...
            filter_stats = {
                f.name: {
                    ...
                    "filter_rate": (
                        f.signals_filtered / (f.signals_filtered + f.signals_passed)
                        if (f.signals_filtered + f.signals_passed) > 0
                        else 0
                    ),
                }
