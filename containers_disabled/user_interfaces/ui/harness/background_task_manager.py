        ...
        *,
        ...
        try:
            while True:
                ...
                # Emit kernel state when version changes
                proj = state.state_projection
                if proj.is_booted and proj.version > last_kernel_version:
                    last_kernel_version = proj.version
                    kernel_event = {
                        "channel": "kernel_state",
                        "ts_iso": utc_now().isoformat(),
                        "payload": proj.summary(),
                    }
                    yield sse_format(kernel_event).encode("utf-8")
                if backfill_only:
                    return
