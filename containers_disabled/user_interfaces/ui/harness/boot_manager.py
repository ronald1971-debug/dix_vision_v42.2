...
class HarnessBootManager:
    ...
    # Core tiers — system is unsafe without these, so failures remain fatal.
    _CORE_STEPS: tuple[str, ...] = (
        "_build_intelligence_tier",
        "_build_execution_tier",
        "_build_system_tier",
        "_build_governance_tier",
        "_build_event_buffers",
    )

    # Optional tiers — degraded but functional without these.
    _OPTIONAL_STEPS: tuple[str, ...] = (
        "_build_dashboard_widgets",
        "_build_cognitive_chat",
        "_build_plugin_registry",
        "_build_approval_edge",
        "_build_live_feeds",
        "_build_learning_evolution_loops",
    )
    ...
    def populate(self, state: _State) -> None:
        ...
        for step in self._CORE_STEPS:
            _logger.info("[BOOT] %s: starting", step)
            getattr(state, step)()
            _logger.info("[BOOT] %s: OK", step)
        ...
        for step in self._OPTIONAL_STEPS:
            try:
                ...
                _logger.exception("[BOOT] %s: FAILED — degraded mode", step)
