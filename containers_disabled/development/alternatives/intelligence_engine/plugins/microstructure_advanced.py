...
class MicrostructurePatternRecognizer:
    ...
    def recognize_patterns(
        self, order_book_snapshots: tuple[OrderBookSnapshot, ...], timestamp_ns: int
    ...
    def _detect_momentum_building(
        self, mid_prices: list[float], timestamp_ns: int
    ...
    def _detect_price_impact(
        self, snapshots: tuple[OrderBookSnapshot, ...], timestamp_ns: int
    ) -> MicrostructureSignal | None:
        """Detect price impact patterns."""
        if len(snapshots) < 3:
            return None
        ...
        cov = (
            sum((s - mean_spread) * (p - mean_price) for s, p in zip(spread_changes, price_changes))
            / n
        )
