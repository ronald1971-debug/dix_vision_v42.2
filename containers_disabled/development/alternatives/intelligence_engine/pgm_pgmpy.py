    ...
    class _PgmpyVariableEliminationEngine:
        ...
        def infer(
            ...
            *,
            ...
            from pgmpy.factors.discrete import TabularCPD  # type: ignore[import-not-found]
            from pgmpy.inference import VariableElimination  # type: ignore[import-not-found]
            from pgmpy.models import BayesianNetwork as BN  # type: ignore[import-not-found]

            bn = BN(list(network.edges))

            for node in network.nodes:
                parents = [p for p, c in network.edges if c == node]
                n_parent_combos = max(1, 2 ** len(parents))
                vals = numpy.ones((2, n_parent_combos)) / 2
                cpd = TabularCPD(
                    variable=node,
                    variable_card=2,
                    values=vals.tolist(),
            ...
            evidence = dict(arguments.evidence) if arguments.evidence else {}
            evidence_int = {k: int(v) for k, v in evidence.items() if k in set(network.nodes)}

            marginals: list[BayesianMarginalResult] = []
            for qvar in arguments.query_variables:
                if qvar not in set(network.nodes):
                    continue
                result = infer_engine.query(
                    [qvar],
                    evidence=evidence_int if evidence_int else None,
                )
                probs = result.values
                total = float(probs.sum())
                if total > 0:
                    normed = tuple(float(p / total) for p in probs)
                else:
                    normed = (0.5, 0.5)
                marginals.append(
            ...
            if not marginals:
                marginals.append(
                    BayesianMarginalResult(
                        variable=arguments.query_variables[0],
                        states=("0", "1"),
                        probabilities=(0.5, 0.5),
                    )
                )

            return BayesianInferenceResult(marginals=tuple(marginals))
