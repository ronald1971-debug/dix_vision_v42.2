    ...
    class _EconMLDMLEstimator:
        ...
        def estimate(
            ...
            *,
            ...
            T = numpy.random.binomial(1, 0.5, n).astype(numpy.float64)
            Y = 0.5 * T + X[:, 0] * 0.3 + numpy.random.randn(n) * 0.2

            from econml.dml import LinearDML  # type: ignore[import-not-found]

            dml = LinearDML(random_state=arguments.random_seed)
            dml.fit(Y, T, X=X)

            effects = dml.effect(X)
            ate = float(numpy.mean(effects))
            ate_se = float(numpy.std(effects) / numpy.sqrt(n))

            points: list[HteEffectPoint] = []
            for i in range(min(n, 100)):
                pe = float(effects[i])
                se = abs(pe) * 0.1 if pe != 0 else 0.01
                points.append(
                    HteEffectPoint(
                        point_id=i,
                        point_estimate=pe,
