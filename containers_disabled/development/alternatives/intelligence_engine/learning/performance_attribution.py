...
class PerformanceAttributor:
    ...
    def attribute_decision(
        ...
        if outcome == "success":
            ...
            attributions[AttributionType.MARKET_REGIME] = (
                0.3 if market_regime == "favorable" else 0.1
            )
        else:
            ...
            attributions[AttributionType.MARKET_REGIME] = (
                -0.3 if market_regime == "unfavorable" else 0.0
            )
    ...
    def _generate_learnings(
        self, attributions: dict[AttributionType, float], context: dict[str, Any], outcome: str
    ) -> list[str]:
    ...
    def _update_attribution_patterns(
        self, decision_type: str, attributions: dict[AttributionType, float]
    ) -> None:
    ...
    def get_attribution_patterns(
        self, decision_type: str
    ) -> dict[AttributionType, tuple[float, ...]]:
...
class AdaptiveLearningEngine:
    ...
    def calculate_parameter_adjustment(
        self, parameter_name: str, current_value: float, performance_feedback: dict[str, float]
    ) -> float:
        ...
        for metric_name, feedback in performance_feedback.items():
            if metric_name in self._performance_metrics:
                ...
                if gap > 0:
                    ...
                    adjustment -= (
                        abs(gap) * self._learning_rate * 0.5
                    )  # More conservative on overshoot
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        for metric_name, metric in self._performance_metrics.items():
            summary[metric_name] = {
                ...
                "trend": metric.trend,
                "gap_percentage": (
                    abs(metric.target_value - metric.metric_value) / metric.target_value
