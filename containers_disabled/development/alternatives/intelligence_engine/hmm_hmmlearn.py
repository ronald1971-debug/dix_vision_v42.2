    ...
    class _HmmlearnGaussianEngine:
        ...
        def infer(
            ...
            *,
            ...
            obs = numpy.array(arguments.observations, dtype=numpy.float64)
            if obs.ndim == 1:
                obs = obs.reshape(-1, 1)
            ...
            log_prob, state_seq = model.decode(obs, algorithm="viterbi")
            posteriors_raw = model.predict_proba(obs)

            posteriors: list[HMMStatePosterior] = []
            for i in range(len(obs)):
                probs = posteriors_raw[i]
                total = float(probs.sum())
                if total > 0:
                    normed = tuple(float(p / total) for p in probs)
                else:
                    normed = tuple(1.0 / spec.n_components for _ in range(spec.n_components))
                posteriors.append(
                    HMMStatePosterior(
                        step_index=i,
                        state_probabilities=normed,
                    )
                )

            return HMMInferenceResult(
                viterbi_path=tuple(int(s) for s in state_seq),
