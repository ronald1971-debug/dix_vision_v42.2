...
class DynamicCorrelationClusterer:
    ...
    def update_correlation_matrix(
        self, correlation_matrix: dict[tuple[str, str], float], timestamp_ns: int
    ...
    def _hierarchical_clustering(
        self, assets: list[str], correlation_matrix: dict[tuple[str, str], float]
        ...
        for i, cluster_assets in enumerate(clusters):
            if len(cluster_assets) >= self._min_cluster_size:
                cluster = self._build_correlation_cluster(cluster_assets, correlation_matrix, i)
    ...
    def _build_correlation_cluster(
        self, assets: set[str], correlation_matrix: dict[tuple[str, str], float], cluster_id: int
    ) -> CorrelationCluster:
    ...
    def _detect_correlation_regime(
        ...
        if len(all_correlations) > 1:
            variance = sum((c - avg_correlation) ** 2 for c in all_correlations) / len(
                all_correlations
            )
        ...
        dominant_clusters = tuple(
            sorted(
                [c.cluster_id for c in clusters],
                key=lambda cid: (
                    len(self._current_clusters[cid].assets) if cid in self._current_clusters else 0
                ),
                reverse=True,
            )
        )[:3]
...
__all__ = ["CorrelationCluster", "CorrelationRegime", "DynamicCorrelationClusterer"]
