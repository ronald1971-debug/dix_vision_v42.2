...
def test_world_aware_approval_queue():
    ...
    try:
        ...
        for i, req in enumerate(prioritized, 1):
            logger.info(
                f"  {i}. {req.request_id} ({req.approval_type.value}, priority: {req.priority.value})"
            )
...
def test_world_aware_proposal_parser():
    ...
    try:
        from intelligence_engine.cognitive.proposal_parser import ProposalParser, WorldContext
...
def test_world_aware_trader_modeling():
    ...
    try:
        ...
        logger.info(
            f"  Regime at modeling: {profile.metadata.get('regime_at_modeling', 'unknown')}"
        )
...
def test_end_to_end_world_integration():
    ...
    try:
        ...
        logger.info(
            f"World integration enabled: {approval_queue._world_integration_bridge is not None}"
        )

        logger.info("\n--- Component 2: Approval Edge ---")
        logger.info(
            f"World integration enabled: {approval_edge._world_integration_bridge is not None}"
        )

        logger.info("\n--- Component 3: Proposal Parser ---")
        logger.info(
            f"World integration enabled: {proposal_parser._world_integration_bridge is not None}"
        )

        logger.info("\n--- Component 4: Trader Analyzer ---")
        logger.info(
            f"World integration enabled: {trader_analyzer._world_integration_bridge is not None}"
        )
