...
class CurrencyPair:
    ...
    def is_valid_currency(self) -> bool:
        ...
        return self.base_currency in valid_currencies and self.quote_currency in valid_currencies
...
class ForexDomainAdapter(DomainAdapter):
    ...
    def create_currency_pair_instrument(
        self, base_currency: str, quote_currency: str, exchange: str = "FX", leverage: float = 100.0
    ) -> Dict[str, Any]:
        ...
        if not currency_pair.is_valid_currency():
            logger.error(
                "Invalid currency pair", base_currency=base_currency, quote_currency=quote_currency
            )
        ...
        logger.info(
            "Currency pair instrument created",
            instrument_id=instrument_id,
            symbol=currency_pair.pair_symbol,
            leverage=leverage,
        )
