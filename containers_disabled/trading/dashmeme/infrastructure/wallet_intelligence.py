...
class WalletIntelligenceSystem:
    ...
    def _determine_wallet_type(self, wallet_address: str) -> WalletType:
        ...
        if wallet_address in self.tracked_wallets:
            ...
            if (
                wallet_data.get("transaction_frequency", 0) > 100
    ...
    def _analyze_wallet_behavior(self, wallet_address: str) -> WalletBehavior:
        ...
        for i, tx in enumerate(transactions):
            if tx.transaction_type == "buy":
                ...
                for j in range(i + 1, len(transactions)):
                    if (
                        transactions[j].transaction_type == "sell"
                        and transactions[j].token_id == tx.token_id
                    ):
                        holding_time = transactions[j].timestamp - tx.timestamp
                        holding_times.append(
                            holding_time.total_seconds() / 3600
                        )  # Convert to hours
                        break
        ...
        if average_holding_time <= self.sniper_thresholds["max_holding_time_hours"]:
            if (
                profile.successful_trades / max(1, profile.total_transactions)
                > self.sniper_thresholds["success_rate_threshold"]
            ):
        ...
        if average_holding_time <= self.day_trader_thresholds["max_holding_time_hours"]:
            if (
                profile.total_transactions / max(1, (datetime.now() - profile.first_seen).days)
                >= self.day_trader_thresholds["min_trades_per_day"]
            ):
    ...
    def identify_suspicious_wallets(self, min_suspicious_activity: int = 10) -> List[str]:
        ...
        for wallet_address, transactions in self.wallet_transactions.items():
            ...
            rapid_dumps = sum(
                ...
                if tx.transaction_type == "sell" and tx.amount > self.config.suspicious_threshold
            )
    ...
    def get_wallet_summary(self) -> Dict[str, Any]:
        ...
        total_transactions = sum(
            len(transactions) for transactions in self.wallet_transactions.values()
        )
        ...
        profitable_wallets = sum(
            1 for profile in self.wallet_profiles.values() if profile.profit_loss > 0
        )
