...
logger = logging.getLogger('pytest_health_check')
...
def main():
    logger.info(f"pytest Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("pytest Container is healthy")
        sys.exit(0)
    else:
        logger.error("pytest Container health check failed")
        sys.exit(1)
