...
logger = logging.getLogger('grpc_health_check')

def check_imports():
    try:
        logger.info("gRPC import successful")
        return True
...
def main():
    logger.info(f"gRPC Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("gRPC Container is healthy")
        sys.exit(0)
    else:
        logger.error("gRPC Container health check failed")
        sys.exit(1)
