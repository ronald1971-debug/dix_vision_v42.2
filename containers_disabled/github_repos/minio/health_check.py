...
logger = logging.getLogger('minio_health_check')

def check_imports():
    try:
        logger.info("MinIO import successful")
        return True
    except ImportError as e:
        logger.error(f"MinIO import failed: {e}")
        return False
...
def main():
    logger.info(f"MinIO Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        ...
        logger.error("MinIO Container health check failed")
        sys.exit(1)
