...
logger = logging.getLogger('selenium_health_check')
...
def main():
    logger.info(f"Selenium Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Selenium Container is healthy")
        sys.exit(0)
    else:
        logger.error("Selenium Container health check failed")
        sys.exit(1)
