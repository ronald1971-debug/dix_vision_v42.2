#!/usr/bin/env python3
"""
duckdb Container Health Check

This script performs health checks for the duckdb container.
"""
...
logger = logging.getLogger('duckdb_health_check')
...
def check_governance_wrapper():
    ...
    try:
        ...
        from duckdb_governance_wrapper import DuckdbGovernanceWrapper
        
        wrapper = DuckdbGovernanceWrapper(PermissionLevel.READ_ONLY)
...
def check_domain_adapter():
    ...
    try:
        from duckdb_domain_adapter import DuckdbDomainAdapter
        adapter = DuckdbDomainAdapter()
...
def check_configuration():
    ...
    import os
    config_path = "/app/config/duckdb_config.yaml"
...
def main():
    ...
    logger.info(f"duckdb Container Health Check - {datetime.utcnow().isoformat()}")
    ...
    if passed == total:
        logger.info("duckdb Container is healthy")
        sys.exit(0)
    else:
        logger.error("duckdb Container health check failed")
        sys.exit(1)
