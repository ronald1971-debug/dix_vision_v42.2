...
logger = logging.getLogger('telegrambot_health_check')

def check_imports():
    try:
        logger.info("TelegramBot import successful")
        return True
...
def check_governance_wrapper():
    try:
        ...
        from telegrambot_governance_wrapper import TelegramBotGovernanceWrapper
...
def check_domain_adapter():
    try:
        from telegrambot_domain_adapter import TelegramBotDomainAdapter
...
def main():
    logger.info(f"TelegramBot Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("TelegramBot Container is healthy")
        sys.exit(0)
    else:
        logger.error("TelegramBot Container health check failed")
        sys.exit(1)
