...
logger = logging.getLogger('vault_health_check')

def check_imports():
    try:
        logger.info("Vault import successful")
        return True
...
def main():
    logger.info(f"Vault Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Vault Container is healthy")
        sys.exit(0)
    else:
        logger.error("Vault Container health check failed")
        sys.exit(1)
