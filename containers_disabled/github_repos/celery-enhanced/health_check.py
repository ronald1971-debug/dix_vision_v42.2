...
logger = logging.getLogger('celery_enhanced_health_check')

def check_imports():
    try:
        logger.info("Celery Enhanced import successful")
        return True
    except ImportError as e:
        logger.error(f"Celery Enhanced import failed: {e}")
        return False
...
def main():
    logger.info(f"Celery Enhanced Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Celery Enhanced Container is healthy")
        sys.exit(0)
    else:
        logger.error("Celery Enhanced Container health check failed")
        sys.exit(1)
