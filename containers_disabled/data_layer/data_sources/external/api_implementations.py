...
class BaseAPIAdapter:
    ...
    def _track_request(self, success: bool, latency_ms: float) -> None:
        ...
        self._request_history.append(
            {"success": success, "latency_ms": latency_ms, "timestamp": time.time()}
        )
            ...
            with urllib.request.urlopen(full_url, timeout=10) as response:
                ...
                if data.get("observations") and len(data["observations"]) > 0:
                    ...
                    return {
                        ...
                        "indicator": series_id,
                        "value": (
                            float(latest.get("value", 0.0)) if latest.get("value") != "." else 0.0
                        ),
                        "timestamp_ns": self._get_timestamp_ns(),
                    }
                else:
        ...
        try:
            ...
            with urllib.request.urlopen(full_url, timeout=10) as response:
                ...
                if (
                    ...
                    and f"{to_curr.upper()}" in data["conversion_rates"]
                ):
                    return {
...
class PerplexityAdapter(BaseAPIAdapter):
    ...
    def search(
        self, query: str, model: str = "llama-3.1-sonar-small-128k-online"
    ) -> dict[str, Any]:
