...
Single source of truth for all runtime events (high frequency).
Events: MARKET, SYSTEM, GOVERNANCE, HAZARD.

DUAL-LEDGER NOTE
----------------
This is the *event store* — one of two complementary hash-chained
SQLite ledgers in DIX VISION:

  1. **This file** (event store) — all runtime events (every tick,
     signal, hazard, wallet cap change). Written via
     ``state/ledger/writer.get_writer()``.

  2. **governance_engine/control_plane/ledger_authority_writer.py**
     (authority ledger) — governance authority decisions only (mode
     transitions, strategy lifecycle, HMAC-signed operator approvals).
     Written exclusively by ``GovernanceEngine`` (GOV-CP-05).

These two chains are NOT duplicates. The event store is high-frequency
operational telemetry; the authority ledger is the low-frequency
signed decision audit trail. A unified read-only query surface over
both chains lives in ``state/ledger/bridge.LedgerBridge``.
``state/ledger/reader.LedgerReader`` already exposes the authority
surface via ``authority_entries()``; ``bridge.py`` merges both.
"""
...
class EventStore:
    ...
    def query(self, event_type: str = None, source: str = None, limit: int = 100) -> list[dict]:
        ...
        params.append(limit)
        cur = self._conn.execute(f"SELECT * FROM events {where} ORDER BY id DESC LIMIT ?", params)
        cols = [d[0] for d in cur.description]
        return [dict(zip(cols, row, strict=False)) for row in cur.fetchall()]
...
def append_event(
    event_type: str, sub_type: str, source: str, payload: dict[str, Any]
) -> LedgerEvent:
