...
class CrowdPsychologyEngine:
    ...
    # ------------------------------------------------------------------
    def tick(self, ts_ns: int, price_return: float = 0.0, market_vol: float = 0.02) -> None:
        try:
            with self._lock:
                self._tick_count += 1
                ...
                self._fear_greed = max(
                    0.0,
                    min(
                        100.0,
                        self._fear_greed + amplified_pressure * 5.0 + self._rng.gauss(0.0, 1.0),
                    ),
                )

                # Herding coefficient adapts: stronger in extremes
                vol_boost = market_vol / 0.02
                self._herding_coeff = max(
                    0.05,
                    min(
                        0.95,
                        self._herding_coeff + (vol_boost - 1.0) * 0.01 + self._rng.gauss(0.0, 0.01),
                    ),
                )
