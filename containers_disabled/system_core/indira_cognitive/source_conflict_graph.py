...
class SourceConflictGraph:
    ...
    def add_knowledge(
        self, knowledge_id: str, source: str, value: Any, confidence: float, timestamp: datetime
    ) -> None:
    ...
    def _detect_conflict(
        self, node_a: ConflictNode, node_b: ConflictNode
        ...
        if self._is_value_conflict(node_a.value, node_b.value):
            ...
            return ConflictEdge(
                ...
                severity=severity,
                metadata={"value_a": str(node_a.value), "value_b": str(node_b.value)},
            )
        ...
        if node_a.confidence > 0.8 and node_b.confidence > 0.8:
            if self._is_value_conflict(node_a.value, node_b.value):
                return ConflictEdge(
                    ...
                    metadata={"confidence_a": node_a.confidence, "confidence_b": node_b.confidence},
                )
    ...
    def _apply_resolution_strategy(
        self, node_a: ConflictNode, node_b: ConflictNode, edge: ConflictEdge
        ...
        if node_a.confidence > node_b.confidence and node_a.confidence > self._confidence_threshold:
            ...
            node_b.confidence > node_a.confidence and node_b.confidence > self._confidence_threshold
        ):
    ...
    def get_conflict_graph_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_confidence_nodes": len(
                [node for node in self._nodes.values() if node.confidence > 0.8]
            ),
