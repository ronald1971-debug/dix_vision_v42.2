...
class ProfileGeneration:
    ...
    def generate_trader_profile(
        ...
        if performance_metrics.total_trades < self.config.min_trades_for_profiling:
            raise ValueError(
                f"Insufficient trades for profiling: {performance_metrics.total_trades}"
            )
        ...
        psychological_profile = (
            self._generate_psychological_profile(performance_metrics, decision_patterns)
            if self.config.enable_psychological_profiling
            else {}
        )
    ...
    def _classify_trader_style(
        self, decision_patterns: List[DecisionPattern], performance_metrics: PerformanceMetrics
    ) -> TraderStyle:
        """
        Classify trader style (real style classification)
        ...
        for pattern in decision_patterns:
            ...
            pattern_scores[pattern.pattern_type.value] += pattern.confidence * (
    ...
    def _assess_skill_level(self, performance_metrics: PerformanceMetrics) -> SkillLevel:
        ...
        sharpe_component = min(
            1.0, (performance_metrics.sharpe_ratio + 2) / 4
        )  # Normalize to [0,1]
        ...
        consistency_component = (
            1.0 - performance_metrics.max_drawdown
    ...
    def _determine_risk_tolerance(
        self, performance_metrics: PerformanceMetrics, decision_patterns: List[DecisionPattern]
    ) -> RiskTolerance:
        """
        Determine risk tolerance (real risk tolerance assessment)
    ...
    def _calculate_profile_confidence(
        ...
        overall_confidence = (
            0.4 * data_confidence + 0.4 * pattern_confidence + 0.2 * reliability_confidence
        )
    ...
    def _extract_behavioral_traits(
        ...
        if performance_metrics.avg_hold_time_hours > 24:  # > 1 day
            traits["patience"] = min(
                1.0, performance_metrics.avg_hold_time_hours / 168
            )  # Normalize to weeks
        else:
        ...
        if decision_patterns:
            ...
            traits["adaptability"] = min(
                1.0, pattern_diversity / 5
            )  # More patterns = higher adaptability
        else:
    ...
    def _generate_psychological_profile(
        ...
        ):  # High Sharpe ratio indicates good risk management
    ...
    def _calculate_profile_similarity(
        self, profile1: TraderProfile, profile2: TraderProfile
    ) -> float:
        ...
        skill_distance = abs(
            skill_levels.index(profile1.skill_level) - skill_levels.index(profile2.skill_level)
        )
