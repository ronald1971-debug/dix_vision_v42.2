...
class MarketOpportunityIdentification:
            ...
            if confidence >= self.config.min_confidence:
                ...
                if risk_reward_ratio >= self.config.min_risk_reward_ratio:
                    opportunity = MarketOpportunity(
                        ...
                        metadata={
                            "breakout_type": "bullish_bollinger",
            ...
            if confidence >= self.config.min_confidence:
                ...
                if risk_reward_ratio >= self.config.min_risk_reward_ratio:
                    opportunity = MarketOpportunity(
                        ...
                        timeframe="short_term",
                        metadata={
                            "breakout_type": "bearish_bollinger",
                            "sma_20": latest_sma_20,
                            "sma_50": latest_sma_50,
                        },
                    )
    ...
    def _calculate_breakout_confidence(self, market_data: pd.DataFrame, direction: str) -> float:
        ...
        overall_confidence = (
            0.4 * volume_confirmation + 0.3 * momentum_confidence + 0.3 * trend_confidence
        )

        return overall_confidence

    def identify_support_resistance_opportunities(
        self, market_data: pd.DataFrame, symbol: str
    ) -> List[MarketOpportunity]:
        """
        ...
        logger.info(
            "Support/resistance opportunities identified",
            symbol=symbol,
            opportunities_found=len(opportunities),
        )
    ...
    def identify_trend_reversal_opportunities(
        self, market_data: pd.DataFrame, symbol: str
    ) -> List[MarketOpportunity]:
        """
        ...
        if (
            latest_rsi < 30
            and sma_20.iloc[-1] > sma_50.iloc[-1]
        ...
        logger.info(
            "Trend reversal opportunities identified",
            symbol=symbol,
            opportunities_found=len(opportunities),
        )
    ...
    def identify_volatility_opportunities(
        self, market_data: pd.DataFrame, symbol: str
    ) -> List[MarketOpportunity]:
        """
        ...
        logger.info(
            "Volatility opportunities identified",
            symbol=symbol,
            opportunities_found=len(opportunities),
        )
