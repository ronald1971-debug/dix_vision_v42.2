...
class PortfolioPerformanceAttribution:
    ...
    def calculate_simple_attribution(
        self, portfolio_returns: pd.Series, benchmark_returns: pd.Series, holdings: Dict[str, float]
        ...
        if len(portfolio_returns) < self.config.min_data_points:
            raise ValueError(
                f"Insufficient data for attribution: {len(portfolio_returns)} < {self.config.min_data_points}"
            )
        ...
        attribution = PerformanceAttribution(
            ...
            metadata={"excess_return": excess_return, "benchmark_return": benchmark_total_return},
        )

        logger.info(
            "Simple attribution calculated",
    ...
    def calculate_brinson_attribution(
        ...
        logger.info(
            "Brinson attribution calculated",
    ...
    def calculate_multi_factor_attribution(
        ...
        if len(portfolio_returns) < self.config.min_data_points:
            raise ValueError(
                f"Insufficient data for multi-factor attribution: {len(portfolio_returns)} < {self.config.min_data_points}"
            )
        ...
        logger.info(
            "Multi-factor attribution calculated",
            r_squared=r_squared,
    ...
    def calculate_risk_adjusted_attribution(
        ...
        if portfolio_volatility > 0:
            sharpe_ratio = (
                portfolio_returns.mean() - 0.02 / 252
            ) / portfolio_volatility  # 2% risk-free rate
        else:
            sharpe_ratio = 0.0
        ...
        logger.info(
            "Risk-adjusted attribution calculated",
