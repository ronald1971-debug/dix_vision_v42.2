...
class ResolutionGenerator:
    ...
    def generate(self, root_cause: RootCause, impact: ImpactAssessment) -> Resolution:
        ...
        with self._lock:
            ...
            is_safe = resolution_type == "MANUAL" or (
                resolution_type == "AUTOMATED" and confidence > 0.8
            )
...
class RollbackManager:
    ...
    def safe_deploy(
        self, resolution: Resolution, deployment_function: Callable[[], bool]
    ) -> HealingResult:
        """
    ...
    def _create_pre_deployment_checkpoint(self, resolution: Resolution) -> None:
        ...
        logger.info(
            f"[ROLLBACK_MANAGER] Created pre-deployment checkpoint for {resolution.resolution_id}"
        )
...
class SelfHealingSystem:
            ...
            for anomaly in anomalies:
                ...
                if resolution.is_safe:
                    healing_result = self._rollback_manager.safe_deploy(
                        resolution, lambda: self._deploy_resolution(resolution, root_cause)
                    )
                else:
    ...
    def _should_auto_heal(self, anomaly: Anomaly) -> bool:
        ...
        if (
            ...
            and self._critical_threshold == AnomalySeverity.HIGH
        ):
            return False
