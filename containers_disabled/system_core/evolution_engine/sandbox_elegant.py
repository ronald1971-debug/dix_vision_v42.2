...
def elegantrl_agent_trainer(
    *,
    ...
    class _ElegantRLAgentTrainer:
        ...
        def train(  # pragma: no cover - exercised when elegantrl present
            self,
            env: DIXStrategyEnv,
            *,
            episode_config: EpisodeConfig,
            ...
            # ------------------------------------------------------------------
            # Build an ElegantRL-style agent network.
            # ElegantRL's agents (AgentPPO, AgentSAC, etc.) can be instantiated
            # directly — we use elegantrl's Arguments/Config object to pass
            # hyperparameters, then drive the explore_env / update_net loop.
            # ------------------------------------------------------------------
            try:
                # ElegantRL v2.x: elegantrl.train.config.Arguments
                from elegantrl.train.config import (
                    Arguments as ElegantConfig,  # type: ignore[import-not-found]
                )
            except ImportError:
                try:
                    # ElegantRL v3.x: elegantrl.Config
                    from elegantrl import (
                        Config as ElegantConfig,  # type: ignore[import-not-found, no-redef]
                    )
                except ImportError:
                    ElegantConfig = None  # type: ignore[assignment, misc]

            # Map ElegantAgentKind to the agent class
            _agent_class_map = {
                ElegantAgentKind.PPO: "AgentPPO",
                ElegantAgentKind.SAC: "AgentSAC",
                ElegantAgentKind.TD3: "AgentTD3",
                ElegantAgentKind.DDPG: "AgentDDPG",
                ElegantAgentKind.DQN: "AgentDQN",
            }
            ...
            is_offpolicy = arguments.agent_kind in (
                ElegantAgentKind.SAC,
                ElegantAgentKind.TD3,
                ElegantAgentKind.DDPG,
                ElegantAgentKind.DQN,
            )

            # ------------------------------------------------------------------
            # ElegantRL env wrapper: reset() → ndarray, step() → 4-tuple.
            # ElegantRL v2 uses the old Gym API (4-tuple step return).
            # ------------------------------------------------------------------
            class _ElegantEnv:
                """DIXStrategyEnv wrapped for ElegantRL's explore_env."""

                state_dim: int = obs_dim
                action_dim: int = 1  # scalar discrete index
                if_discrete: bool = True
                max_step: int = episode_config.max_steps
                target_reward: float = 1e6  # never early-stop on target

                def __init__(self) -> None:
                    self._ep_idx = 0

                @staticmethod
                def _arr(obs: Observation) -> np.ndarray:
                    return np.array(
                        [
                            float(obs.step_idx),
                            float(obs.mid_price),
            ...
            elegant_env = _ElegantEnv()

            # ------------------------------------------------------------------
            # Training loop:
            # If we have a live ElegantRL agent class, drive it via
            # init() / explore_env() / update_net().
            # Fall back to a manual PPO loop (same MLP architecture) otherwise.
            # ------------------------------------------------------------------
            ...
            if AgentClass is not None:
                try:
                    # Attempt the ElegantRL v2 agent protocol:
                    # agent.init(net_dim, state_dim, action_dim, gpu_id, args)
                    cfg_args: object
                    if ElegantConfig is not None:
                        cfg_args = ElegantConfig()
                        cfg_args.random_seed = arguments.random_seed  # type: ignore[union-attr]
                        cfg_args.target_step = arguments.target_step  # type: ignore[union-attr]
                        cfg_args.gamma = arguments.gamma  # type: ignore[union-attr]
                        cfg_args.learning_rate = arguments.learning_rate  # type: ignore[union-attr]
                        cfg_args.batch_size = arguments.batch_size  # type: ignore[union-attr]
                        cfg_args.repeat_times = arguments.repeat_times  # type: ignore[union-attr]
                    else:
                        cfg_args = None
                    ...
                    while total_steps_done < arguments.target_step * max_iters:
                        ...
                        ep_lengths.append(float(ep_len))
                        episodes_done += 1
                        iter_count += 1

                        # Update via elegantrl's update_net
                        if hasattr(agent, "update_net"):
                            losses = agent.update_net(traj)  # type: ignore[union-attr]
                            if isinstance(losses, (list, tuple)) and len(losses) >= 2:
                                critic_losses.append(float(losses[0]))
                                policy_losses.append(float(losses[1]))
                            elif isinstance(losses, (int, float)):
                                policy_losses.append(float(losses))
                                critic_losses.append(0.0)

                        if iter_count * arguments.target_step >= arguments.target_step * max_iters:
                            break
            ...
            if not agent_used:
                # ----------------------------------------------------------
                # Manual fallback: 2-hidden-layer actor-critic PPO loop.
                # Used when the elegantrl agent API is incompatible.
                # ----------------------------------------------------------
                ...
                for _win in range(max_windows):
                    ...
                    with torch.no_grad():
                        for _step in range(window):
                            ...
                            next_state, reward, done, _ = elegant_env.step(action)
                            obs_buf.append(state.copy())
                            ...
                            if done or ep_len >= arguments.max_step:
                                ep_rewards.append(ep_r)
                                ep_lengths.append(float(ep_len))
                                episodes_done += 1
                                callback.on_episode_end(
                                    ts_ns=ts_ns,
                                    episode_idx=episodes_done - 1,
                                    episode_reward=ep_r,
                    ...
                    adv: list[float] = [0.0] * n
                    gae = 0.0
                    for i in reversed(range(n)):
                        next_v = val_buf[i + 1] if i + 1 < n else 0.0
                        non_term = 0.0 if done_buf[i] else 1.0
                        delta = rew_buf[i] + gamma * next_v * non_term - val_buf[i]
                    ...
                    for _ in range(arguments.repeat_times):
                        ...
                        cl = nn.functional.mse_loss(val_b, ret_b)
                        loss = pl + value_coef * cl - entropy_coef * entropy_b
                    ...
                    if n_updates > 0:
                        policy_losses.append(batch_pl / n_updates)
                        critic_losses.append(batch_cl / n_updates)

            # ------------------------------------------------------------------
            # Serialize trained model / policy via caller-supplied artifact sink
            # ------------------------------------------------------------------
            artifact_buf = _io.BytesIO()
            if agent_used and AgentClass is not None:
                try:
                    torch.save(agent.act.state_dict(), artifact_buf)  # type: ignore[union-attr]
                except Exception:
                    torch.save({}, artifact_buf)
            else:
                torch.save(
                    {"actor": actor.state_dict(), "critic": critic.state_dict()},  # type: ignore[union-attr]
                    artifact_buf,
                )
            ...
            final_cl = float(critic_losses[-1]) if critic_losses else 0.0

            return ElegantSandboxMetrics(
                episodes_completed=episodes_done,
                total_steps_executed=total_steps_done,
                mean_episode_reward=mean_rew,
                mean_episode_length=mean_len,
                best_episode_reward=best_rew,
                final_critic_loss=final_cl,
