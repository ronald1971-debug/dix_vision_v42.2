...
class CCXTExchangeProvider(DataProvider):
    ...
    async def fetch_ohlcv(
        ...
        try:
            ...
            ohlcv = await self._ccxt_exchange.fetch_ohlcv(
                symbol, timeframe=ccxt_timeframe, since=since, limit=1000  # CCXT default limit
            )
    ...
    async def fetch_tick_data(
        ...
        try:
            ...
            trades = await self._ccxt_exchange.fetch_trades(symbol, since=since, limit=1000)
...
class ProviderManager:
    ...
    async def bootstrap_all_providers(self, **kwargs: Any) -> Dict[str, bool]:
        ...
        logger.info(
            f"[PROVIDER_MANAGER] Bootstrap complete: {successful_count}/{len(bootstrap_results)} successful"
        )
    ...
    def get_primary_provider(self, data_type: DataType) -> Optional[DataProvider]:
        ...
        primary_candidates = [
            ...
            if self._provider_configs[provider._config.provider_id].priority
    ...
    async def fetch_with_failover(
        self, data_type: DataType, fetch_func, *args, **kwargs
    ) -> Optional[Any]:
        ...
        for provider in providers:
            ...
            try:
                ...
                if result is not None:
                    logger.debug(
                        f"[PROVIDER_MANAGER] Successfully fetched data from {provider._config.provider_id}"
                    )
                    return result
            except Exception as e:
                last_error = e
                logger.warning(
                    f"[PROVIDER_MANAGER] Provider {provider._config.provider_id} failed: {e}"
                )
                continue
    ...
    def provider_summary(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        if self._providers:
            ...
            avg_success_rate = np.mean(
                [p.get_metrics().success_rate for p in self._providers.values()]
            )
            avg_latency = np.mean(
                [p.get_metrics().average_latency_ms for p in self._providers.values()]
            )
            ...
            summary["aggregate_statistics"] = {
                ...
                "active_providers": len(
                    [p for p in self._providers.values() if p.get_status() == ProviderStatus.ACTIVE]
                ),
            }
