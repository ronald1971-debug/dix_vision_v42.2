...
class AuthenticationService:
    ...
    def _initialize_default_operators(self):
        ...
        # Read-only operator
        self._operator_permissions["readonly_operator"] = [Permission.READ_ONLY]
    ...
    def authenticate(
        self, operator_id: str, credentials: Dict[str, Any], context: Dict[str, Any] = None
    ) -> Tuple[AuthenticationStatus, Optional[AuthenticationToken]]:
    ...
    def _create_session(
        self, operator_id: str, token: AuthenticationToken, context: Dict[str, Any] = None
        ...
        with self._lock:
            ...
            if operator_id and token.operator_id != operator_id:
                ...
                if Permission.OPERATOR_OVERRIDE not in permissions:
                    logger.warning(
                        f"[AUTH_SERVICE] Unauthorized revocation attempt by {operator_id}"
                    )
                    self._log_security_event(
                        "unauthorized_revocation_attempt", operator_id, {"token_id": token_id}
                    )
                    return False
            ...
            self._log_audit_event(
                "token_revoked", token.operator_id, token_id, {"revoked_by": operator_id}
            )
    ...
    def _log_security_event(
        self, event_type: str, operator_id: str, context: Dict[str, Any] = None
    ):
    ...
    def cleanup_expired_tokens(self) -> int:
        ...
        with self._lock:
            ...
            expired_tokens = [
                t for t in self._tokens.values() if t.expires_at <= now and t.is_active
            ]
            ...
            logger.info(
                f"[AUTH_SERVICE] Cleaned up {cleaned_tokens} expired tokens and {len(expired_sessions)} expired sessions"
            )
