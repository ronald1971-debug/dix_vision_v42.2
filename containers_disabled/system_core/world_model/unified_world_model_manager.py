...
class UnifiedWorldModelManager:
    ...
    def initialize_unified_world_model(self) -> UnifiedWorldModelState:
        ...
        logger.info(
            f"[UNIFIED_WORLD_MODEL_MANAGER] Unified world model initialized with {state.total_systems_registered} systems"
        )
    ...
    def get_system_state(self) -> UnifiedWorldModelState:
        ...
        shared_reality_stats = (
            self._shared_reality.get_system_statistics() if self._shared_reality else {}
        )
        ...
        total_systems = sum(
            system_data.get("count", 0) for system_data in registered_systems.values()
        )
