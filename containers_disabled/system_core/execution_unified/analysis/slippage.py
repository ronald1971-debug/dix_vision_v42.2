"""execution_engine.analysis.slippage — pre-trade slippage + market-impact estimator.
...
def estimate(
    *,
    ...
    if vol_bps_daily > 0 and duration_sec > 0:
        vol_comp = lambda_vol * vol_bps_daily * math.sqrt(max(0.0, duration_sec / 86400.0))
...
def worst_acceptable_price(
    *, mid: float, side: str, exp_slip_bps: float, max_extra_bps: float = 20.0
) -> float:
