...
Connects to Binance via CCXT when credentials are available.
Falls back to paper mode (no network) when CCXT is absent or
...
class BinanceAdapter(BaseAdapter):
    """Exchange adapter for Binance (CCXT-backed)."""
    ...
    def connect(self) -> bool:
        self._ccxt_exchange, self._paper_mode = ccxt_connect(
            self.name, "binance", self.api_key, self.api_secret
        )
