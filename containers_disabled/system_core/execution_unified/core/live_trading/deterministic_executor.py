...
class DeterministicExecutor:
    ...
    async def execute_deterministically(self, execution_request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {"execution_id": execution_id, "status": "completed", "deterministic": True}
    ...
    def check_determinism(self) -> DeterminismCheckResult:
        ...
        return DeterminismCheckResult(
            ...
            violation_type=(
                DeterminismViolationType.NONE
                if self._deterministic_mode
                else DeterminismViolationType.STATE_DIFF
            ),
            reason=(
                "Deterministic mode enabled"
                if self._deterministic_mode
                else "Deterministic mode disabled"
            ),
        )
