"""
execution_engine.protections.mev_guard — MEV-aware DEX transaction wrapper.

Migrated from execution/mev_guard.py
...
def prepare_swap(
    *,
    ...
    slip = SlippageAnalyzer().estimate(
        qty=amount_in, adv_qty=adv_qty or amount_in * 100, spread_bps=spread_bps
    )
...
def validate_and_emit(swap: GuardedSwap, *, simulate: SimulateFn | None = None) -> bool:
...
__all__ = ["GuardedSwap", "private_relay_for", "prepare_swap", "validate_and_emit"]
