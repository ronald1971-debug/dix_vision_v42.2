...
class WorkloadPredictor:
    ...
    def predict(
        self, current_metrics: Dict[str, float], time_horizon_minutes: int = 30
            ...
            for metric_name, resource_type in metric_mappings:
                if metric_name in current_metrics:
                    ...
                    if len(self._workload_history[metric_name]) > self._history_size:
                        self._workload_history[metric_name] = self._workload_history[metric_name][
                            -self._history_size :
                        ]
    ...
    def optimize(
        ...
        with self._lock:
            ...
            for prediction in predicted_workload:
                ...
                improvement = (
                    abs(prediction.predicted_load - prediction.current_load)
                    / prediction.current_load
                    if prediction.current_load > 0
                    else 0.0
                )
...
class ScalingOptimizer:
    ...
    def generate_scaling_plan(self, optimized_allocation: ResourceOptimization) -> ScalingPlan:
        """
...
class AdaptiveResourceManager:
    ...
    def optimize_resources(self, execution_metrics: Dict[str, float]) -> Dict[str, Any]:
        """
    ...
    def _estimate_improvement(
        self, optimizations: List[ResourceOptimization], predictions: List[WorkloadPrediction]
    ) -> float:
        ...
        avg_confidence = (
            sum(p.confidence for p in predictions) / len(predictions) if predictions else 0.5
        )
