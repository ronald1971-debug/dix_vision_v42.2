...
class DynamicCapabilityManager:
        ...
        try:
            ...
            for attr_name in dir(CognitiveFeatureFlags):
                if not attr_name.startswith("_"):
                    ...
                    logger.debug(
                        f"[DYNAMIC] Initialized {flag.name} as {'enabled' if enabled else 'disabled'}"
                    )
    ...
    def execute_decision(self, decision: CapabilityDecision) -> bool:
        ...
        if not self._auto_apply_enabled:
            logger.info(
                f"[DYNAMIC] Auto apply disabled, not executing decision for {decision.capability_name}"
            )
            return False
    ...
    def get_enabled_capabilities(self) -> list[str]:
        ...
        return [name for name, state in self._capability_states.items() if state.enabled]

    def get_disabled_capabilities(self) -> list[str]:
        """Get list of disabled capabilities."""
        return [name for name, state in self._capability_states.items() if not state.enabled]
