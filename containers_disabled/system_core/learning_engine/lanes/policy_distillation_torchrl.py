...
# ---------------------------------------------------------------------------
# Pure-Python fallback distiller (no torch required)
# ---------------------------------------------------------------------------
def null_torchrl_distiller() -> TorchRLDistiller:
    """Return a deterministic no-op distiller that requires no torch install.

    Produces zeroed :class:`TrainingMetrics` and a digest-only
    :class:`TorchRLPolicyArtifact` derived from the rollout canonical
    bytes.  Used in environments where torch/torchrl are not installed,
    offline audit replays, and unit tests that only need the coordinator
    logic.
    """
    ...
    class _NullDistiller:
        def train(
            ...
            *,
            ...
            # Digest from rollout canonical text + seed so replay is stable.
            rollout_text = _canonical_rollout(tuple(rollouts))
            content_digest = _blake2b_16(f"null;seed={seed};{rollout_text}")
            artifact = TorchRLPolicyArtifact(
                backend="null-distiller",
    ...
    return _NullDistiller()


# ---------------------------------------------------------------------------
# Lazy factory — production seam (torch required)
# ---------------------------------------------------------------------------
    ...
    class _LiveDistiller:
        def train(
            ...
            *,
            ...
            for ro in rollouts:
                rewards = [s.reward for s in ro.steps]
                values = [s.state_value for s in ro.steps]
                dones = [s.done for s in ro.steps]
                advs = compute_advantages(
                    rewards=rewards,
                    values=values,
                    dones=dones,
                    bootstrap_value=ro.bootstrap_value,
                    gamma=config.gamma,
                    gae_lambda=config.gae_lambda,
                )
                rets = [a + v for a, v in zip(advs, values)]
                for step, adv, ret in zip(ro.steps, advs, rets):
                    obs_list.append(list(step.obs))
                    act_list.append(list(step.action))
                    lp_list.append(step.sample_log_prob)
                    val_list.append(step.state_value)
                    adv_list.append(adv)
                    ret_list.append(ret)
                boot_vals.append(ro.bootstrap_value)
            ...
            act_t = torch.tensor(act_list, dtype=torch.float32, device=device)
            old_lp_t = torch.tensor(lp_list, dtype=torch.float32, device=device)
            old_val_t = torch.tensor(val_list, dtype=torch.float32, device=device)
            ...
            acc_lo = acc_lc = acc_le = acc_kl = acc_cf = 0.0
            total_updates = 0

            # ── ClipPPOLoss-style training epochs ─────────────────────────
            for epoch in range(config.num_epochs):
                gen = torch.Generator()
                gen.manual_seed(seed + epoch)
                perm = torch.randperm(n, generator=gen)

                e_lo = e_lc = e_le = e_kl = 0.0
                e_upd = 0
                ...
                for start in range(0, n, mb_size):
                    ...
                    std_a = log_std.exp().expand_as(mean_a)
                    dist = torch.distributions.Normal(mean_a, std_a)
                    new_lp = dist.log_prob(mb_act).sum(-1)
                    entropy = dist.entropy().sum(-1).mean()

                    # ClipPPOLoss objective
                    ...
                    # Critic (ValueOperator analogue)
                    new_val = critic_net(mb_obs).squeeze(-1)
                    ...
                    acc_lo += float(loss_obj)
                    acc_lc += float(loss_cri)
                    acc_le += float(loss_ent)
                    acc_kl += kl_mb
                    acc_cf += cf_mb
                    total_updates += 1
                    e_lo += float(loss_obj)
            ...
            metrics = TrainingMetrics(
                mean_reward=mean_r,
                best_reward=best_r,
                loss_objective=acc_lo / nd,
                loss_critic=acc_lc / nd,
                loss_entropy=acc_le / nd,
                approx_kl=acc_kl / nd,
                clip_fraction=acc_cf / nd,
                total_timesteps=n,
            )
