...
class ProductionReinforcementLearner:
    ...
    def train_q_learning(
        self, env_id: str, n_states: int, n_actions: int, n_episodes: Optional[int] = None
    ) -> RLTrainingResult:
            ...
            for episode in range(n_episodes):
                ...
                while not done and steps < self._max_steps_per_episode:
                    ...
                    q_table[state, action] = q_table[state, action] + self._learning_rate * (
                        reward
                        + self._discount_factor * np.max(q_table[next_state])
    ...
    def train_dqn(
        self, env_id: str, state_dim: int, action_dim: int, n_episodes: Optional[int] = None
    ) -> RLTrainingResult:
                ...
                while not done and steps < self._max_steps_per_episode:
                    ...
                    if len(experience_buffer) >= 32:  # Batch size
                        ...
                        self._update_network_batch(
                            network_weights, experience_buffer, batch, state_dim, action_dim
                        )
    ...
    def _simulate_dqn_step(
        self, state: List[float], action: int, state_dim: int
