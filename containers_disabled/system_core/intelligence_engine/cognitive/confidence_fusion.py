...
class BayesianFusion:
    ...
    def fuse_confidences(
        self, confidences: List[float], likelihoods: Optional[List[float]] = None
    ) -> FusionResult:
        ...
        reasoning = (
            f"Bayesian fusion: prior={self.prior_belief:.2f}, "
...
class DempsterShaferFusion:
    ...
    def fuse_confidences(
        self, confidences: List[float], masses: Optional[List[Dict[str, float]]] = None
    ) -> FusionResult:
        ...
        reasoning = (
            f"Dempster-Shafer fusion: belief={belief:.2f}, "
    ...
    def _confidence_to_mass(self, confidence: float) -> Dict[str, float]:
        ...
        return {"true": belief_true, "false": belief_false, "uncertain": belief_uncertain}
...
class WeightedAverageFusion:
    ...
    def fuse_confidences(
        self, confidences: List[float], weights: Optional[List[float]] = None
    ) -> FusionResult:
        ...
        reasoning = (
            f"Weighted average fusion: {weighted_confidence:.2f}, " f"n_sources={len(confidences)}"
        )

        logger.debug(
            f"[WEIGHTED_FUSION] Fused {len(confidences)} confidences: {weighted_confidence:.2f}"
        )
...
class ConservativeFusion:
    ...
    def fuse_confidences(self, confidences: List[float]) -> FusionResult:
        ...
        reasoning = f"Conservative fusion: min={min_confidence:.2f}, " f"range={uncertainty:.2f}"

        logger.debug(
            f"[CONSERVATIVE_FUSION] Fused {len(confidences)} confidences: {min_confidence:.2f}"
        )
...
class AdaptiveFusion:
    ...
    def fuse_confidences(
        self, confidences: List[float], context: Dict[str, Any] = None
    ) -> FusionResult:
