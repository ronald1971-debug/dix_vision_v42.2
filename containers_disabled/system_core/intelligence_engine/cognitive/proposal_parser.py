...
class ProposalParser:
    ...
    def __init__(self, enable_semantic_parsing: bool = False, **kwargs: Any):
        ...
        if WORLD_MODEL_AVAILABLE:
            try:
                ...
                logger.warning(
                    f"[PROPOSAL_PARSER] Failed to initialize world model integration: {e}"
                )
    ...
    def _initialize_validation_rules(self):
        ...
        self._validation_rules["strategy_deployment"] = [
            ...
            ValidationRule(
                ...
                rule_type="enum",
                rule_parameters={
                    "values": ["momentum", "mean_reversion", "trend_following", "arbitrage"]
                },
    ...
    def extract_proposal(
        self, text: str, proposal_type: ProposalType = None, proposer_id: str = None, **kwargs: Any
    ) -> ParsedProposal:
        ...
        try:
            ...
            self._update_metrics(
                extraction_time, True, proposal_type, extraction_method, validation_status
            )
            ...
            logger.info(
                f"[PROPOSAL_PARSER] Extracted proposal: {proposal_type.value} (confidence: {confidence_score:.2f})"
            )
            ...
            self._update_metrics(
                ...
                False,
                proposal_type,
                ExtractionMethod.KEY_VALUE_PARSING,
    ...
    def _detect_proposal_type(self, text: str) -> ProposalType:
        ...
        if any(keyword in text_lower for keyword in ["strategy", "deployment", "deploy"]):
            ...
            keyword in text_lower for keyword in ["architecture", "architectural", "structure"]
        ):
    ...
    def _extract_fields(
        self, text: str, proposal_type: ProposalType, extraction_method: ExtractionMethod
    ) -> Dict[str, ProposalField]:
    ...
    def _extract_json_fields(
        self, text: str, proposal_type: ProposalType
    ) -> Dict[str, ProposalField]:
    ...
    def _validate_proposal(
        self, proposal_type: ProposalType, extracted_fields: Dict[str, ProposalField]
    ...
    def _calculate_confidence_score(
        self, extracted_fields: Dict[str, ProposalField], extraction_method: ExtractionMethod
    ) -> float:
    ...
    def parse_world_enhanced_proposal(
        self, proposal_text: str, world_context: Optional[WorldContext] = None
    ) -> ParsedProposal:
        ...
        if world_context:
            ...
            enhanced_proposal = self._enhance_proposal_with_world_context(
                parsed_proposal, world_context
            )
            return enhanced_proposal

        return parsed_proposal

    def _enhance_proposal_with_world_context(
        self, proposal: ParsedProposal, world_context: WorldContext
    ) -> ParsedProposal:
        ...
        logger.debug(
            f"[PROPOSAL_PARSER] Enhanced proposal {proposal.proposal_id} with world context"
        )

        return proposal

    def _extract_world_requirements(
        self, proposal: ParsedProposal, world_context: WorldContext
    ) -> List[str]:
        """Extract world-aware requirements from proposal.
        ...
        if "bullish" in proposal_text and world_context.market_regime != "bullish":
            world_requirements.append(
                "Regime misalignment: Proposal assumes bullish but world state is different"
            )

        if "bearish" in proposal_text and world_context.market_regime != "bearish":
            world_requirements.append(
                "Regime misalignment: Proposal assumes bearish but world state is different"
            )
        ...
        proposal_objectives = [
            ...
            if field.field_name in ["objective", "goal", "target"]
        ]
    ...
    def _validate_against_world_state(
        self, proposal: ParsedProposal, world_context: WorldContext
    ) -> Dict[str, Any]:
        ...
        if world_context.market_regime == "high_volatility":
            if "large position" in proposal_text or "big trade" in proposal_text:
                ...
                validation_result["errors"].append(
                    "Large positions not recommended in high volatility regime"
                )
        ...
        if world_context.liquidity_state == "low":
            if "market order" in proposal_text:
                validation_result["valid"] = False
                validation_result["errors"].append(
                    "Market orders not recommended in low liquidity regime"
                )
        ...
        if world_context.market_trend == "trending":
            if "mean reversion" in proposal_text:
                validation_result["warnings"].append(
                    "Mean reversion strategy may not align with trending market"
                )
        ...
        if any(rf in world_context.causal_factors for rf in risk_factors):
            if "aggressive" in proposal_text or "leverage" in proposal_text:
                validation_result["valid"] = False
                validation_result["errors"].append(
                    f"Risk factor '{world_context.causal_factors}' detected against aggressive strategy"
                )
    ...
    def _calculate_world_aware_confidence(
        self, proposal: ParsedProposal, world_context: WorldContext
    ) -> float:
        ...
        proposal_objectives = [
            field.field_value
            for field in proposal.extracted_fields
            if field.field_name in ["objective", "goal", "target"]
        ]
        aligned_factors = set()
        for objective in proposal_objectives:
    ...
    def _generate_world_aware_reasoning(
        self, proposal: ParsedProposal, world_context: WorldContext
    ) -> str:
