...
class DecisionFusionEngine:
    ...
    def __init__(
        self,
        default_strategy: ConflictResolutionStrategy = ConflictResolutionStrategy.COGNITIVE_PRIMACY,
    ):
        ...
        if FUSION_ENGINE_AVAILABLE:
            try:
                ...
                logger.warning(
                    f"[HYBRID_DECISION] Failed to initialize confidence fusion engine: {e}"
                )

        logger.info(
            f"[HYBRID_DECISION] Decision Fusion Engine initialized (strategy: {default_strategy.value})"
        )

    def fuse_decisions(
        self, decision_inputs: List[DecisionInput], context: Dict[str, Any] = None
    ) -> HybridDecision:
        ...
        try:
            ...
            override_inputs = [
                ...
                if inp.source in [DecisionSource.OPERATOR_OVERRIDE, DecisionSource.EMERGENCY_SYSTEM]
            ]
            ...
            constraint_inputs = [
                inp for inp in decision_inputs if inp.source == DecisionSource.GOVERNANCE_CONSTRAINT
            ]
            ...
            final_decision = self._resolve_and_fuse(
                decision_inputs, conflicts, resolution_strategy, context
            )
            ...
            logger.info(
                f"[HYBRID_DECISION] Fused decision: {final_decision.decision_type.value} (confidence: {final_decision.confidence:.2f})"
            )
    ...
    def _apply_constraints(
        self, constraint_input: DecisionInput, decision_inputs: List[DecisionInput]
    ) -> HybridDecision:
    ...
    def _group_by_decision_type(
        self, decision_inputs: List[DecisionInput]
    ) -> Dict[DecisionType, List[DecisionInput]]:
    ...
    def _detect_conflicts(
        ...
        decision_type_groups: Dict[DecisionType, List[DecisionInput]],
    ...
    def _determine_resolution_strategy(
        self, conflicts: List[DecisionConflict], context: Dict[str, Any]
    ...
    def _indicator_primary_fusion(
        ...
        indicator_inputs = [
            inp for inp in decision_inputs if inp.source == DecisionSource.INDICATOR_PROCESSING
        ]
        ...
        if self._confidence_fusion_engine and len(confidences) > 1:
            try:
                ...
                fusion_result = self._confidence_fusion_engine.fuse(
                    confidences=confidences, method=fusion_method, context=context
                )
    ...
    def _operator_escalation_fusion(
        self,
        decision_inputs: List[DecisionInput],
        conflicts: List[DecisionConflict],
        context: Dict[str, Any],
        ...
        return HybridDecision(
            ...
            source_weights={
                inp.source.value: 1.0 / len(decision_inputs) for inp in decision_inputs
            },
            conflicts_resolved=0,
            resolution_strategy=ConflictResolutionStrategy.OPERATOR_ESCALATION,
            cognitive_value_score=0.0,
    ...
    def _create_hybrid_decision(
        ...
        secondary_inputs: List[DecisionInput],
        conflicts_resolved: int,
        resolution_strategy: ConflictResolutionStrategy,
        context: Dict[str, Any],
        ...
        operator_approval_required = (
            context.get("operator_approval_required", False) if context else False
        )
        ...
        with self._lock:
            ...
            if self._metrics.total_decisions == 1:
                ...
                self._metrics.cognitive_value_improvement = (
                    0.9 * self._metrics.cognitive_value_improvement
                    + 0.1 * decision.cognitive_value_score
                )
...
class HybridDecisionEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"[HYBRID_DECISION] Hybrid Decision Engine initialized (strategy: {default_strategy.value})"
        )
    ...
    def make_decision(
        self, market_context: Dict[str, Any], additional_inputs: List[DecisionInput] = None
    ) -> HybridDecision:
