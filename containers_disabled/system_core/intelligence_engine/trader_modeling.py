...
class TraderBehaviorAnalyzer:
    ...
    def __init__(self):
        ...
        if WORLD_MODEL_AVAILABLE:
            try:
                ...
                logger.warning(
                    f"[TRADER_MODELING] Failed to initialize world model integration: {e}"
                )
    ...
    def _detect_momentum_chasing(
        self, observation: TraderObservation
    ...
    def _detect_liquidity_providing(
        self, observation: TraderObservation
    ) -> Optional[BehavioralPattern]:
        ...
        if spread_improvement > 0.6 and depth_contribution > 0.5:
            ...
            return BehavioralPattern(
                ...
                confidence=confidence,
                market_conditions=[
                    observation.market_conditions.get("liquidity_regime", "unknown")
                ],
                performance_impact=behavioral_indicators.get("performance_correlation", 0.0),
                last_detected=observation.timestamp,
                strength=confidence,
    ...
    def _detect_speculative(self, observation: TraderObservation) -> Optional[BehavioralPattern]:
        ...
        if high_frequency > 0.7 and high_leverage > 0.6:
            ...
            return BehavioralPattern(
                ...
                confidence=confidence,
                market_conditions=[
                    observation.market_conditions.get("volatility_regime", "unknown")
                ],
                performance_impact=behavioral_indicators.get("performance_correlation", 0.0),
                last_detected=observation.timestamp,
                strength=confidence,
    ...
    def analyze_observation_with_world_context(
        self, observation: TraderObservation, world_context: Optional[WorldContext] = None
        ...
        if world_context:
            ...
            enhanced_patterns = self._enhance_patterns_with_world_context(
                detected_patterns, world_context
            )
            return enhanced_patterns

        return detected_patterns

    def _enhance_patterns_with_world_context(
        self, patterns: List[BehavioralPattern], world_context: WorldContext
    ) -> List[BehavioralPattern]:
        ...
        for pattern in patterns:
            ...
            adjusted_confidence = self._calculate_world_aware_pattern_confidence(
                pattern, world_context
            )
            ...
            pattern.metadata["regime_notes"] = self._generate_regime_specific_notes(
                pattern, world_context
            )
    ...
    def _calculate_world_aware_pattern_confidence(
        self, pattern: BehavioralPattern, world_context: WorldContext
    ) -> float:
        ...
        if pattern.pattern_type == TraderBehavior.MOMENTUM_CHASING:
            if (
                world_context.market_trend == "trending"
                and world_context.market_regime == "bullish"
            ):
    ...
    def _generate_regime_specific_notes(
        self, pattern: BehavioralPattern, world_context: WorldContext
    ) -> List[str]:
    ...
    def model_trader_behavior_with_world_context(
        self, trader_data: Dict[str, Any], world_context: Optional[WorldContext] = None
    ) -> TraderProfile:
        ...
        behavior_weights = {
            behavior: sum(weights) / len(weights) for behavior, weights in pattern_weights.items()
        }
        ...
        profile_confidence = (
            sum(behavior_weights.values()) / len(behavior_weights) if behavior_weights else 0.0
        )
        ...
        logger.info(
            f"[TRADER_MODELING] Modeled trader {trader_id} with world context (confidence: {profile_confidence:.2f})"
        )

        return profile

    def predict_trader_actions_with_world_state(
        self, trader_profile: TraderProfile, world_context: Optional[WorldContext] = None
    ) -> ActionPredictions:
        ...
        logger.info(
            f"[TRADER_MODELING] Predicted action for {trader_profile.trader_id}: "
    ...
    def _calculate_world_influence(
        self, trader_profile: TraderProfile, world_context: WorldContext
    ) -> float:
    ...
    def _calculate_regime_adjustments(
        self, trader_profile: TraderProfile, world_context: WorldContext
    ) -> Dict[str, float]:
    ...
    def _calculate_causal_factor_impact(
        self, trader_profile: TraderProfile, world_context: WorldContext
    ) -> Dict[str, float]:
...
class TraderModelingSystem:
    ...
    def make_trader_observation(
        ...
        try:
            ...
            impact_estimate = self._estimate_market_impact(
                volume, behavioral_indicators, market_conditions
            )
    ...
    def _update_profile(self, profile: TraderProfile, observation: TraderObservation):
        ...
        if observation.impact_estimate == MarketImpact.HIGH:
            ...
            observation.impact_estimate == MarketImpact.MEDIUM
            and profile.impact_classification == MarketImpact.LOW
        ):
        ...
        if profile.trader_id in self._behavioral_patterns:
            ...
            if total_patterns > 0:
                ...
                sorted_behaviors = sorted(
                    profile.behavior_weights.items(), key=lambda x: x[1], reverse=True
                )
    ...
    def _update_classification(self, profile: TraderProfile):
        ...
        # High frequency
            ...
            behavior_weights.get(TraderBehavior.SPECULATIVE, 0) > 0.6
            and profile.observation_count > 100
        ):
            profile.classification = TraderClassification.HIGH_FREQUENCY
        # Institutional
        elif (
            profile.impact_classification in [MarketImpact.HIGH, MarketImpact.MEDIUM]
            and profile.observation_count > 50
        ):
    ...
    def _update_metrics(self, processing_time_ms: float, success: bool):
        ...
        with self._lock:
            ...
            if self._trader_profiles:
                avg_confidence = sum(
                    p.profile_confidence for p in self._trader_profiles.values()
                ) / len(self._trader_profiles)
