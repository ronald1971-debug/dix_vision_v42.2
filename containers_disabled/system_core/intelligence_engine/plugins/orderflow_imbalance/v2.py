...
class OrderflowImbalanceV2(MicrostructurePlugin):
    ...
    def _calculate_enhanced_imbalance(self) -> float:
        ...
        if len(self._momentum_window) >= self.momentum_window:
            ...
            enhanced_imbalance = (1 - momentum_factor) * basic_imbalance + momentum_factor * (
                momentum / abs(momentum) if momentum != 0 else 0
            )
    ...
    def _classify_flow_intensity(self, imbalance: float) -> FlowIntensity:
        ...
        if abs_imbalance < self.imbalance_threshold:
            ...
            return (
                FlowIntensity.MODERATE_BUYING if imbalance > 0 else FlowIntensity.MODERATE_SELLING
            )
    ...
    def _generate_signal(
        self, tick: MarketTick, imbalance: float, intensity: FlowIntensity
    ) -> SignalEvent | None:
