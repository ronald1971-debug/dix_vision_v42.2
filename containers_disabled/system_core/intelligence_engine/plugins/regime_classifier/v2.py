...
class RegimeClassifierV2(MicrostructurePlugin):
    ...
    _current_regime: MarketRegime = field(
        init=False, repr=False, default=MarketRegime.NORMAL_TRADING
    )
    ...
    def _adapt_thresholds(self, current_volatility: float) -> None:
        ...
        self._adaptive_vol_low = (
            1 - self.adaptation_rate
        ) * self._adaptive_vol_low + self.adaptation_rate * current_volatility * 0.5
        self._adaptive_vol_high = (
            1 - self.adaptation_rate
        ) * self._adaptive_vol_high + self.adaptation_rate * current_volatility * 2.0
    ...
    def _classify_regime(self) -> MarketRegime:
        ...
        vol_regime_map = {
            ...
            "high": (
                ...
                if current_vol < self._adaptive_vol_high * 2
                else MarketRegime.EXTREME_VOLATILITY
            ),
        }
    ...
    def _generate_regime_signal(self, tick: MarketTick, regime: MarketRegime) -> SignalEvent:
        ...
        if len(self._volatility_history) >= 5:
            vol_std = (
                statistics.stdev(self._volatility_history)
        ...
        if (
            ...
            and len(self._return_history) >= self.trend_confirmation_bars
        ):
            recent_returns = list(self._return_history)[-self.trend_confirmation_bars :]
            avg_return = sum(recent_returns) / len(recent_returns)
