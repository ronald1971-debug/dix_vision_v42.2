...
class CoordinationAdapter:
            ...
            with self._lock:
                ...
                if self._config.use_new_coordination:
                    try:
                        ...
                        logger.warning(
                            f"[COORDINATION_ADAPTER] Failed to initialize new operating modes: {e}"
                        )
                        self._new_operating_modes = None
                ...
                if self._config.enable_cognitive_economy:
                    try:
                        ...
                        logger.warning(
                            f"[COORDINATION_ADAPTER] Failed to initialize cognitive economy: {e}"
                        )
    ...
    def request_mode_transition(
        self, new_mode: str, reason: str = "", initiator: str = "system"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if (
                self._config.use_new_coordination
                and self._new_operating_modes
                and self._new_coordination_healthy
            ):
    ...
    def _try_new_mode_transition(
        self, new_mode: str, reason: str, initiator: str
    ) -> Optional[Dict]:
        ...
        try:
            ...
            if self._new_operating_modes and hasattr(
                self._new_operating_modes, "request_mode_transition"
            ):
                result = self._new_operating_modes.request_mode_transition(
                    target_mode=mode_enum, reason=reason or "adapter_requested", initiator=initiator
                )
    ...
    def _fallback_mode_transition(
        self, new_mode: str, reason: str, initiator: str, start_time_ms: float
    ) -> Dict:
        """Fallback to existing mode manager."""
        try:
    ...
    def check_cognitive_budget(
        self, operation_type: str, resource_estimate: float
    ) -> Dict[str, Any]:
        """
                ...
                try:
                    ...
                    if hasattr(cognitive_economy, "check_budget"):
                        result = cognitive_economy.check_budget(
                            operation_type=operation_type, resource_estimate=resource_estimate
                        )
    ...
    def get_current_mode(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"mode": "UNKNOWN", "mode_system": "error", "error": str(e)}

    def get_performance_metrics(self) -> Dict[str, Any]:
        """Get performance metrics for coordination."""
        avg_acl_latency = (
            sum(self._acl_latencies) / len(self._acl_latencies) if self._acl_latencies else 0.0
        )
