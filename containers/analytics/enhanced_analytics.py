...
class EnhancedRiskAnalytics:
    ...
    def calculate_information_ratio(self, portfolio_returns: List[float], benchmark_returns: List[float]) -> float:
        ...
        alpha = self.calculate_alpha(portfolio_returns, benchmark_returns, 0.0) / 100  # Remove percentage
        tracking_error = self.calculate_tracking_error(portfolio_returns, benchmark_returns) / 100  # Remove percentage
    ...
    def perform_comprehensive_risk_analysis(self, portfolio_returns: List[float],
        ...
        analysis = {
            ...
            'volatility': statistics.stdev(portfolio_returns) * 100 if len(portfolio_returns) > 1 else 0.0
        }
...
class PerformanceAttributionAnalyzer:
    ...
    def calculate_attribution(self, portfolio_returns: List[float], benchmark_returns: List[float],
                              sector_returns: Dict[str, List[float]], sector_weights: Dict[str, float],
                              strategy: str) -> PerformanceAttribution:
        ...
        logger.info("Performance attribution calculated", strategy=strategy, total_return=total_return)
...
class StrategyComparisonTool:
    ...
    def record_performance(self, strategy: str, returns: float, 
    ...
    def calculate_strategy_metrics(self, strategy: str) -> Dict[str, float]:
        ...
        metrics = {
            ...
            'sharpe_ratio': statistics.mean(returns) / statistics.stdev(returns) if statistics.stdev(returns) > 0 else 0.0,
        ...
        custom_metrics = {k: statistics.mean(v) if v else 0.0 
    ...
    def compare_strategies(self, strategies: List[str] = None) -> Dict[str, Dict[str, float]]:
        ...
        if comparison:
            rankings = {
                'best_return': max(comparison.items(), key=lambda x: x[1]['average_return'])[0] if comparison else None,
                'lowest_volatility': min(comparison.items(), key=lambda x: x[1]['volatility'])[0] if comparison else None,
                'highest_sharpe': max(comparison.items(), key=lambda x: x[1]['sharpe_ratio'])[0] if comparison else None,
                'highest_win_rate': max(comparison.items(), key=lambda x: x[1]['win_rate'])[0] if comparison else None
            }
...
if __name__ == '__main__':
    ...
    risk_analysis = risk_analytics.perform_comprehensive_risk_analysis(portfolio_returns, benchmark_returns)
    ...
    attribution = perf_analyzer.calculate_attribution(portfolio_returns, benchmark_returns, 
                                                    sector_returns, sector_weights, "momentum")
