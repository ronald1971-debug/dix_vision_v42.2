...
class OptionPosition:
    ...
    def calculate_unrealized_pnl(self) -> float:
        ...
        if self.quantity > 0:  # Long position
            return (self.current_price - self.entry_price) * self.quantity * self.contract.contract_size
        else:  # Short position
            return (self.entry_price - self.current_price) * abs(self.quantity) * self.contract.contract_size
...
class BlackScholesPricer:
    ...
    def calculate_d1(self, spot_price: float, strike_price: float, time_to_expiry: float,
                    volatility: float, risk_free_rate: float) -> float:
        """Calculate d1 parameter for Black-Scholes (real calculation)"""
        if time_to_expiry <= 0:
            return 0.0
        
        numerator = log(spot_price / strike_price) + (risk_free_rate + volatility ** 2 / 2) * time_to_expiry
    ...
    def calculate_call_price(self, spot_price: float, strike_price: float, time_to_expiry: float,
                            volatility: float, risk_free_rate: float = None) -> float:
    ...
    def calculate_put_price(self, spot_price: float, strike_price: float, time_to_expiry: float,
                           volatility: float, risk_free_rate: float = None) -> float:
    ...
    def calculate_greeks(self, spot_price: float, strike_price: float, time_to_expiry: float,
                         volatility: float, option_type: OptionType, risk_free_rate: float = None) -> OptionGreeks:
        ...
        # Theta
        if option_type == OptionType.CALL:
            theta = (-spot_price * norm.pdf(d1) * volatility / (2 * sqrt(time_to_expiry)) -
...
class BinomialTreePricer:
    ...
    def calculate_option_price(self, spot_price: float, strike_price: float, time_to_expiry: float,
        ...
        for j in range(self.steps - 1, -1, -1):
            for i in range(j + 1):
                ...
                expected_value = exp(-risk_free_rate * dt) * (p * option_values[i] + (1 - p) * option_values[i + 1])
...
class VolatilitySurface:
    ...
    def add_implied_volatility(self, underlying: str, strike: float, expiration: datetime, iv: float) -> None:
    ...
    def get_implied_volatility(self, underlying: str, strike: float, expiration: datetime) -> Optional[float]:
        """Get implied volatility for specific parameters"""
        key = (underlying, strike, expiration)
        return self.implied_volatilities.get(key)
    
    def calculate_smile_volatility(self, underlying: str, strike: float, spot_price: float,
                                  expiration: datetime, base_volatility: float = 0.20) -> float:
    ...
    def calculate_term_structure(self, underlying: str, time_to_expiry: float,
...
class OptionsDomain:
    ...
    def create_contract(self, underlying_symbol: str, option_type: OptionType,
                       exercise_style: ExerciseStyle, strike_price: float,
        ...
        logger.info("Option contract created", contract_id=contract_id, underlying=underlying_symbol)
        
        return contract
    
    def price_option(self, contract: OptionContract, spot_price: float, volatility: float,
        ...
        if contract.exercise_style == ExerciseStyle.EUROPEAN:
            ...
            price = self.binomial_pricer.calculate_option_price(
                spot_price, contract.strike_price, time_to_expiry, volatility,
                contract.option_type, contract.exercise_style, risk_free_rate
            )
        
        return price
    
    def calculate_greeks(self, contract: OptionContract, spot_price: float, volatility: float,
                         risk_free_rate: float = 0.05) -> OptionGreeks:
        """Calculate option Greeks (real Greek calculation)"""
        time_to_expiry = (contract.expiration_date - datetime.now()).total_seconds() / 365.25
        
        if contract.exercise_style == ExerciseStyle.EUROPEAN:
            greeks = self.bs_pricer.calculate_greeks(
                spot_price, contract.strike_price, time_to_expiry,
                volatility, contract.option_type, risk_free_rate
            )
        else:
    ...
    def open_position(self, contract: OptionContract, quantity: int, entry_price: float,
                     spot_price: float, volatility: float) -> OptionPosition:
    ...
    def close_position(self, position_id: str, exit_price: float) -> float:
        ...
        if position.quantity > 0:  # Long position
            realized_pnl = (exit_price - position.entry_price) * position.quantity * position.contract.contract_size
        else:  # Short position
            realized_pnl = (position.entry_price - exit_price) * abs(position.quantity) * position.contract.contract_size
    ...
    def get_expiring_positions(self, days: int = 7) -> List[OptionPosition]:
        ...
        expiring = [
            ...
            if pos.contract.expiration_date <= cutoff_date
        ]
...
if __name__ == '__main__':
    ...
    # Open position
    position = options_domain.open_position(contract, quantity=10, entry_price=price,
                                           spot_price=145.0, volatility=0.25)
