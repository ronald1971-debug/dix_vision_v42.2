...
class StocksDomainAdapter(DomainAdapter):
    ...
    def validate_order(self, order: UnifiedOrder) -> bool:
        ...
        if order.price and order.quantity * order.price < 1.0:
            logger.error("Order value too low", order_id=order.order_id, value=order.quantity * order.price)
            return False
        
        # Validate time in force (real time-in-force validation)
        valid_conditions = [OrderCondition.DAY.value, OrderCondition.GTC.value, OrderCondition.IOC.value, OrderCondition.FOK.value]
    ...
    def validate_instrument(self, instrument_id: str) -> bool:
        ...
        if instrument.instrument_type != InstrumentType.EQUITY:
            logger.error("Invalid instrument type for stocks domain",
        ...
        if instrument.tick_size <= 0:
            logger.error("Invalid tick size", instrument_id=instrument_id, tick_size=instrument.tick_size)
            return False
        
        return True
    
    def create_stock_instrument(self, symbol: str, name: str, exchange: str,
                              currency: str = "USD", tick_size: float = 0.01,
                              min_quantity: float = 1.0) -> Dict[str, Any]:
        ...
        logger.info("Stock instrument created",
                   instrument_id=instrument_id,
                   symbol=symbol,
                   exchange=exchange)
