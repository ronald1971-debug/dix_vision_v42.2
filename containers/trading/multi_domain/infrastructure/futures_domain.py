...
class FuturesDomainAdapter(DomainAdapter):
    ...
    def validate_order(self, order: UnifiedOrder) -> bool:
        ...
        if not isinstance(order.quantity, int) or order.quantity <= 0:
            logger.error("Invalid futures contract quantity", order_id=order.order_id, quantity=order.quantity)
            return False
        ...
        if order.quantity > max_contracts:
            logger.error("Order quantity exceeds maximum", order_id=order.order_id, quantity=order.quantity, max_contracts=max_contracts)
            return False
        ...
        if order.price:
            ...
            if instrument_id in self.instruments:
                ...
                if order.price % tick_size != 0:
                    logger.error("Price not valid for tick size", order_id=order.order_id, price=order.price, tick_size=tick_size)
                    return False
    ...
    def get_futures_summary(self) -> Dict[str, Any]:
        ...
        total_margin_requirements = sum(contract.margin_requirement for contract in self.futures_contracts.values())
