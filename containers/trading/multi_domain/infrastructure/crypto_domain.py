...
class CryptoDomainAdapter(DomainAdapter):
    ...
    def validate_order(self, order: UnifiedOrder) -> bool:
        ...
        if instrument_id in self.instruments:
            ...
            for crypto_inst in self.crypto_instruments.values():
                if crypto_inst.instrument_id in instrument_id or instrument_id in crypto_inst.instrument_id:
                    crypto_instrument = crypto_inst
                    break
            
            if crypto_instrument:
                if order.quantity < crypto_instrument.min_order_size:
                    logger.error("Order size below minimum", order_id=order.order_id, quantity=order.quantity, min_size=crypto_instrument.min_order_size)
                    return False
                
                if order.quantity > crypto_instrument.max_order_size:
                    logger.error("Order size above maximum", order_id=order.order_id, quantity=order.quantity, max_size=crypto_instrument.max_order_size)
                    return False
                ...
                for crypto_inst in self.crypto_instruments.values():
                    if crypto_inst.instrument_id in instrument_id:
                        ...
                        try:
                            ...
                            logger.error("Price precision invalid", order_id=order.order_id, price=order.price, decimals=crypto_inst.decimals)
                            return False
                        break
    ...
    def calculate_commission(self, order: UnifiedOrder) -> float:
        ...
        for crypto_inst in self.crypto_instruments.values():
            if crypto_inst.instrument_id in order.instrument_id or order.instrument_id in crypto_inst.instrument_id:
    ...
    def create_crypto_instrument(self, base_token: str, quote_token: str, exchange: str,
                               blockchain: BlockchainType, decimals: int = 18,
                               min_order_size: float = 0.0001, max_order_size: float = 1000.0) -> Dict[str, Any]:
        ...
        logger.info("Crypto instrument created",
                   instrument_id=instrument_id,
                   base_token=base_token,
                   quote_token=quote_token,
                   exchange=exchange,
