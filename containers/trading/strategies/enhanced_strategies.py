...
class MicrostructureStrategy:
    ...
    def calculate_spread_metrics(self, symbol: str, order_book: Dict[str, Any]) -> Dict[str, float]:
        ...
        return {
            ...
            "mid_price": mid_price,
            "avg_spread_pct": (
                statistics.mean(self.spread_history[symbol])
    ...
    def detect_large_trades(
        self, symbol: str, trade_size: float, avg_trade_size: float = 1000.0
    ) -> bool:
    ...
    def generate_signal(
        ...
        # Combine signals
        if (
            order_flow["imbalance"] > 0.2
            and spread_metrics["spread_pct"] < spread_metrics["avg_spread_pct"]
        ):
            action = "buy"
...
class PortfolioOptimizationStrategy:
    ...
    def calculate_covariance_matrix(self, returns_data: Dict[str, List[float]]) -> np.ndarray:
        ...
        returns_matrix = np.array(
            [[returns_data[asset][i] for i in range(min_length)] for asset in assets]
        )
    ...
    def calculate_mean_variance_optimal_weights(
    ...
    def optimize_portfolio(
        self, current_weights: Dict[str, float], returns_history: Dict[str, List[float]]
    ) -> Dict[str, float]:
    ...
    def calculate_portfolio_metrics(
        self, weights: Dict[str, float], returns_history: Dict[str, List[float]]
    ) -> Dict[str, float]:
        ...
        portfolio_volatility = (
            statistics.stdev(portfolio_returns) if len(portfolio_returns) > 1 else 0.0
        )
                ...
                if strategy_name == "microstructure":
                    if "order_book" in market_data:
                        signal = strategy.generate_signal(
                            symbol, market_data["order_book"], market_data.get("price", 0.0)
                        )
                    ...
                    if "price" in market_data:
                        ...
                        signal = strategy.generate_signal(
                            symbol, market_data["price"], market_data.get("iv")
                        )
                        signals.append(signal)

                elif strategy_name == "portfolio_optimization":
                    # Portfolio optimization doesn't generate trading signals directly
                    logger.debug("Portfolio optimization strategy: skipping signal generation (portfolio-level strategy)")
                    continue
...
if __name__ == "__main__":
    ...
    market_data = {"order_book": order_book, "price": 100.05}
