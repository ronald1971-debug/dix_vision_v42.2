...
class SocialTradingPlatform:
    ...
    def update_user_performance(self, user_id: str, performance_data: Dict[str, float]) -> UserProfile:
        ...
        user.avg_trade_duration = performance_data.get('avg_trade_duration', user.avg_trade_duration)
        ...
        logger.info("User performance updated", user_id=user_id, reputation_score=user.reputation_score)
    ...
    def create_post(self, user_id: str, content: str, post_type: str = "trade_idea",
                    symbols: List[str] = None, performance_data: Dict[str, Any] = None) -> SocialPost:
    ...
    def comment_on_post(self, post_id: str, user_id: str, comment: str) -> bool:
        ...
        logger.info("Post commented", post_id=post_id, user_id=user_id, total_comments=len(post.comments))
    ...
    def get_social_feed(self, user_id: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        feed_posts = [
            ...
            if post.user_id in followed_users or post.user_id == user_id
        ]
...
class CopyTradingManager:
    ...
    def create_copy_relation(self, follower_user_id: str, leader_user_id: str,
                           copy_ratio: float = 0.5, copy_mode: CopyTradingMode = CopyTradingMode.FIXED_RATIO,
                           max_amount: float = 10000.0, min_amount: float = 100.0,
                           risk_multiplier: float = 1.0, stop_loss_pct: float = 0.0,
                           take_profit_pct: float = 0.0) -> CopyTradeRelation:
        ...
        logger.info("Copy relation created", relation_id=relation_id, follower=follower_user_id, leader=leader_user_id)
        
        return relation
    
    def calculate_copy_trade_size(self, leader_trade_amount: float, 
                                  relation: CopyTradeRelation) -> float:
        ...
        if relation.copy_mode == CopyTradingMode.FIXED_RATIO:
            ...
            copy_amount = leader_trade_amount * relation.copy_ratio * (1.0 / relation.risk_multiplier)
        else:
            copy_amount = leader_trade_amount * relation.copy_ratio
    ...
    def get_copy_performance(self, relation_id: str) -> Dict[str, Any]:
        ...
        avg_copy_ratio = total_copied / sum(t['leader_amount'] for t in copy_trades) if copy_trades else 0.0
...
class LeaderboardManager:
    ...
    def update_leaderboard(self, leaderboard_type: LeaderboardType, 
                         users: Dict[str, UserProfile]) -> List[Tuple[str, float]]:
...
class CommunityAnalytics:
    ...
    def calculate_engagement_metrics(self, users: Dict[str, UserProfile], 
                                     posts: List[SocialPost]) -> Dict[str, float]:
        ...
        # Average reputation
        avg_reputation = sum(user.reputation_score for user in users.values()) / total_users if total_users > 0 else 0.0
        ...
        logger.info("Engagement metrics calculated", total_users=total_users, total_posts=total_posts)
...
class SocialTradingSystem:
    ...
    def update_user_performance(self, user_id: str, performance_data: Dict[str, float]) -> UserProfile:
        ...
        for leaderboard_type in LeaderboardType:
            self.leaderboard_manager.update_leaderboard(leaderboard_type, self.social_platform.users)
        
        return user
    
    def create_copy_relation(self, follower_user_id: str, leader_user_id: str,
                           copy_ratio: float = 0.5, copy_mode: CopyTradingMode = CopyTradingMode.FIXED_RATIO) -> CopyTradeRelation:
...
if __name__ == '__main__':
    ...
    # Register users
    user1 = social_system.register_user("user1", "trader_pro", "trader@example.com", "Professional trader")
    user2 = social_system.register_user("user2", "newbie", "newbie@example.com", "Learning to trade")
    ...
    post = social_system.social_platform.create_post(
        "user1", "Bullish on BTC/USDT for the next week. Technical analysis shows strong support at $65,000.",
