...
class ExecutionSystem:
    ...
    def route_order(self, order: Order) -> RoutingDecision:
        ...
        if order.status != OrderStatus.PENDING:
            logger.warning(
                "Order not in pending state", order_id=order.order_id, status=order.status.value
            )
    ...
    def _calculate_alternative_venues(
        self, order: Order, selected_venue: Venue
    ...
    def update_order_status(
        ...
        logger.info("Order status updated", order_id=order_id, new_status=new_status.value)
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        if order.status in [OrderStatus.FILLED, OrderStatus.CANCELLED, OrderStatus.EXPIRED]:
            logger.warning(
                "Order cannot be cancelled", order_id=order_id, status=order.status.value
            )
            return False
    ...
    def add_venue(
        ...
        logger.info(
            "Venue added", venue_id=venue_id, venue_name=venue_name, venue_type=venue_type.value
        )
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        avg_confidence = (
            np.mean([rd.confidence for rd in self.routing_decisions])
