...
class StateLedger:
    ...
    def add_state_entry(self, state_type: StateType, content: Dict[str, Any],
                       operator: str, signature: str = None) -> StateEntry:
    ...
    def verify_chain_integrity(self) -> bool:
        ...
        for i, hash_value in enumerate(self.state_chain):
            ...
            entry = next((e for e in self.state_entries.values() if e.calculate_hash() == hash_value), None)
            
            if not entry:
                logger.error("Hash chain verification failed - missing entry for hash",
            ...
            if entry.previous_hash != expected_previous:
                logger.error("Hash chain verification failed - hash link broken",
    ...
    def create_state_snapshot(self) -> StateSnapshot:
        ...
        logger.info("State snapshot created",
                   snapshot_id=snapshot_id,
                   total_entries=len(state_entries_copy))
    ...
    def replay_state(self, snapshot_id: str) -> Dict[str, Any]:
        ...
        logger.info("State replayed",
                   snapshot_id=snapshot_id,
                   replayed_entries=len(snapshot.state_entries))
    ...
    def revoke_entry(self, entry_id: str, operator: str) -> bool:
        ...
        if self.state_entries[entry_id].operator != operator:
            logger.warning("Operator not authorized to revoke entry",
                       entry_id=entry_id,
                       operator=operator,
    ...
    def cleanup_old_snapshots(self, retention_days: int = None) -> int:
        ...
        self.state_snapshots = deque(
            [snap for snap in self.state_snapshots if snap.timestamp >= cutoff_time],
            maxlen=100
        )
