...
try:
    from governance_unified.plugin_lifecycle import ActivationGate, ActivationVerdict, ManagedPlugin
    from governance_unified.plugin_lifecycle import PluginLifecycle as GovPluginLifecycle
...
class PluginLoader:
    ...
    def initialize(self) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"[PLUGIN_LOADER] Plugin loader initialized with {len(self._loaded_plugins)} plugins"
            )
    ...
    def _load_registry(self) -> None:
        ...
        logger.info(
            f"[PLUGIN_LOADER] Loaded {len(self._plugin_configs)} plugin configurations from registry"
        )
    ...
    def _load_all_plugins(self) -> None:
        ...
        for plugin_id in load_order:
            ...
            if verdict != ActivationVerdict.ALLOWED:
                logger.info(
                    f"[PLUGIN_LOADER] Plugin {plugin_id} activation verdict: {verdict}, skipping"
                )
                continue
    ...
    def _load_plugin(self, plugin_id: str, config: PluginConfig) -> None:
        ...
        try:
            ...
            logger.info(
                f"[PLUGIN_LOADER] Successfully loaded plugin: {plugin_id} (lifecycle={plugin_instance.lifecycle})"
            )
    ...
    def _import_plugin_class(self, slot: str, plugin_id: str) -> type:
        ...
        if not plugin_class:
            ...
            if plugin_id == "microstructure_advanced":
                try:
                    from intelligence_engine.plugins.microstructure.advanced import (
                        MicrostructureAdvanced,
                    )

                    return MicrostructureAdvanced
                except ImportError as e:
                    LOG.warning(f"Failed to import MicrostructureAdvanced plugin: {e}")
                    LOG.debug("Will continue searching for plugin class")
    ...
    def _instantiate_plugin(self, plugin_class: type, config: PluginConfig) -> MicrostructurePlugin:
        ...
        if instance.lifecycle != PluginLifecycle.ACTIVE:
            logger.warning(
                f"[PLUGIN_LOADER] Plugin {config.id} lifecycle is {instance.lifecycle}, setting to ACTIVE"
            )
    ...
    def wire_intelligence_engine(self, intelligence_engine: IntelligenceEngine) -> None:
        ...
        with self._lock:
            ...
            if hasattr(intelligence_engine, "microstructure_plugins"):
                ...
                logger.info(
                    f"[PLUGIN_LOADER] Wired {len(plugin_sequence)} plugins into intelligence engine"
                )
            else:
                logger.warning(
                    "[PLUGIN_LOADER] Intelligence engine does not have microstructure_plugins attribute"
                )
    ...
    def set_system_mode(self, mode: PluginSystemMode) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"[PLUGIN_LOADER] System mode changed from {old_mode} to {mode}, reloading plugins"
            )
    ...
    def load_plugin_with_world_context(
        self, plugin_id: str, world_context: Optional[WorldContext] = None
    ) -> bool:
        """
        ...
        if world_context and not self._should_load_plugin_in_world_context(
            plugin_id, world_context
        ):
    ...
    def _should_load_plugin_in_world_context(
        self, plugin_id: str, world_context: WorldContext
    ) -> bool:
