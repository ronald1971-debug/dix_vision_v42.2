...
class CognitiveConfigLoader:
    ...
    def _validate_config(self):
        ...
        if current_state not in valid_states:
            logger.warning(
                f"[CONFIG] Invalid learning gate state: {current_state}, using 'restricted'"
            )
        ...
        for component in ["indira_brain", "dyon_brain"]:
            component_config = (
                self._config.indira_brain
                if component == "indira_brain"
                else self._config.dyon_brain
            )
            max_cpu = (
                component_config.get("performance", {}).get("max_cpu_usage", 1.0)
