...
class DeadManSwitch:
    ...
    def heartbeat(self, *, source: str = "cockpit") -> None:
        with self._lock:
            ...
            if self._tripped:
                self._tripped = False
                get_writer().write("SYSTEM", "DEAD_MAN_REARMED", "GOVERNANCE", {"source": source})
            ...
            if not self._tripped and age > self._timeout:
                ...
                try:
                    get_writer().write(
                        ...
                        "DEAD_MAN_TRIPPED",
                        "GOVERNANCE",
                        {"age_sec": round(age, 1), "timeout_sec": self._timeout},
                    )
                except Exception as e:
                    LOG.warning(f"Failed to write dead man event to ledger: {e}")
                    LOG.debug("Ledger write failed, but proceeding with safety measures")
                ...
                try:
                    ...
                    LOG.warning(f"Failed to halt trading via risk cache: {e}")
                    LOG.debug("Risk cache halt failed, but system will continue with other safety measures")
            return DeadManStatus(
                last_beat_utc=self._last.isoformat(),
                age_sec=age,
                timeout_sec=self._timeout,
