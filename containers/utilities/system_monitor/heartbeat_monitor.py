...
class HeartbeatMonitor:
    ...
    def start(self) -> None:
        if self._thread and self._thread.is_alive():
            return
        self._thread = threading.Thread(target=self._loop, daemon=True, name="DIX-HeartbeatMonitor")
        self._thread.start()
    ...
    def _loop(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                LOG.debug("Heartbeat monitoring will continue despite check failure")
            time.sleep(1.0)
