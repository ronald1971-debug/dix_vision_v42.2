...
class DependencyMapping:
    ...
    def map_dependencies(self) -> DependencyMapping:
        ...
        internal_deps = [
            d for d in self.dependencies if d.dependency_type == DependencyType.INTERNAL
        ]
        external_deps = [
            d for d in self.dependencies if d.dependency_type == DependencyType.EXTERNAL
        ]
        standard_deps = [
            d for d in self.dependencies if d.dependency_type == DependencyType.STANDARD_LIBRARY
        ]
        third_party_deps = [
            d for d in self.dependencies if d.dependency_type == DependencyType.THIRD_PARTY
        ]
    ...
    def _should_skip_file(self, file_path: Path) -> bool:
        ...
        if not self.config.include_test_dependencies and (
                ...
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        ...
                        dependency = Dependency(
                            ...
                            metadata={"import_type": "import", "alias": alias.asname},
                        )
                    ...
                    dependency = Dependency(
                        ...
                        metadata={"import_type": "from_import", "level": node.level},
                    )
                    ...
                    for alias in node.names:
                        if alias.name != "*":  # Don't process star imports
                            ...
                            dependency = Dependency(
                                ...
                                dependency_type=dep_type,
                                confidence=0.8,
                                metadata={"import_type": "from_import_name", "alias": alias.asname},
                            )
                            self.dependencies.append(dependency)
    ...
    def _build_dependency_graph(self) -> None:
        ...
        for dependency in self.dependencies:
            ...
            self.dependency_graph.add_edge(
                dependency.source_module, dependency.target_module, weight=dependency.confidence
            )
    ...
    def _find_most_dependent_modules(self) -> List[Tuple[str, int]]:
        ...
        in_degrees = {
            node: self.dependency_graph.in_degree(node) for node in self.dependency_graph.nodes()
        }
