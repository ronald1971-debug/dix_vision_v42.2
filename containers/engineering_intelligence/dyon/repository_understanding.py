...
class RepositoryUnderstanding:
    ...
    def __init__(self, config: UnderstandingConfig = None):
        ...
        logger.info("RepositoryUnderstanding initialized",
    ...
    def _discover_code_files(self) -> List[str]:
        ...
        logger.info("Code files discovered",
    ...
    def _should_skip_file(self, file_path: Path) -> bool:
        ...
        if not self.config.include_test_files and ('test' in file_path_str or 'tests' in file_path_str):
            return True
    ...
    def _calculate_complexity_score(self, tree: ast.AST) -> float:
        ...
        complexity_indicators += len([node for node in ast.walk(tree) if isinstance(node, ast.While)])
    ...
    def _calculate_docstring_coverage(self, lines: List[str], function_count: int, class_count: int) -> float:
    ...
    def _calculate_repository_structure(self, code_files: List[CodeFile]) -> RepositoryStructure:
        ...
        javascript_files = len([f for f in code_files if f.file_type in [FileType.JAVASCRIPT, FileType.TYPESCRIPT]])
        config_files = len([f for f in code_files if f.file_type in [FileType.YAML, FileType.JSON, FileType.CONFIG]])
        ...
        structure = RepositoryStructure(
            ...
            metadata={
                'repository_root': str(self.repository_root),
                'average_complexity': np.mean([f.complexity_score for f in code_files]) if code_files else 0.0,
                'average_docstring_coverage': np.mean([f.docstring_coverage for f in code_files]) if code_files else 0.0
            }
        )
    ...
    def get_code_file_statistics(self) -> Dict[str, Any]:
        ...
        statistics = {
            ...
            'average_docstring_coverage': np.mean(docstring_coverages) if docstring_coverages else 0.0,
