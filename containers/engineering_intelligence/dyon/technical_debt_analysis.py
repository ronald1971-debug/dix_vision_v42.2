...
class TechnicalDebtAnalysis:
    ...
    def __init__(self, config: DebtConfig = None):
        ...
        logger.info(
            "TechnicalDebtAnalysis initialized",
    ...
    def _generate_improvement_recommendations(
        self, debt_by_type: Dict[str, int], debt_by_severity: Dict[str, int]
    ) -> List[str]:
        ...
        if debt_by_severity.get("critical", 0) > 0:
            recommendations.append(
                "Address critical debt items immediately to prevent production issues"
            )
    ...
    def get_debt_summary(self) -> Dict[str, Any]:
        ...
        high_priority_effort = sum(
            d.remediation_effort for d in self.debt_items if d.remediation_priority >= 0.7
        )
