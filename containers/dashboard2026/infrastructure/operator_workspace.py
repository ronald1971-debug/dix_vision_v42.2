...
class OperatorWorkspace:
    ...
    def __init__(self, config: WorkspaceConfig = None):
        ...
        self.active_workspaces: Dict[str, List[str]] = defaultdict(
            list
        )  # operator_id -> workspace_ids
    ...
    def create_workspace(
        ...
        if operator_workspace_count >= self.config.max_workspaces_per_operator:
            logger.warning(
                "Maximum workspaces reached for operator",
                operator=owner,
    ...
    def update_workspace_state(self, workspace_id: str, new_state: WorkspaceState) -> bool:
        ...
        logger.info("Workspace state updated", workspace_id=workspace_id, new_state=new_state.value)
    ...
    def start_operator_session(self, operator_id: str) -> OperatorSession:
        ...
        logger.info("Operator session started", session_id=session_id, operator_id=operator_id)
    ...
    def end_operator_session(self, session_id: str) -> bool:
        ...
        logger.info(
            ...
            session_id=session_id,
            duration=(
                self.operator_sessions[session_id].end_time
                - self.operator_sessions[session_id].start_time
            ).total_seconds(),
        )
    ...
    def log_operator_action(
        ...
        logger.info("Operator action logged", session_id=session_id, action_type=action_type)
