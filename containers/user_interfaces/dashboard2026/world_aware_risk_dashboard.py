        ...
        with self._lock:
            return {
                ...
                "world_context": {
                    ...
                    "liquidity_state": self._current_world_context.liquidity_state if self._current_world_context else "unknown",
                },
                "risk_metrics": {
                    name: {
                ...
                "portfolio_risk": {
                    "total_risk_score": self._portfolio_risk.total_risk_score if self._portfolio_risk else 0.0,
                    ...
                    "correlation_risk": self._portfolio_risk.correlation_risk if self._portfolio_risk else 0.0,
                    "concentration_risk": self._portfolio_risk.concentration_risk if self._portfolio_risk else 0.0,
                    "liquidity_risk": self._portfolio_risk.liquidity_risk if self._portfolio_risk else 0.0,
                    "stress_test_loss": self._portfolio_risk.stress_test_loss if self._portfolio_risk else 0.0,
                    "confidence_interval": self._portfolio_risk.confidence_interval if self._portfolio_risk else (0.0, 0.0),
                    "risk_trend": self._portfolio_risk.risk_trend if self._portfolio_risk else "stable",
                    "asset_class_breakdown": self._portfolio_risk.asset_class_breakdown if self._portfolio_risk else {},
                },
                "active_alerts": [
                    {
                ...
                "critical_alerts": len([a for a in self._active_alerts if a.severity == RiskSeverity.CRITICAL]),
                "high_alerts": len([a for a in self._active_alerts if a.severity == RiskSeverity.HIGH]),
            }
    ...
    def get_alert_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "alert_by_type": {
                    alert_type.value: len([a for a in self._alert_history if a.alert_type == alert_type])
