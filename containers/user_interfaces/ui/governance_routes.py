...
_HAZ_TAXONOMY: tuple[dict[str, str], ...] = (
    ...
    {"code": "HAZ-05", "label": "MEMORY_OVERFLOW", "description": "RSS / heap budget breached"},
    {
        "code": "HAZ-06",
        "label": "LATENCY_SPIKE",
        "description": "round-trip exceeds latency budget",
    },
    {"code": "HAZ-07", "label": "HEARTBEAT_MISSED", "description": "engine heartbeat absent"},
    {
        "code": "HAZ-08",
        "label": "RISK_SNAPSHOT_STALE",
        "description": "fast risk cache version unchanged too long",
    },
    {"code": "HAZ-09", "label": "ORDER_FLOOD", "description": "order rate breaches per-window cap"},
    {
        "code": "HAZ-10",
...
def _hazards_payload(state: Any) -> dict[str, Any]:
    ...
    BUGFIX P1-1: Changed from _safe_attr(state, "hazard_sensor_array")
    to _safe_attr(state, "sensor_array"). The _State class stores the
    sensor array as self.sensor_array, not self.hazard_sensor_array.
    """

    array = _safe_attr(state, "sensor_array")
