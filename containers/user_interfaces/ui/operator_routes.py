...
class _OperatorStateLike(Protocol):
    ...
    @property
    def operator_authority(self) -> Any: ...
    @property
    def semi_auto_approval_queue(self) -> Any: ...
...
def build_operator_router(
    ...
    # -----------------------------------------------------------------
    # /api/operator/authority  (BUILD-DIRECTIVE §new-routes)
    # -----------------------------------------------------------------
    from core.contracts.api.operator import (  # noqa: PLC0415
    ...
    from core.contracts.operator_authority import (  # noqa: PLC0415
        LearningAuthority,
    ...
    def _project_operator_authority(auth: OperatorAuthority) -> OperatorAuthorityResponse:
        tm = {d.value: m.value for d, m in auth.trading_mode.items()}
        sap = {}
        for d, p in auth.semi_auto_policy.items():
            sap[d.value] = {
                "entry_requires_approval": p.entry_requires_approval,
                "exit_auto": p.exit_auto,
                "risk_reduce_auto": p.risk_reduce_auto,
                "notional_threshold_usd": p.notional_threshold_usd,
                "position_fraction_cap": p.position_fraction_cap,
                "volatility_cap_zscore": p.volatility_cap_zscore,
            }
        return OperatorAuthorityResponse(
            learning=auth.learning.value,
            practice=auth.practice.value,
            live_execution=auth.live_execution.value,
            operator_id=auth.operator_id,
            granted_ts_ns=auth.granted_ts_ns,
            notes=auth.notes,
            trading_mode=tm,
            semi_auto_policy=sap,
        )

    @router.get(
        "/api/operator/authority",
        response_model=OperatorAuthorityResponse,
    )
    def operator_authority_get() -> OperatorAuthorityResponse:
        state = state_accessor()
        with state.lock:
    ...
    def operator_authority_post(
        body: OperatorAuthorityRequest,
    ) -> OperatorAuthorityResponse:
        state = state_accessor()
        try:
            learning = LearningAuthority(body.learning)
            practice = PracticeAuthority(body.practice)
            live_exec = LiveExecutionAuthority(body.live_execution)
        except ValueError as exc:
        ...
        new_auth = OperatorAuthority(
            ...
            semi_auto_policy=existing.semi_auto_policy,
            operator_id=body.operator_id,
            granted_ts_ns=wall_ns(),
            notes=body.notes,
        )
        ts_ns = wall_ns()
        with state.lock:
            state.governance.ledger.append(
                ts_ns=ts_ns,
                kind="OPERATOR_AUTHORITY_CHANGED",
                payload={
                    "requestor": body.operator_id,
                    "authority": str(new_auth),
                },
            )
            state.governance.operator.apply_operator_authority_to_state(
                state_obj=state,
                authority=new_auth,
            )
        return _project_operator_authority(new_auth)

    @router.post(
        "/api/operator/trading_mode",
        response_model=OperatorActionResponse,
    )
    def operator_trading_mode_post(
        body: TradingModeRequest,
    ) -> OperatorActionResponse:
        state = state_accessor()
        try:
            domain = TradingDomain(body.domain)
            mode = TradingMode(body.mode)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        ts_ns = wall_ns()
        with state.lock:
            outcome = state.governance.operator.set_trading_mode(
                ts_ns=ts_ns,
                requestor=body.requestor,
                domain=domain.value,
                mode=mode.value,
        ...
        decision_dict = _decision_to_dict(outcome)
        return OperatorActionResponse(
            approved=outcome.approved,
            summary=outcome.summary,
            decision=decision_dict,
    ...
    @router.post(
        "/api/operator/semi_auto_policy",
        response_model=OperatorActionResponse,
    )
    def operator_semi_auto_policy_post(
        body: SemiAutoPolicyRequest,
    ) -> OperatorActionResponse:
        from core.contracts.operator_authority import SemiAutoPolicy  # noqa: PLC0415
        ...
        policy = SemiAutoPolicy(
            entry_requires_approval=body.entry_requires_approval,
            exit_auto=body.exit_auto,
            risk_reduce_auto=body.risk_reduce_auto,
            notional_threshold_usd=body.notional_threshold_usd,
            position_fraction_cap=body.position_fraction_cap,
            volatility_cap_zscore=body.volatility_cap_zscore,
        )
        ts_ns = wall_ns()
        with state.lock:
            outcome = state.governance.operator.set_semi_auto_policy(
                ts_ns=ts_ns,
                requestor=body.requestor,
                domain=domain.value,
                policy=policy,
    ...
    @router.post(
        "/api/operator/promote_buildout_params",
        response_model=OperatorActionResponse,
    )
    def operator_promote_buildout_params(
        body: PromoteBuildoutParamsRequest,
    ) -> OperatorActionResponse:
        state = state_accessor()
        ts_ns = wall_ns()
        with state.lock:
        ...
        return OperatorActionResponse(
            approved=True,
            summary=f"promoted {body.parameter_path} to live set",
            decision={"ledger_seq": entry.seq},
        )

    @router.post(
        "/api/operator/external_source",
        response_model=OperatorActionResponse,
    )
    def operator_external_source(
        ...
        return OperatorActionResponse(
            approved=True,
            summary=f"external source {body.platform}: {body.action}",
            decision={"ledger_seq": entry.seq},
        )

    @router.post(
        "/api/operator/semi_auto_approve",
        response_model=OperatorActionResponse,
    )
    def operator_semi_auto_approve(
        body: SemiAutoApprovalRequest,
    ) -> OperatorActionResponse:
        state = state_accessor()
        ts_ns = wall_ns()
        with state.lock:
            queue = state.semi_auto_approval_queue
            item = queue.approve(body.request_id)
            if item is not None:
                kind = "SEMI_AUTO_APPROVED"
                summary = f"approved {body.request_id}"
            else:
                kind = "SEMI_AUTO_REJECTED"
                summary = f"not found: {body.request_id}"
            state.governance.ledger.append(
                ts_ns=ts_ns,
                kind=kind,
                payload={
        ...
        return OperatorActionResponse(
            approved=item is not None,
            summary=summary,
            decision={"request_id": body.request_id},
        )

    @router.post(
        "/api/operator/semi_auto_reject",
        response_model=OperatorActionResponse,
    )
    def operator_semi_auto_reject(
        body: SemiAutoRejectRequest,
    ) -> OperatorActionResponse:
        state = state_accessor()
        ts_ns = wall_ns()
        with state.lock:
    ...
    @router.post(
        "/api/operator/manual_order",
        response_model=OperatorActionResponse,
    )
    def operator_manual_order(
        ...
        return OperatorActionResponse(
            approved=True,
            summary=f"manual order {body.side} {body.qty} {body.symbol}",
            decision={"ledger_seq": entry.seq},
        )

    @router.get(
        "/api/operator/learning_progress",
        response_model=LearningProgressResponse,
    )
    def operator_learning_progress() -> LearningProgressResponse:
        state = state_accessor()
        with state.lock:
            return LearningProgressResponse(
                class_a_updates_applied=getattr(state, "_class_a_count", 0),
                paper_fills_today=getattr(state, "_paper_fills_today", 0),
                backtest_runs_today=getattr(state, "_backtest_runs_today", 0),
                live_fills_today=getattr(state, "_live_fills_today", 0),
            )

    @router.get(
        "/api/operator/semi_auto_queue",
        response_model=SemiAutoQueueResponse,
    )
    def operator_semi_auto_queue() -> SemiAutoQueueResponse:
        state = state_accessor()
        with state.lock:
            queue = state.semi_auto_approval_queue
            pending = [
