...
class FeedStatus(enum.Enum):
    ...
    PLACEHOLDER = "PLACEHOLDER"  # Using placeholder data
    TRANSITIONING = "TRANSITIONING"  # Moving from placeholder to real
...
class DataSource(enum.Enum):
    ...
    PLACEHOLDER_GENERATOR = "PLACEHOLDER_GENERATOR"
    WEBSOCKET = "WEBSOCKET"
    REST_API = "REST_API"
    DATABASE = "DATABASE"
    FILE = "FILE"
...
class ReplacementMetrics:
    ...
    total_feeds: int
    placeholder_feeds: int
    live_feeds: int
    transitioning_feeds: int
    failed_feeds: int
    total_data_points: int
    placeholder_data_points: int
...
class MockFeedReplacer:
    ...
    def register_feed(
        ...
        with self._lock:
            ...
            if feed_definition.current_status == FeedStatus.PLACEHOLDER:
    ...
    def transition_to_real(
        ...
        with self._lock:
            ...
            if feed.current_status != FeedStatus.PLACEHOLDER:
                return False
    ...
    def fallback_to_placeholder(
        ...
        """Fallback a feed to placeholder data.
        ...
        with self._lock:
            ...
            updated_feed = dataclasses.replace(
                feed,
                current_status=FeedStatus.PLACEHOLDER,
                current_source=DataSource.PLACEHOLDER_GENERATOR,
            )
            self._feeds[feed_id] = updated_feed
    ...
    def get_data_point(
        ...
        with self._lock:
            ...
            if feed.current_source != DataSource.PLACEHOLDER_GENERATOR:
                ...
                if source:
                    try:
                        ...
                        if (
                            ...
                            self.fallback_to_placeholder(feed_id, str(e))

            # Fallback to placeholder data
            placeholder_data_point = self._generate_placeholder_data_point(feed)

            # Add to cache
            cache = self._data_cache.get(feed_id, [])
            cache.append(placeholder_data_point)
            if len(cache) > self._config.data_cache_size:
                cache.pop(0)

            # Update metrics
            self._metrics.total_data_points += 1
            self._metrics.placeholder_data_points += 1

            return placeholder_data_point
    ...
    def _generate_placeholder_data_point(self, feed: FeedDefinition) -> DataPoint:
        """Generate placeholder data for a feed.
        ...
        # Generate placeholder data based on feed type
        placeholder_data = {}
        ...
        return DataPoint(
            ...
            quality_score=1.0,  # Placeholder data always has perfect quality
            source=DataSource.PLACEHOLDER_GENERATOR,
            is_placeholder=True,
        )
