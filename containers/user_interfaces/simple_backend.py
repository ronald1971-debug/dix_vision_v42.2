...
async def get_dashboard_mode():
    """Get dashboard mode"""
    return ModeSnapshot(
        current_mode="MANUAL", legal_targets=["MANUAL", "SEMI_AUTO", "FULL_AUTO"], is_locked=False
    )
        ...
        engines=[
            {
                ...
                "bucket": "alive",
                "detail": "Development mode active",
                "plugin_count": 0,
            },
            {
...
async def get_trading_allowed():
    ...
    return TradingAllowedResponse(trading_allowed=False, development_enabled=True, mode="MANUAL")


@app.post("/api/operator/trading-allowed")
async def set_trading_allowed(enabled: bool):
    """Set trading allowed status"""
    return TradingAllowedResponse(trading_allowed=enabled, development_enabled=True, mode="MANUAL")
    ...
    return {
        "components": [
            ...
            {"name": "backend", "status": "ok", "detail": "Simplified API running"},
            {
                "name": "database",
                "status": "degraded",
                "detail": "Not configured in simplified mode",
            },
        ],
