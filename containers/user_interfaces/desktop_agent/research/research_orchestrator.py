...
class ResearchOrchestrator:
        ...
        try:
            ...
            for workflow_id, task in self._active_workflows.items():
                ...
                try:
                    ...
                    self.logger.debug(f"Workflow {workflow_id} cancelled as expected during shutdown")
            ...
            if action == "execute_query" and self._research_engine:
                ...
                if query_id and query_text:
                    ...
                    result = await self._research_engine.execute_query(
                        query_id, query_text, ResearchType.QUERY
                    )
                    if result:
                ...
                if node_id and node_type and label:
                    ...
                    result = await self._knowledge_graph.add_node(
                        node_id, NodeType(node_type), label, workflow.get("properties", {})
                    )
                    if result:
                ...
                if title and authors:
                    ...
                    result = await self._citation_manager.create_citation(
                        SourceType(source_type), title, authors, **workflow.get("properties", {})
                    )
                    if result:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "component_statuses": {
                "research_engine": (
                    self._research_engine.get_status() if self._research_engine else None
                ),
                "knowledge_graph": (
                    self._knowledge_graph.get_status() if self._knowledge_graph else None
                ),
                "citation_manager": (
                    self._citation_manager.get_status() if self._citation_manager else None
                ),
            },
        }
