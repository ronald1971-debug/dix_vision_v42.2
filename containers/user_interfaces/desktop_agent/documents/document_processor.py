    ...
    async def _extract_text(self, document: Document) -> None:
        ...
        try:
            ...
            if document.document_type == DocumentType.TXT:
                ...
                await asyncio.sleep(1.0)
                document.extracted_text = f"Text extracted from {document.document_type.value} document"
            
            self.logger.info(f"Text extraction complete: {document.document_id} (length: {len(document.extracted_text or '')} chars)")
