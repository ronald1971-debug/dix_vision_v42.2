...
class HealthConfig:
    ...
    health_score_weight: Dict[str, float] = field(
...
class SystemHealthMonitoring:
    ...
    def perform_health_check(self, component_id: str) -> ComponentHealthCheck:
        ...
        component_check = next(
            (c for c in self.component_health_checks if c.component_id == component_id), None
        )
        ...
        for component_check in self.component_health_checks:
            try:
                ...
                logger.error(
                    ...
                    component_id=component_check.component_id,
                    error=str(e),
                )
                component_check.health_status = ComponentHealth.ERROR
                component_check.error_message = str(e)
    ...
    def calculate_health_score(
        self, health_checks: List[ComponentHealthCheck], system_metrics: Dict[str, float]
    ) -> float:
        """Calculate overall health score (real health score calculation)"""
        # Component health score (real component score calculation)
        operational_count = sum(
            1 for check in health_checks if check.health_status == ComponentHealth.OPERATIONAL
        )
        ...
        avg_response_time = (
            np.mean([check.response_time_ms for check in health_checks]) if health_checks else 0.0
        )
        ...
        error_count = sum(
            ...
            if check.health_status in [ComponentHealth.ERROR, ComponentHealth.TIMEOUT]
        )
        error_score = (
            max(0.0, 1.0 - (error_count / total_components)) if total_components > 0 else 0.5
        )
    ...
    def generate_health_report(self) -> SystemHealthReport:
        ...
        if system_metrics["cpu_percent"] > self.config.cpu_threshold_percent:
            warnings.append(
                f"CPU usage {system_metrics['cpu_percent']:.1f}% exceeds threshold {self.config.cpu_threshold_percent:.1f}%"
            )

        if system_metrics["memory_percent"] > self.config.memory_threshold_percent:
            warnings.append(
                f"Memory usage {system_metrics['memory_percent']:.1f}% exceeds threshold {self.config.memory_threshold_percent:.1f}%"
            )

        if system_metrics["disk_percent"] > self.config.disk_threshold_percent:
            warnings.append(
                f"Disk usage {system_metrics['disk_percent']:.1f}% exceeds threshold {self.config.disk_threshold_percent:.1f}%"
            )
        ...
        for check in critical_components:
            critical_issues.append(
                f"Component {check.component_name} ({check.component_id}) is {check.health_status.value}: {check.error_message or 'No error message'}"
            )
