...
class TestEnhancedCausalModel(unittest.TestCase):
    ...
    def test_causal_effect_inference(self):
        ...
        self.assertLess(abs(inference_result.causal_effect - 0.5), 0.3, "Causal effect estimate should be close to true effect")
        
        # Should have confidence interval
        self.assertEqual(len(inference_result.confidence_interval), 2)
        self.assertLess(inference_result.confidence_interval[0], inference_result.confidence_interval[1])
    ...
    def test_intervention_analysis(self):
        ...
        self.causal_model.add_causal_relationship("interest_rate", "bond_price", strength=-0.8, confidence=0.9)
        self.causal_model.add_causal_relationship("bond_price", "stock_volatility", strength=0.6, confidence=0.8)
