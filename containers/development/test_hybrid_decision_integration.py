...
def test_hybrid_decision_engine():
    ...
    try:
        ...
        engine = HybridDecisionEngine(default_strategy=ConflictResolutionStrategy.COGNITIVE_PRIMACY)
        ...
        for inp in decision_inputs:
            logger.info(
                f"  {inp.source.value}: {inp.decision_type.value} (confidence: {inp.confidence:.2f})"
            )
        ...
        logger.info(
            f"  Contributing Sources: {[s.value for s in hybrid_decision.contributing_sources]}"
        )
...
def test_indira_integration():
    ...
    try:
        ...
        request = INDARADecisionRequest(
            ...
            indicator_signals={"RSI": 0.7, "MACD": 0.8, "Bollinger_Bands": 0.6},
            ...
            governance_constraints=[
                {"id": "constraint_001", "reasoning": "Position limit check", "risk_level": 0.2}
            ],
        )
        ...
        logger.info(
            f"  Learning suggestion: {request.learning_engine_suggestion.get('action') if request.learning_engine_suggestion else None}"
        )
        logger.info(
            f"  Governance constraints: {len(request.governance_constraints) if request.governance_constraints else 0}"
        )
...
def test_governance_integration():
    ...
    try:
        ...
        logger.info(
            f"  Compliance requirements: {len(request.compliance_requirements)} requirements"
        )
...
def test_end_to_end_integration():
    ...
    try:
        ...
        logger.info(
            f"INDIRA result: {indira_result.get('decision_type')} (confidence: {indira_result.get('confidence', 0):.2f})"
        )
        ...
        logger.info(
            f"Governance result: approved={gov_result.get('approved')} (confidence: {gov_result.get('confidence', 0):.2f})"
        )
        ...
        if gov_result.get("approved"):
            ...
            logger.info(
                f"Execution result: {exec_result.get('execution_intent')} (confidence: {exec_result.get('confidence', 0):.2f})"
            )
