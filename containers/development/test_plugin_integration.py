...
def test_plugin_registry_loading():
    ...
    try:
        ...
        for plugin_id, config in configs.items():
            logger.info(f"   - {plugin_id}: {config.category} (lifecycle=ACTIVE, enabled={config.enabled})")
...
def test_plugin_contract_compliance():
    ...
    try:
        ...
        for plugin_id, plugin in loaded_plugins.items():
            ...
            assert isinstance(plugin, MicrostructurePlugin), f"Plugin {plugin_id} must inherit from MicrostructurePlugin"
            ...
            assert hasattr(plugin, 'lifecycle'), f"Plugin {plugin_id} must have 'lifecycle' attribute"
            
            # Verify required methods
            assert hasattr(plugin, 'on_tick'), f"Plugin {plugin_id} must have 'on_tick' method"
            assert hasattr(plugin, 'check_self'), f"Plugin {plugin_id} must have 'check_self' method"
            ...
            assert plugin.lifecycle == PluginLifecycle.ACTIVE, f"Plugin {plugin_id} must have ACTIVE lifecycle"
...
def test_governance_integration():
    ...
    try:
        ...
        for plugin_id in loaded_plugins:
            ...
            assert verdict == ActivationVerdict.ALLOWED, f"Plugin {plugin_id} should be ALLOWED in MANUAL mode"
...
def test_intelligence_engine_wiring():
    ...
    try:
        ...
        logger.info(f"✅ {len(intelligence_engine.microstructure_plugins)} plugins wired into intelligence engine")
        return True
    except Exception as e:
...
def test_system_integration():
    ...
    try:
        ...
        assert integration_manager._intelligence_engine, "Integration manager should have intelligence engine"
        
        # Verify integration point is registered
        integration_id = "plugin_system→intelligence_engine"
        assert integration_id in integration_manager._integration_points, "Plugin integration should be registered"
