...
class VolatilityTransmissionAnalyzer:
    ...
    def update_market_data(self, asset: str, price: float, timestamp_ns: int) -> None:
        ...
        if len(self._price_history[asset]) >= 10:
            ...
            volatility = math.sqrt(sum(r**2 for r in returns[-10:]) / 10) * math.sqrt(
                252
            )  # Annualized
    ...
    def analyze_transmission_network(self, timestamp_ns: int) -> VolatilityNetwork:
        ...
        for i, source in enumerate(assets):
            for target in assets[i + 1 :]:
                ...
                if spillover and spillover.transmission_strength >= self._min_transmission_strength:
                    ...
                    if (
                        reverse_spillover
                        and reverse_spillover.transmission_strength
                        >= self._min_transmission_strength
                    ):
                        if (
                            spillover.transmission_strength
                            > reverse_spillover.transmission_strength
                        ):
                            spillovers.append(
                                VolatilitySpillover(
                                    source_asset=target,
        ...
        network_density = (
            len(spillovers) / possible_connections if possible_connections > 0 else 0.0
        )
        transmission_intensity = (
            sum(s.transmission_strength for s in spillovers) / len(spillovers)
            if spillovers
            else 0.0
        )
    ...
    def _calculate_spillover(
        ...
        if source not in self._volatility_history or target not in self._volatility_history:
            return None
    ...
    def _detect_transmission_events(self, network: VolatilityNetwork, timestamp_ns: int) -> None:
    ...
    def get_asset_transmission_profile(self, asset: str) -> dict[str, Any]:
        ...
        outgoing_spillovers = [s for s in latest_network.spillovers if s.source_asset == asset]
        incoming_spillovers = [s for s in latest_network.spillovers if s.target_asset == asset]
