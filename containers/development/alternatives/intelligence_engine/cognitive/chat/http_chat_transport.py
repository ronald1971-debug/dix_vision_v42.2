...
adapter (:class:`RegistryDrivenChatModel`) plus a placeholder
...
__all__ = [
    ...
    "LocalVLLMChatTransport",
    "LocalLlamaChatTransport",
    "LocalTensorRTChatTransport",
...
# --------------------------------------------------------------------
# Local inference transport wrappers
# (vLLM, llama-cpp-python, TensorRT-LLM)
# These wrap the respective transports and expose the ChatTransport
# protocol so the registry dispatcher can route to them.
# --------------------------------------------------------------------


class LocalVLLMChatTransport:
    """ChatTransport backed by a local vLLM OpenAI-compatible server.

    Reads the base URL from the ``VLLM_BASE_URL`` env var
    (default: ``http://localhost:8000``). If vLLM is not reachable,
    raises :class:`TransientProviderError` so the dispatcher falls
    ...
    def invoke(
        ...
        base_url = env.get(self.ENV_VAR, "").strip() or self.DEFAULT_URL
        ...
        if resp.error:
            raise TransientProviderError(f"local_vllm: {resp.error}")
        return resp.content


class LocalLlamaChatTransport:
    """ChatTransport backed by llama-cpp-python in-process GGUF inference.

    Reads the model path from the ``LLAMA_MODEL_PATH`` env var.
    If the env var is not set or the package is not installed, raises
    ...
    def invoke(
        ...
        env = self._env if self._env is not None else os.environ
        model_path = env.get(self.ENV_VAR, "").strip()
        if not model_path:
            raise TransientProviderError(
                f"local_llama: {self.ENV_VAR} is not set — no model path configured"
            )
        ...
        if resp.error:
            raise TransientProviderError(f"local_llama: {resp.error}")
        return resp.content


class LocalTensorRTChatTransport:
    """ChatTransport backed by a TensorRT-LLM OpenAI-compatible server.

    Reads the base URL from the ``TENSORRT_BASE_URL`` env var
    (default: ``http://localhost:8001``). Raises
    :class:`TransientProviderError` when the server is unreachable.
    """
    ...
    def invoke(
        ...
        if resp.error:
            raise TransientProviderError(f"local_tensorrt: {resp.error}")
        return resp.content
...
def build_default_dispatch_transport(
    *,
    ...
    Wires:
    * OpenAI-compat backend for ``openai``, ``xai``, ``deepseek``
    * Gemini backend for ``google``
    * Devin session backend for ``cognition``
    * vLLM HTTP transport for ``local_vllm``
    * llama-cpp-python in-process transport for ``local_llama``
    * TensorRT-LLM HTTP transport for ``local_tensorrt``

    Every backend reads its key / URL from ``env`` (or
    :func:`os.environ` if ``env`` is ``None``) on each turn, so an
    operator who configures a local server after boot gets immediate
    ...
    local_vllm = LocalVLLMChatTransport(timeout=timeout, env=env)
    local_llama = LocalLlamaChatTransport(timeout=timeout, env=env)
    local_tensorrt = LocalTensorRTChatTransport(timeout=timeout, env=env)
