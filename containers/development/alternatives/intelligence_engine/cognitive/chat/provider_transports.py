...
class ProviderTransportSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if self.family != "local" and not self.base_url.startswith("https://"):
            raise ProviderTransportError(
                f"ProviderTransportSpec.base_url must be https://"
                f" (or family='local' for localhost transports),"
...
_ALLOWED_FAMILIES: tuple[str, ...] = (
    ...
    "google",
    "cognition",
    # Local inference servers (vLLM, llama-cpp-python, TensorRT-LLM).
    # These use http://localhost:* and need no API key — env_var is the
    # base-URL override so operators can redirect to a non-default port.
    "local",
)
...
TRANSPORT_SPECS: tuple[ProviderTransportSpec, ...] = (
    ...
    # ------------------------------------------------------------------ local
    # Local inference transports. ``env_var`` holds the base-URL override
    # (e.g. VLLM_BASE_URL=http://gpu-box:8000). Defaults are localhost.
    # ``auth_scheme`` is ``"bearer"`` so the spec validates; the actual
    # transports treat missing keys as "use default URL, no auth".
    ProviderTransportSpec(
        provider="local_vllm",
        family="local",
