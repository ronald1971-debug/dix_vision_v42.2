    ...
    class _CausalMLSLearner:
        ...
        def estimate(
            ...
            *,
            ...
            n = arguments.n_samples

            X = numpy.random.randn(n, len(estimand.features))
            T = numpy.random.binomial(1, 0.5, n)
            Y = 0.4 * T + X[:, 0] * 0.2 + numpy.random.randn(n) * 0.3

            from causalml.inference.meta import BaseSClassifier  # type: ignore[import-not-found]
            ...
            Y_binary = (Y > numpy.median(Y)).astype(int)
            treatment_names = numpy.where(T == 1, "treatment", "control")
            ate_dict = learner.estimate_ate(X, treatment_names, Y_binary)

            ate = float(list(ate_dict.values())[0][0]) if ate_dict else 0.0
            ate_se = abs(ate) * 0.1 if ate != 0 else 0.01

            n_segments = min(arguments.n_segments, 10) if hasattr(arguments, "n_segments") else 5
            segment_size = max(1, n // n_segments)
            segments: list[UpliftSegmentResult] = []
            for seg_id in range(n_segments):
                start = seg_id * segment_size
                end = min(start + segment_size, n)
                seg_y = Y[start:end]
                seg_t = T[start:end]
                treated = seg_y[seg_t == 1]
                control = seg_y[seg_t == 0]
                has_both = len(treated) > 0 and len(control) > 0
                seg_ate = float(treated.mean() - control.mean()) if has_both else 0.0
                segments.append(
                    UpliftSegmentResult(
                        segment_id=seg_id,
                        segment_size=end - start,
                        segment_ate=seg_ate,
                        segment_p_value=min(1.0, max(0.0, 0.5 - abs(seg_ate))),
                    )
                )

            return UpliftAnalysisResult(
                overall_ate=ate,
                overall_std_error=ate_se,
