...
class ProductionPlanner:
    ...
    def create_plan(
        ...
        try:
            ...
            logger.info(
                f"[PLANNER] Plan created: {planning_id} with success probability {success_probability:.2f}"
            )
            return result
    ...
    def _decompose_goals_to_tasks(
        self, goals: List[PlanningGoal], constraints: List[PlanningConstraint]
    ) -> List[PlanningTask]:
    ...
    def _optimize_task_order(
        self, tasks: List[PlanningTask], constraints: List[PlanningConstraint]
    ) -> List[PlanningTask]:
    ...
    def _allocate_resources(self, tasks: List[PlanningTask]) -> Dict[str, Resource]:
        ...
        for task in tasks:
            for resource_type, amount in task.resource_requirements.items():
                ...
                matching_resources = [
                    r for r in self._resource_pool.values() if r.resource_type == resource_type
                ]
                ...
                if matching_resources:
                    ...
                    if resource.available >= amount:
                        ...
                        logger.warning(
                            f"[PLANNER] Insufficient {resource_type} for task {task.task_id}"
                        )

        return allocated

    def _calculate_execution_timeline(
        self, tasks: List[PlanningTask], horizon: PlanningHorizon
    ) -> Dict[str, Any]:
        ...
        timeline = {
            ...
            "completion_time": (
                now().utc_datetime() + now().timedelta(seconds=completion_offset)
            ).isoformat(),
    ...
    def _generate_recommendations(
        self, plan: Plan, constraints: List[PlanningConstraint]
    ) -> List[str]:
        ...
        if plan.success_probability > 0.8:
            ...
            recommendations.append(
                "Plan has moderate success probability - proceed with monitoring"
            )
        else:
        ...
        if len(plan.tasks) > self._max_plan_complexity * 0.8:
            recommendations.append(
                "Plan complexity is high - consider breaking into smaller sub-plans"
            )

        if constraints:
            recommendations.append(
                f"Review {len(constraints)} constraints for potential relaxation"
            )

        return recommendations

    def _generate_warnings(self, plan: Plan, constraints: List[PlanningConstraint]) -> List[str]:
    ...
    def _create_complexity_error(
        self, planning_id: str, plan_type: PlanType, task_count: int
    ) -> PlanningResult:
        ...
        return PlanningResult(
            ...
            recommendations=[
                f"Reduce complexity: {task_count} tasks exceeds maximum of {self._max_plan_complexity}"
            ],
    ...
    def _create_error_result(
        self, planning_id: str, plan_type: PlanType, error: str
    ) -> PlanningResult:
    ...
    def update_plan_status(self, plan_id: str, task_id: str, new_status: TaskStatus) -> bool:
