    ...
    class _ArviZDiagnosticEngine:
        ...
        def diagnose(
            ...
            *,
            ...
            # Seed numpy for INV-15 determinism.
            numpy.random.seed(arguments.random_seed)

            # Reshape flat (num_chains * num_draws * num_vars,) sample
            # tensor to (num_chains, num_draws, num_vars).
            n_chains = spec.num_chains
            n_draws = spec.num_draws
            n_vars = spec.num_vars
            expected = n_chains * n_draws * n_vars
            flat = numpy.asarray(arguments.samples, dtype=numpy.float64)
            ...
            default_ess = float(max(1, n_chains * n_draws // 2))
            summaries: list[ArviZVariableSummary] = []

            for i in range(n_vars):
                var_name = f"var_{i}"

                # Initialise to safe defaults; overwrite from summary_df.
                mean_v: float = 0.0
                sd_v: float = 0.0
                hdi_lo: float = 0.0
                hdi_hi: float = 0.0
                ess_bulk: float = default_ess
                ess_tail: float = default_ess
                r_hat: float = 1.0

                if var_name in summary_df.index:
                    row = summary_df.loc[var_name]

                    def _safe(key: str, default: float) -> float:  # noqa: E306
                        try:
                            v = float(row[key])
                    ...
                    ess_tail = max(0.0, _safe("ess_tail", default_ess))
                    r_hat = max(0.0, _safe("r_hat", 1.0))

                vs = ArviZVariableSummary(
                    name=var_name,
                    mean=mean_v,
                    sd=sd_v,
                    hdi_3=hdi_lo,
                ...
                callback.on_variable_summary(ts_ns=ts_ns, summary=vs)
                summaries.append(vs)
