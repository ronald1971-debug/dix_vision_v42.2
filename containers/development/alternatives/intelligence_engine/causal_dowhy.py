    ...
    class _DoWhyLinearRegressionEstimator:
        ...
        def estimate(
            ...
            *,
            ...
            treatment_data = numpy.random.randn(n)
            noise = numpy.random.randn(n) * 0.5
            common = numpy.random.randn(n, max(1, len(estimand.common_causes)))
            outcome_data = 0.5 * treatment_data + common.sum(axis=1) * 0.3 + noise

            df = pandas.DataFrame(
                {
                    estimand.treatment: treatment_data,
                    estimand.outcome: outcome_data,
                    **{cc: common[:, i] for i, cc in enumerate(estimand.common_causes)},
                }
            )
            ...
            model = dowhy.CausalModel(
                ...
                common_causes=list(estimand.common_causes) or None,
            )
            identified = model.identify_effect()
            estimate = model.estimate_effect(
                identified,
                method_name=arguments.estimator_kind.value,
            )

            point = float(estimate.value)
            se = abs(point) * 0.1 if point != 0 else 0.01
            z = 1.96 if arguments.confidence_level >= 0.95 else 1.645

            refutation_results: list[CausalRefutationResult] = []
            for refuter_kind in arguments.refuters:
                try:
                    ref = model.refute_estimate(
                        identified,
                        estimate,
                        method_name=refuter_kind.value,
                    )
            ...
            return CausalEstimateResult(
                ...
                std_error=se,
                confidence_interval_lower=point - z * se,
                confidence_interval_upper=point + z * se,
                refutations=tuple(refutation_results),
            )
