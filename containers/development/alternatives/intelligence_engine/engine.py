...
from intelligence_engine.cognitive.meta_learning_adapter import MetaLearningAdapter
from intelligence_engine.cognitive.reward_adapter import RewardAdapter
from intelligence_engine.learning_gate import LearningGate
from intelligence_engine.meta.latent_regime_adapter import LatentRegimeEmbedder, RegimeFeatures
...
class IntelligenceEngine(RuntimeEngine):
    ...
    def __init__(
        ...
        *,
        ...
        self._agent_pool = RegimeAgentPool()
        self._agent_orchestrator = AgentOrchestrator(pool=self._agent_pool)
        self._latent_embedder = LatentRegimeEmbedder()
        self._reward_adapter = RewardAdapter()
        self._meta_learning_adapter = MetaLearningAdapter()
    ...
    @property
    def latent_regime_embedder(self) -> LatentRegimeEmbedder:
        return self._latent_embedder
    ...
    @property
    def meta_learning_adapter(self) -> MetaLearningAdapter:
        return self._meta_learning_adapter

    def register_orchestrator_agents(
        self,
        agents: Mapping[str, Any] | None = None,
    ) -> None:
        """Bulk-register agents into the orchestrator.

        ``agents`` maps ``agent_id -> agent_instance``.  The orchestrator
        stores them for per-tick fan-out / fusion in
        :meth:`run_orchestrated_tick`.
        """
        if not agents:
            return
        for agent_id, agent in agents.items():
            self._agent_orchestrator.register(agent_id, agent)

    def run_orchestrated_tick(
        self,
        *,
        tick: MarketTick,
        ...
        """Extended hot path: agent orchestration + latent regime.

        Calls :meth:`run_meta_tick` internally, but before doing so:
        1. fans *tick* to all registered orchestrator agents,
        2. fuses agent decisions into an augmented signal and injects
           it as an ``extra_signal`` into the meta-controller window,
        3. attaches a latent-regime embedding to the fused signal's meta.

        Returns the same triple as :meth:`run_meta_tick`.
        """
        self._agent_orchestrator.observe_tick(tick)
        synth = SignalEvent(
            ts_ns=tick.ts_ns,
            ...
            confidence=0.0,
            trust=0.9,
            meta={"agent_orchestrator": "true"},
        )
        agent_signal = self._agent_orchestrator.decide(
            ts_ns=tick.ts_ns,
            signal=synth,
            symbol=symbol,
        )
        ...
        latent_sig = self._latent_embedder.embed_signal(synth, features)
        latent_sig = SignalEvent(
            ts_ns=latent_sig.ts_ns,
            source=latent_sig.source,
            symbol=agent_signal.side.value if agent_signal.side is not Side.HOLD else symbol,
            side=agent_signal.side,
            confidence=agent_signal.confidence,
            trust=0.9,
            meta=dict(latent_sig.meta),
        )
        return self.run_meta_tick(
            tick=tick,
            context=context,
            extra_signals=(latent_sig,),
        )
        ...
        if hot is not None:
            ...
            if new_regime != self._last_regime:
                ...
                try:
                    ...
                    pass  # IMPLEMENTATION NOTE: Intelligence alternatives implementation deferred for current development phase
    ...
    def check_self(self) -> HealthStatus:
        ...
        if self._microstructure:
            ...
            detail = f"{detail} microstructure=[{modes}]"
        extras = []
        if self._meta_controller_hot_path is not None:
            extras.append("meta_controller=wired")
        extras.append(f"agent_orchestrator={len(self._agent_orchestrator.agent_ids)} agents")
        extras.append("latent_regime=wired")
        extras.append("reward_adapter=wired")
        extras.append("meta_learning=wired")
