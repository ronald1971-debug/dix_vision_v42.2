...
def test_auth_service_token_generation():
    ...
    from cognitive_control_center.shared_services.auth import CognitiveAuthService, get_or_create_token
...
def test_auth_service_middleware():
    ...
    print(f"[PASS] Public paths configuration preserved ({len(PUBLIC_PATHS_EXACT)} exact, {len(PUBLIC_PATH_PREFIXES)} prefixes)")
...
def test_auth_service_cognitive_integration():
    ...
    assert len(state_after["active_entities"]) >= len(state_before["active_entities"]), \
