...
class Anomaly:
    ...
    anomaly_type: (
        str  # "price_spike" | "volume_anomaly" | "sentiment_divergence" | "correlation_break"
    )
    asset: str
...
class HypothesisAutoGenerator:
    ...
    def validate_with_backtest(
        self, hypothesis: Hypothesis, backtest_results: dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            if is_profitable and has_acceptable_risk and has_good_risk_reward:
                ...
                hypothesis.add_evidence(
                    f"Backtest validated: return={total_return:.2%}, sharpe={sharpe_ratio:.2f}"
                )
                ...
                hypothesis.add_evidence(
                    f"Backtest rejected: return={total_return:.2%}, sharpe={sharpe_ratio:.2f}"
                )
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        validated_hypotheses = (
            len([h for h in self._tracker.get_all() if h.status == HypothesisStatus.VALIDATED])
            if self._tracker
            else 0
        )
