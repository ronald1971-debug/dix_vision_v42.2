...
class AnalyzeWalletSkill(Skill):
    ...
    async def _get_transaction_history(self, address: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(
                f"Fetching blockchain data for {address} with compliance weight {data_weight:.2f}"
            )
    ...
    def _get_simulated_transactions(
        self, address: str, compliance_weight: float = 1.0
    ) -> List[Dict[str, Any]]:
            ...
            transactions.append(
                {
                    ...
                    "from": (
                        ...
                        else f"0x{''.join(random.choices('0123456789abcdef', k=40))}"
                    ),
    ...
    async def _get_holdings(self, address: str) -> Dict[str, Any]:
        ...
        if data_weight < 0.3:
            self.logger.info(
                f"Data compliance weight {data_weight:.2f} < 0.3, using simulated holdings"
            )
        ...
        try:
            ...
            self.logger.info(
                f"Fetching holdings for {address} with compliance weight {data_weight:.2f}"
            )
