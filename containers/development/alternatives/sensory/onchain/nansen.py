...
class NansenClient:
    ...
    def __init__(self, *, api_key: str = "", in_memory: bool | None = None) -> None:
        ...
        self._placeholder_txns: list[SmartMoneyTx] = []

    def get_smart_money_txns(self, *, token: str = "", limit: int = 50) -> list[SmartMoneyTx]:
        """Get recent smart money transactions."""
        if self._in_memory:
            if token:
                return [t for t in self._placeholder_txns if t.token == token][:limit]
            return self._placeholder_txns[:limit]
    ...
    def get_address_labels(self, address: str) -> list[str]:
        ...
        if self._in_memory:
            for tx in self._placeholder_txns:
    ...
    def add_mock_tx(self, tx: SmartMoneyTx) -> None:
        ...
        self._placeholder_txns.append(tx)
