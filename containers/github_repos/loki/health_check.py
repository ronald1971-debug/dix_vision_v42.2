...
logger = logging.getLogger('loki_health_check')

def check_imports():
    try:
        import loki_api_client
        logger.info("Loki import successful")
        return True
    except ImportError as e:
        logger.error(f"Loki import failed: {e}")
        return False
...
def main():
    logger.info(f"Loki Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Loki Container is healthy")
        sys.exit(0)
    else:
        logger.error("Loki Container health check failed")
        sys.exit(1)
