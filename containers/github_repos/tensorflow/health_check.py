...
logger = logging.getLogger('tensorflow_health_check')

def check_imports():
    try:
        import tensorflow
        logger.info("TensorFlow import successful")
        return True
...
def main():
    logger.info(f"TensorFlow Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("TensorFlow Container is healthy")
        sys.exit(0)
    else:
        logger.error("TensorFlow Container health check failed")
        sys.exit(1)
