#!/usr/bin/env python3
"""
dagster Container Health Check

This script performs health checks for the dagster container.
"""
...
logger = logging.getLogger('dagster_health_check')
...
def check_governance_wrapper():
    ...
    try:
        ...
        wrapper = DagsterGovernanceWrapper(PermissionLevel.READ_ONLY)
...
def check_configuration():
    ...
    import os
    config_path = "/app/config/dagster_config.yaml"
...
def main():
    ...
    logger.info(f"dagster Container Health Check - {datetime.utcnow().isoformat()}")
    ...
    if passed == total:
        logger.info("dagster Container is healthy")
        sys.exit(0)
    else:
        logger.error("dagster Container health check failed")
        sys.exit(1)
