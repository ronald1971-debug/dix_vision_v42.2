...
logger = logging.getLogger('django_health_check')

def check_imports():
    try:
        logger.info("Django import successful")
        return True
...
def main():
    logger.info(f"Django Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Django Container is healthy")
        sys.exit(0)
    else:
        logger.error("Django Container health check failed")
        sys.exit(1)
