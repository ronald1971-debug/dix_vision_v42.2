#!/usr/bin/env python3
"""
sentry-sdk Container Health Check

This script performs health checks for the sentry-sdk container.
"""
...
logger = logging.getLogger('sentry_sdk_health_check')
...
def check_governance_wrapper():
    ...
    try:
        ...
        wrapper = Sentry_sdkGovernanceWrapper(PermissionLevel.READ_ONLY)
...
def check_domain_adapter():
    ...
    try:
        ...
        adapter = Sentry_sdkDomainAdapter()
...
def check_configuration():
    ...
    import os
    config_path = "/app/config/sentry_sdk_config.yaml"
...
def main():
    ...
    logger.info(f"sentry-sdk Container Health Check - {datetime.utcnow().isoformat()}")
    ...
    if passed == total:
        logger.info("sentry-sdk Container is healthy")
        sys.exit(0)
    else:
        logger.error("sentry-sdk Container health check failed")
        sys.exit(1)
