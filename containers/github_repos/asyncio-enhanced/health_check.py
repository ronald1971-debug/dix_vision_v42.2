...
logger = logging.getLogger('asyncio_enhanced_health_check')
...
def check_imports():
    try:
        ...
        logger.error(f"AsyncIO Enhanced import failed: {e}")
        return False
...
def main():
    logger.info(f"AsyncIO Enhanced Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("AsyncIO Enhanced Container is healthy")
        sys.exit(0)
    else:
        logger.error("AsyncIO Enhanced Container health check failed")
        sys.exit(1)
