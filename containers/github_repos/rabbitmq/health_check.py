...
logger = logging.getLogger('rabbitmq_health_check')

def check_imports():
    try:
        logger.info("RabbitMQ import successful")
        return True
    except ImportError as e:
        logger.error(f"RabbitMQ import failed: {e}")
        return False
...
def main():
    logger.info(f"RabbitMQ Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("RabbitMQ Container is healthy")
        sys.exit(0)
    else:
        logger.error("RabbitMQ Container health check failed")
        sys.exit(1)
