...
logger = logging.getLogger('pytest_enhanced_health_check')

def check_imports():
    try:
        import pytest
        logger.info("Pytest Enhanced import successful")
        return True
    except ImportError as e:
        logger.error(f"Pytest Enhanced import failed: {e}")
        return False
...
def main():
    logger.info(f"Pytest Enhanced Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Pytest Enhanced Container is healthy")
        sys.exit(0)
    else:
        logger.error("Pytest Enhanced Container health check failed")
        sys.exit(1)
