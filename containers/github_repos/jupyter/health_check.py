...
logger = logging.getLogger('jupyter_health_check')

def check_imports():
    try:
        import jupyter
        logger.info("Jupyter import successful")
        return True
    except ImportError as e:
        logger.error(f"Jupyter import failed: {e}")
        return False
...
def main():
    logger.info(f"Jupyter Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        ...
        logger.error("Jupyter Container health check failed")
        sys.exit(1)
