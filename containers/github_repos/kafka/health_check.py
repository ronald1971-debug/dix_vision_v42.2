...
logger = logging.getLogger('kafka_health_check')

def check_imports():
    try:
        import kafka
        logger.info("Kafka import successful")
        return True
...
def main():
    logger.info(f"Apache Kafka Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Apache Kafka Container is healthy")
        sys.exit(0)
    else:
        logger.error("Apache Kafka Container health check failed")
        sys.exit(1)
