...
logger = logging.getLogger('pillow_health_check')

def check_imports():
    try:
        from PIL import Image
        logger.info("Pillow import successful")
        return True
...
def main():
    logger.info(f"Pillow Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Pillow Container is healthy")
        sys.exit(0)
    else:
        logger.error("Pillow Container health check failed")
        sys.exit(1)
