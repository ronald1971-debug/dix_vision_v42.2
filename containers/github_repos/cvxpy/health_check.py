...
logger = logging.getLogger('cvxpy_health_check')
...
def check_configuration():
    ...
    import os
    config_path = "/app/config/cvxpy_config.yaml"
...
def main():
    ...
    logger.info(f"cvxpy Container Health Check - {datetime.utcnow().isoformat()}")
    ...
    if passed == total:
        logger.info("cvxpy Container is healthy")
        sys.exit(0)
    else:
        logger.error("cvxpy Container health check failed")
        sys.exit(1)
