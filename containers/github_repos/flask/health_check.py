...
logger = logging.getLogger('flask_health_check')
...
def main():
    logger.info(f"Flask Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Flask Container is healthy")
        sys.exit(0)
    else:
        logger.error("Flask Container health check failed")
        sys.exit(1)
