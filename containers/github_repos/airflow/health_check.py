...
logger = logging.getLogger('airflow_health_check')

def check_imports():
    try:
        logger.info("Airflow import successful")
        return True
...
def main():
    logger.info(f"Airflow Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Airflow Container is healthy")
        sys.exit(0)
    else:
        logger.error("Airflow Container health check failed")
        sys.exit(1)
