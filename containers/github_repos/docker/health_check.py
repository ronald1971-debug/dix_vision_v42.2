...
def check_imports():
    try:
        logger.info("Docker import successful")
        return True
    except ImportError as e:
        logger.error(f"Docker import failed: {e}")
        return False
...
def main():
    logger.info(f"Docker Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        ...
        logger.error("Docker Container health check failed")
        sys.exit(1)
