...
logger = logging.getLogger('sklearn_health_check')
...
def main():
    logger.info(f"Scikit-Learn Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Scikit-Learn Container is healthy")
        sys.exit(0)
    else:
        logger.error("Scikit-Learn Container health check failed")
        sys.exit(1)
