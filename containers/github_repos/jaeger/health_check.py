...
logger = logging.getLogger('jaeger_health_check')
...
def check_imports():
    try:
        ...
        logger.error(f"Jaeger import failed: {e}")
        return False
...
def main():
    logger.info(f"Jaeger Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Jaeger Container is healthy")
        sys.exit(0)
    else:
        logger.error("Jaeger Container health check failed")
        sys.exit(1)
