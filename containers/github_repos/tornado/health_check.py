#!/usr/bin/env python3
"""
tornado Container Health Check

This script performs health checks for the tornado container.
"""
...
logger = logging.getLogger('tornado_health_check')
...
def check_governance_wrapper():
    ...
    try:
        ...
        from tornado_governance_wrapper import TornadoGovernanceWrapper
        
        wrapper = TornadoGovernanceWrapper(PermissionLevel.READ_ONLY)
...
def check_domain_adapter():
    ...
    try:
        from tornado_domain_adapter import TornadoDomainAdapter
        adapter = TornadoDomainAdapter()
...
def check_configuration():
    ...
    import os
    config_path = "/app/config/tornado_config.yaml"
...
def main():
    ...
    logger.info(f"tornado Container Health Check - {datetime.utcnow().isoformat()}")
    ...
    if passed == total:
        logger.info("tornado Container is healthy")
        sys.exit(0)
    else:
        logger.error("tornado Container health check failed")
        sys.exit(1)
