...
logger = logging.getLogger('opencv_health_check')
...
def main():
    logger.info(f"OpenCV Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("OpenCV Container is healthy")
        sys.exit(0)
    else:
        logger.error("OpenCV Container health check failed")
        sys.exit(1)
