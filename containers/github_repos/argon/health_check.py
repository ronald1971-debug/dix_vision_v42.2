...
logger = logging.getLogger('argon_health_check')
...
def check_governance_wrapper():
    ...
    try:
        ...
        wrapper = ArgonGovernanceWrapper(PermissionLevel.READ_ONLY)
...
def check_configuration():
    ...
    import os
    config_path = "/app/config/argon_config.yaml"
...
def main():
    ...
    logger.info(f"argon Container Health Check - {datetime.utcnow().isoformat()}")
    ...
    if passed == total:
        logger.info("argon Container is healthy")
        sys.exit(0)
    else:
        logger.error("argon Container health check failed")
        sys.exit(1)
