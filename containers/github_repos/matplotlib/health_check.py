...
logger = logging.getLogger('matplotlib_health_check')

def check_imports():
    try:
        logger.info("Matplotlib import successful")
        return True
...
def main():
    logger.info(f"Matplotlib Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Matplotlib Container is healthy")
        sys.exit(0)
    else:
        logger.error("Matplotlib Container health check failed")
        sys.exit(1)
