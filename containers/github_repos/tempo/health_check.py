...
logger = logging.getLogger('tempo_health_check')

def check_imports():
    try:
        logger.info("Tempo import successful")
        return True
    except ImportError as e:
        logger.error(f"Tempo import failed: {e}")
        return False
...
def main():
    logger.info(f"Tempo Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Tempo Container is healthy")
        sys.exit(0)
    else:
        logger.error("Tempo Container health check failed")
        sys.exit(1)
