#!/usr/bin/env python3
"""
nltk Container Health Check

This script performs health checks for the nltk container.
"""
...
logger = logging.getLogger('nltk_health_check')
...
def check_governance_wrapper():
    ...
    try:
        ...
        wrapper = NltkGovernanceWrapper(PermissionLevel.READ_ONLY)
...
def check_configuration():
    ...
    import os
    config_path = "/app/config/nltk_config.yaml"
...
def main():
    ...
    logger.info(f"nltk Container Health Check - {datetime.utcnow().isoformat()}")
    ...
    if passed == total:
        logger.info("nltk Container is healthy")
        sys.exit(0)
    else:
        logger.error("nltk Container health check failed")
        sys.exit(1)
