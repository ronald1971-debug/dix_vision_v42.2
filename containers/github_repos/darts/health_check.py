...
logger = logging.getLogger('darts_health_check')

def check_imports():
    try:
        import darts
        logger.info("Darts import successful")
        return True
    except ImportError as e:
        logger.error(f"Darts import failed: {e}")
        return False
...
def main():
    logger.info(f"Darts Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Darts Container is healthy")
        sys.exit(0)
    else:
        logger.error("Darts Container health check failed")
        sys.exit(1)
