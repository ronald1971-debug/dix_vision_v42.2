...
logger = logging.getLogger('consul_health_check')

def check_imports():
    try:
        import consul
        logger.info("Consul import successful")
        return True
    except ImportError as e:
        logger.error(f"Consul import failed: {e}")
        return False
...
def main():
    logger.info(f"Consul Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Consul Container is healthy")
        sys.exit(0)
    else:
        logger.error("Consul Container health check failed")
        sys.exit(1)
