#!/usr/bin/env python3
"""
slowapi Container Health Check

This script performs health checks for the slowapi container.
"""
...
logger = logging.getLogger('slowapi_health_check')
...
def check_governance_wrapper():
    ...
    try:
        ...
        wrapper = SlowapiGovernanceWrapper(PermissionLevel.READ_ONLY)
...
def check_configuration():
    ...
    import os
    config_path = "/app/config/slowapi_config.yaml"
...
def main():
    ...
    logger.info(f"slowapi Container Health Check - {datetime.utcnow().isoformat()}")
    ...
    if passed == total:
        logger.info("slowapi Container is healthy")
        sys.exit(0)
    else:
        logger.error("slowapi Container health check failed")
        sys.exit(1)
