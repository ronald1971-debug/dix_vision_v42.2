...
logger = logging.getLogger('discordbot_health_check')

def check_imports():
    try:
        import discord
        logger.info("DiscordBot import successful")
        return True
    except ImportError as e:
        logger.error(f"DiscordBot import failed: {e}")
        return False
...
def main():
    logger.info(f"DiscordBot Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("DiscordBot Container is healthy")
        sys.exit(0)
    else:
        logger.error("DiscordBot Container health check failed")
        sys.exit(1)
