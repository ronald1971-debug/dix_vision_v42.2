...
logger = logging.getLogger('elasticsearch_health_check')

def check_imports():
    try:
        logger.info("Elasticsearch import successful")
        return True
    except ImportError as e:
        logger.error(f"Elasticsearch import failed: {e}")
        return False
...
def main():
    logger.info(f"Elasticsearch Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Elasticsearch Container is healthy")
        sys.exit(0)
    else:
        logger.error("Elasticsearch Container health check failed")
        sys.exit(1)
