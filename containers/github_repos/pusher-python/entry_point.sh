#!/bin/bash
# pusher-python Container Entry Point Script

set -e

echo "Starting pusher-python Container for DIX VISION..."
echo "Version: 42.2"
echo "Timestamp: $(date)"

# Load environment variables if .env file exists
if [ -f /app/config/.env ]; then
    echo "Loading environment variables from .env file"
    export $(cat /app/config/.env | grep -v '^#' | xargs)
fi

# Create necessary directories
mkdir -p /app/logs
mkdir -p /app/data
mkdir -p /app/config

# Start the governance wrapper
echo "Starting pusher-python Governance Wrapper..."
python3 -c "
from pusher_python_governance_wrapper import Pusher_pythonGovernanceWrapper
from pusher_python_domain_adapter import Pusher_pythonDomainAdapter
import logging

# Setup logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger('pusher_python_container')

try:
    logger.info('pusher-python Governance Wrapper initialized successfully')
    logger.info('Ready to process operations with governance oversight')
    
    # Keep container running
    import time
    while True:
        time.sleep(3600)
        
except Exception as e:
    logger.error(f'pusher-python Container error: {str(e)}')
    raise
" || {
    echo "pusher-python container failed to start"
    exit 1
}
