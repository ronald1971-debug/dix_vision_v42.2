...
logger = logging.getLogger('graphql_health_check')
...
def main():
    logger.info(f"GraphQL Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("GraphQL Container is healthy")
        sys.exit(0)
    else:
        logger.error("GraphQL Container health check failed")
        sys.exit(1)
