...
logger = logging.getLogger('ray_health_check')
...
def check_imports():
    try:
        ...
        logger.error(f"Ray import failed: {e}")
        return False
...
def main():
    logger.info(f"Ray Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Ray Container is healthy")
        sys.exit(0)
    else:
        logger.error("Ray Container health check failed")
        sys.exit(1)
