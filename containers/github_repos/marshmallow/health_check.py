...
logger = logging.getLogger('marshmallow_health_check')

def check_imports():
    try:
        import marshmallow
        logger.info("Marshmallow import successful")
        return True
    except ImportError as e:
        logger.error(f"Marshmallow import failed: {e}")
        return False
...
def main():
    logger.info(f"Marshmallow Container Health Check - {datetime.utcnow().isoformat()}")
    checks = [
    ...
    if passed == total:
        logger.info("Marshmallow Container is healthy")
        sys.exit(0)
    else:
        logger.error("Marshmallow Container health check failed")
        sys.exit(1)
