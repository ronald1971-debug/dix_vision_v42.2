...
class PerformanceOptimizer:
    ...
    def collect_performance_metric(self, target: OptimizationTarget, component: str,
                                  metric_name: str, value: float, unit: str,
                                  threshold: float = None, is_critical: bool = False) -> PerformanceMetric:
        ...
        if threshold is None:
            ...
            threshold = self.baseline_metrics.get(baseline_key, value * self.config.performance_threshold_multiplier)
    ...
    def _trigger_optimization(self, target: OptimizationTarget, component: str,
        ...
        if len([p for p in self.optimization_plans.values() if p.status in [OptimizationStatus.IMPLEMENTING, OptimizationStatus.ANALYZING]]) < self.config.max_concurrent_optimizations:
    ...
    def _determine_optimization_strategy(self, target: OptimizationTarget,
                                      current_value: float, threshold: float) -> OptimizationStrategy:
    ...
    def _implement_optimization(self, plan: OptimizationPlan) -> bool:
        ...
        improvement = (plan.current_value - plan.target_value) / plan.current_value if plan.current_value > 0 else 0.0
    ...
    def analyze_performance_trends(self, target: OptimizationTarget,
    ...
    def recommend_optimizations(self, component: str) -> List[OptimizationStrategy]:
        ...
        for metric in component_metrics:
            if metric.value > metric.threshold:
                ...
                strategy = self._determine_optimization_strategy(metric.target, metric.value, metric.threshold)
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        completed_plans = [plan for plan in self.optimization_plans.values() if plan.status == OptimizationStatus.COMPLETED]
