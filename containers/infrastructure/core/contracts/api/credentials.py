...
class PresenceStateApi:
    ...
    def get_summary(self) -> CredentialsSummary:
        ...
        invalid = sum(
            1 for c in credentials if c.state in [PresenceState.INVALID, PresenceState.EXPIRED]
        )
        missing = sum(1 for c in credentials if c.state == PresenceState.ABSENT)

        return CredentialsSummary(
            total=len(credentials), present=present, valid=present, invalid=invalid, missing=missing
        )
