...
Domain.GOVERNANCE = Domain("governance", "Governance Domain", DomainKind.GOVERNANCE, AuthorityLevel.CRITICAL)
...
Domain.OPERATIONS = Domain("operations", "Operations Domain", DomainKind.OPERATIONS, AuthorityLevel.HIGH)
Domain.SYSTEM = Domain("system", "System Domain", DomainKind.SYSTEM, AuthorityLevel.CRITICAL)
Domain.INFRASTRUCTURE = Domain("infrastructure", "Infrastructure Domain", DomainKind.INFRASTRUCTURE, AuthorityLevel.HIGH)
Domain.DATA = Domain("data", "Data Domain", DomainKind.MARKET_DATA, AuthorityLevel.HIGH)
Domain.ANALYTICS = Domain("analytics", "Analytics Domain", DomainKind.ANALYTICS, AuthorityLevel.MEDIUM)
...
def create_domain(domain_id: str, name: str, kind: DomainKind, authority_level: AuthorityLevel) -> Domain:
    """Create a new domain"""
    return Domain(
        domain_id=domain_id,
        name=name,
...
def create_authority_record(domain_id: str, action: str, actor: str) -> AuthorityRecord:
    ...
    return AuthorityRecord(
        record_id=f"{domain_id}_{int(time.time())}",
        domain_id=domain_id,
        action=action,
        actor=actor
    )
