...
Voice.GOVERNANCE = Voice(
    "governance", "Governance Voice", VoiceKind.AUTHORITATIVE, "critical", "governance"
)
...
Voice.RISK_MANAGER = Voice(
    "risk_manager", "Risk Manager Voice", VoiceKind.AUTHORITATIVE, "high", "risk"
)
...
Voice.COMPLIANCE = Voice(
    "compliance", "Compliance Voice", VoiceKind.AUTHORITATIVE, "high", "governance"
)
Voice.SAFETY = Voice("safety", "Safety Voice", VoiceKind.CRITICAL, "critical", "governance")
Voice.EMERGENCY = Voice(
    "emergency", "Emergency Voice", VoiceKind.EMERGENCY, "critical", "governance"
)
Voice.OPERATIONS = Voice(
    "operations", "Operations Voice", VoiceKind.COLLABORATIVE, "medium", "operations"
)
...
class Charter:
    ...
    def __post_init__(self):
        ...
        if not self.charter_id:
            if self.domain:
                domain_name = (
                    self.domain.domain_id if hasattr(self.domain, "domain_id") else str(self.domain)
                )
...
class CharterRegistry:
    ...
    def register_charter(self, charter: Charter) -> bool:
        ...
        if not charter.charter_id:
            ...
            domain_name = (
                domain_value.domain_id
                if hasattr(domain_value, "domain_id")
                else (
                    str(domain_value)
                    if domain_value
                    else (charter.voice.domain if charter.voice else "unknown")
                )
            )
...
def create_charter(charter_id: str, name: str, domain: str) -> Charter:
    ...
    return Charter(charter_id=charter_id, name=name, domain=domain)


def create_voice(
    voice_id: str, name: str, kind: VoiceKind, authority_level: str, domain: str
) -> Voice:
    """Create a new voice"""
    return Voice(
        voice_id=voice_id, name=name, kind=kind, authority_level=authority_level, domain=domain
    )
