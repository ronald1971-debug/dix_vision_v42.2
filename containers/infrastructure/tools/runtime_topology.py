...
class RuntimeEdge:
    ...
    def __init__(self, edge_id: str = None, **kwargs):
        ...
        for key, value in kwargs.items():
            if key not in ['edge_id', 'source_id', 'target_id', 'relation', 'properties', 'timestamp', 'metadata']:
...
class RuntimeNode:
    ...
    def __init__(self, node_id: str, **kwargs):
        ...
        self.lifecycle_state = kwargs.get('lifecycle_state') or kwargs.get('state') or kwargs.get('state_attr', LifecycleState.READY)
        self.properties = kwargs.get('properties', {})
        self.timestamp = kwargs.get('timestamp', time.time())
        self.metadata = kwargs.get('metadata', {})
        ...
        # Store any additional parameters in metadata
        for key, value in kwargs.items():
            if key not in ['node_id', 'kind', 'node_kind', 'tier', 'node_tier', 'lifecycle_state', 'state', 'state_attr', 'properties', 'timestamp', 'metadata', 'capabilities', 'declared_version']:
                self.metadata[key] = value
