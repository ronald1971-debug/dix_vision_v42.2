...
class CognitiveCost:
    ...
    def __post_init__(self):
        ...
        if self.benefit_cost_ratio == 0 and self.total_cost > 0:
            ...
            self.benefit_cost_ratio = (
                self.expected_benefit / self.total_cost if self.total_cost > 0 else 0.0
            )
...
class CognitiveEconomyManagerInterface(ABC):
    ...
    def optimize_resource_allocation(
        self, resources: Dict[str, float], demands: List[Dict[str, Any]]
    ) -> Dict[str, float]:
        """Optimize cognitive resource distribution."""

    @abstractmethod
    def track_cognitive_budget(
        self, budget_id: str, resource_type: CognitiveResourceType
    ) -> CognitiveBudget:
    ...
    def analyze_cost_benefit(
        self, operation_id: str, expected_outcomes: Dict[str, Any]
    ) -> CognitiveCost:
...
class CognitiveEconomyManager(CognitiveEconomyManagerInterface):
    ...
    def optimize_resource_allocation(
        ...
        prioritized_demands = sorted(
            demands, key=lambda d: self._priority_value(d.get("priority", "medium")), reverse=True
        )
    ...
    def _priority_value(self, priority_str: str) -> float:
        ...
        priority_map = {"critical": 5.0, "high": 4.0, "medium": 3.0, "low": 2.0, "background": 1.0}
    ...
    def make_allocation_decision(
        ...
        budget = self.track_cognitive_budget(f"{resource_type.value}_budget", resource_type)

        # Calculate cost
        cost = self.calculate_cognitive_cost(
            operation_id, resource_type, {"requested_amount": requested_amount}
        )
        ...
        if budget.available_budget >= requested_amount:
            ...
            if priority_value >= 4.0:  # High or critical priority
                ...
                rationale = (
                    f"Cost-effective operation (benefit/cost ratio: {cost.benefit_cost_ratio:.2f})"
                )
            else:
                approved = False
    ...
    def analyze_cost_benefit(
        ...
        if cost_history:
            ...
            cost = CognitiveCost(
                operation_id=operation_id, resource_type=CognitiveResourceType.CUSTOM
            )
        ...
        cost.benefit_cost_ratio = (
            cost.expected_benefit / cost.total_cost if cost.total_cost > 0 else 0.0
        )
            ...
            "allocation_summary": {
                ...
                "decisions": {
                    decision_id: {
                        ...
                        "benefit_cost_ratio": (
                            decision.cost_benefit_analysis.benefit_cost_ratio
                            if decision.cost_benefit_analysis
                            else 0.0
                        ),
                    }
