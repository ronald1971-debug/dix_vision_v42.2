...
class OperatingModeManagerInterface(ABC):
    ...
    def transition_to_mode(
        self, target_mode: OperatingMode, reason: ModeTransitionReason, initiator: str = ""
    ) -> ModeTransition:
        """Transition to a new operating mode."""

    @abstractmethod
    def can_transition_to(
        self, target_mode: OperatingMode, check_policies: bool = True
    ) -> tuple[bool, str]:
...
class OperatingModeManager(OperatingModeManagerInterface):
    ...
    def _initialize_default_policies(self):
        ...
        self._mode_policies[OperatingMode.ACTIVE] = ModePolicy(
            ...
            mode=OperatingMode.ACTIVE,
            forbidden_transitions=[OperatingMode.OFFLINE],  # Require intermediate step
            entry_conditions=[
    ...
    def can_transition_to(
        ...
        if check_policies:
            ...
            if current_policy:
                ...
                if (
                    current_policy.allowed_transitions
                    and target_mode not in current_policy.allowed_transitions
                ):
        ...
        if policy:
            ...
            for condition in policy.entry_conditions:
                ...
                if condition_type == "risk_check":
                    ...
                    conditions[f"risk_check_{max_level}"] = self._risk_level_compare(
                        current_risk, max_level
                    )
    ...
    def _execute_hooks(
        self, mode: OperatingMode, hooks_dict: Dict[OperatingMode, List[Callable]]
    ) -> None:
