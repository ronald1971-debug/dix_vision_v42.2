...
class LearningGatePolicy:
    ...
    allowed_states: List[LearningGateState] = field(
        default_factory=lambda: [LearningGateState.OPEN, LearningGateState.RESTRICTED]
    )
...
class LearningGateManager(LearningGateManagerInterface):
    ...
    def _initialize_default_policy(self):
        ...
        self._policy.learning_windows = [
            {"start_hour": 0, "end_hour": 6, "description": "Off-peak learning window"},
            {"start_hour": 12, "end_hour": 14, "description": "Midday learning window"},
        ]

        # Set blackout periods (e.g., during market open)
        self._policy.blackout_periods = [
            {"start_hour": 9, "end_hour": 10, "description": "Market open blackout"}
        ]
    ...
    def request_learning_operation(
        ...
        with self._lock:
            ...
            requires_approval = (
                self._policy.require_approval or not in_learning_window or not operation_permitted
            )
    ...
    def _assess_operation_risk(
        self, operation_type: LearningOperationType, parameters: Dict[str, Any]
    ) -> tuple[str, Dict[str, Any]]:
        """Assess risk level for a learning operation."""
        risk_level = "medium"
        risk_assessment = {"factors": [], "score": 0.5}
        ...
        with self._lock:
            ...
            if execution_hook:
                ...
                operation.execution_result = {
                    "success": True,
                    "message": "No handler, auto-completed",
                }
                operation.status = "completed"
                self._metrics.total_operations_completed += 1
            ...
            if operation.started_at and operation.completed_at:
                ...
                total_duration = self._metrics.average_operation_duration_seconds * (
                    self._metrics.total_operations_completed - 1
                )
                self._metrics.average_operation_duration_seconds = (
                    total_duration + duration
                ) / self._metrics.total_operations_completed
    ...
    def register_execution_hook(
        self, operation_type: LearningOperationType, hook: Callable
    ) -> None:
        ...
        with self._lock:
            ...
            return {
                ...
                "operations_summary": {
                    ...
                    "approval_rate": (
                        self._metrics.total_operations_approved
                        / self._metrics.total_operations_requested
                        * 100
                        if self._metrics.total_operations_requested > 0
                        else 0
                    ),
                },
                ...
                "resource_usage": {
                    ...
                    "utilization": (
                        self._metrics.current_active_operations
                        / self._policy.max_concurrent_operations
                        * 100
                        if self._policy.max_concurrent_operations > 0
                        else 0
                    ),
                },
                "performance_metrics": {
