...
class InMemoryKnowledgeGraphAdapter(KnowledgeGraphAdapterInterface):
    ...
    def query(self, query: KnowledgeQuery) -> KnowledgeGraphResult:
        ...
        try:
            ...
            logger.info(f"[KNOWLEDGE_GRAPH] Query executed: {query.query_type} ({execution_time_ms:.2f}ms, {len(result.nodes)} nodes)")
    ...
    def _find_pattern_matches(self, query: KnowledgeQuery, result: KnowledgeGraphResult) -> KnowledgeGraphResult:
    ...
    def delete_edge(self, edge_id: str) -> bool:
        ...
        with self._lock:
            ...
            self._adjacency_outgoing[edge.source_node_id] = [
                eid for eid in self._adjacency_outgoing[edge.source_node_id]
                if eid != edge_id
            ]
            self._adjacency_incoming[edge.target_node_id] = [
                eid for eid in self._adjacency_incoming[edge.target_node_id]
                if eid != edge_id
            ]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_query_time = (
                self._query_total_time_ms / self._query_count
