...
class InMemoryVectorDatabaseAdapter(VectorDatabaseAdapterInterface):
    ...
    def __init__(self, default_vector_size: int = 768):
        ...
        logger.info(
            f"[VECTOR_DB] In-memory adapter initialized (vector_size: {default_vector_size})"
        )
    ...
    def upsert(self, record: VectorRecord) -> str:
        ...
        with self._lock:
            ...
            if collection and len(record.vector) != collection["vector_size"]:
                logger.warning(
                    f"[VECTOR_DB] Vector size mismatch: expected {collection['vector_size']}, got {len(record.vector)}"
                )
                ...
                if len(record.vector) > collection["vector_size"]:
                    ...
                    record.vector = record.vector + [0.0] * (
                        collection["vector_size"] - len(record.vector)
                    )
    ...
    def search(self, query: VectorSearchQuery) -> List[VectorSearchResult]:
        ...
        try:
            ...
            logger.info(
                f"[VECTOR_DB] Search completed: {len(results)} results ({execution_time_ms:.2f}ms)"
            )
    ...
    def create_collection(
        ...
        with self._lock:
            ...
            logger.info(
                f"[VECTOR_DB] Created collection: {collection_name} (vector_size: {vector_size})"
            )
            return True
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_search_time = (
                self._search_total_time_ms / self._search_count if self._search_count > 0 else 0.0
            )
...
class SemanticMemoryRetriever:
    ...
    def retrieve(
        ...
        try:
            ...
            logger.info(
                f"[SEMANTIC_MEMORY] Retrieved {len(results)} semantic memories for: {query_text[:50]}..."
            )
    ...
    def store(
        ...
        try:
            ...
            record = VectorRecord(
                record_id=record_id,
                vector=embedding,
                payload={"text": text, "created_at": datetime.utcnow().isoformat()},
