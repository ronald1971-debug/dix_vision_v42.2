...
class PriceActionAnalysis:
    ...
    def calculate_technical_indicators(self, data: pd.DataFrame) -> TechnicalIndicators:
        ...
        if len(data) < self.min_period:
            raise ValueError(f"Insufficient data for indicator calculation: {len(data)} < {self.min_period}")
        ...
        try:
            ...
            return TechnicalIndicators(
                ...
                bollinger_middle=bollinger_middle[-1] if not np.isnan(bollinger_middle[-1]) else 0.0,
    ...
    def detect_support_resistance_levels(self, data: pd.DataFrame, 
                                       window: int = 5, 
                                       min_strength: float = 0.3) -> List[float]:
        """
    ...
    def detect_breakouts(self, data: pd.DataFrame, 
                        ...
                        penetration_threshold: float = 0.005) -> List[PatternRecognitionResult]:
        """
    ...
    def calculate_volatility(self, data: pd.DataFrame, period: int = 20) -> VolatilityMetrics:
        ...
        """
        if len(data) < period:
            raise ValueError(f"Insufficient data for volatility calculation: {len(data)} < {period}")
