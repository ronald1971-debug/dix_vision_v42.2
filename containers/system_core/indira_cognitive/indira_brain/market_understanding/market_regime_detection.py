...
class MarketRegimeDetection:
    ...
    def extract_market_features(self, data: pd.DataFrame) -> MarketStateFeatures:
        ...
        if len(data) < self.window_size:
            raise ValueError(
                f"Insufficient data for feature extraction: {len(data)} < {self.window_size}"
            )
        ...
        volume_profile = {
            ...
            "volume_trend": (
                data["volume"].tail(20).mean() / data["volume"].tail(self.window_size).mean()
                if data["volume"].tail(self.window_size).mean() > 0
                else 1.0
            ),
        }
        ...
        price_momentum = (
            (data["close"].iloc[-1] - data["close"].iloc[-self.window_size])
            / data["close"].iloc[-self.window_size]
            if data["close"].iloc[-self.window_size] > 0
            else 0
        )
        ...
        resistance_proximity = (
            (recent_highs - current_price) / recent_highs if recent_highs > 0 else 0
        )
    ...
    def detect_regime(self, data: pd.DataFrame) -> RegimeDetectionResult:
        ...
        if len(data) < self.window_size:
            raise ValueError(
                f"Insufficient data for regime detection: {len(data)} < {self.window_size}"
            )
    ...
    def _generate_regime_description(
        self, regime: MarketRegime, features: MarketStateFeatures
    ) -> str:
        """
    ...
    def detect_anomalies(self, data: pd.DataFrame, symbol: str = "BTC/USD") -> List[MarketAnomaly]:
        ...
        logger.info("Anomaly detection completed", anomalies_detected=len(anomalies), symbol=symbol)
    ...
    def predict_regime_transition(
        self, data: pd.DataFrame, lookahead_minutes: int = 60
