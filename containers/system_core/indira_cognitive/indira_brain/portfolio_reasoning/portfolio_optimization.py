...
class PortfolioOptimization:
    ...
    def optimize_mean_variance(
        ...
        if self.config.constraints.target_return is not None:
            constraints.append(
                {
                    "type": "eq",
                    "fun": lambda weights: np.dot(weights, expected_returns.values)
                    - self.config.constraints.target_return,
                }
            )
            ...
            constraints.append(
                {
                    ...
                    "fun": lambda weights: np.sqrt(
                        np.dot(weights.T, np.dot(covariance_matrix.values, weights))
                    )
                    - self.config.constraints.target_volatility,
                }
            )
        ...
        optimized_portfolio = OptimizedPortfolio(
            ...
            metadata={"optimization_status": result.message, "objective_value": result.fun},
        )

        logger.info(
            "Mean-variance optimization completed",
            expected_return=expected_return,
            expected_volatility=expected_volatility,
            sharpe_ratio=sharpe_ratio,
            constraints_satisfied=constraints_satisfied,
    ...
    def optimize_max_sharpe(
        ...
        logger.info(
            "Max Sharpe optimization completed",
            expected_return=expected_return,
            expected_volatility=expected_volatility,
            sharpe_ratio=sharpe_ratio,
            constraints_satisfied=constraints_satisfied,
    ...
    def optimize_min_variance(self, covariance_matrix: pd.DataFrame) -> OptimizedPortfolio:
        ...
        logger.info(
            "Min variance optimization completed",
            expected_volatility=expected_volatility,
            constraints_satisfied=constraints_satisfied,
        )
    ...
    def optimize_portfolio(
        ...
        if method == OptimizationMethod.MEAN_VARIANCE:
            if expected_returns is None or covariance_matrix is None:
                raise ValueError(
                    "Mean-variance optimization requires expected returns and covariance matrix"
                )
            ...
            if expected_returns is None or covariance_matrix is None:
                raise ValueError(
                    "Max Sharpe optimization requires expected returns and covariance matrix"
                )
    ...
    def optimize_equal_weight(self, expected_returns: pd.Series = None) -> OptimizedPortfolio:
        ...
        logger.info(
            "Equal weight portfolio created", num_assets=num_assets, equal_weight=equal_weights
        )
