...
class PortfolioRiskMeasurement:
    ...
    def calculate_var(self, returns: pd.Series, confidence_level: float = 0.95) -> float:
        ...
        if len(returns) < self.config.min_data_points:
            raise ValueError(f"Insufficient data for VaR calculation: {len(returns)} < {self.config.min_data_points}")
    ...
    def calculate_cvar(self, returns: pd.Series, confidence_level: float = 0.95) -> float:
        ...
        """
        if len(returns) < self.config.min_data_points:
            raise ValueError(f"Insufficient data for CVaR calculation: {len(returns)} < {self.config.min_data_points}")
    ...
    def calculate_portfolio_beta(self, portfolio_returns: pd.Series, 
        ...
        logger.debug("Portfolio beta calculated",
                   beta=beta,
    ...
    def calculate_correlation_matrix(self, asset_returns: pd.DataFrame) -> Dict[str, Dict[str, float]]:
        """
        ...
        if len(asset_returns) < self.config.min_data_points:
            raise ValueError(f"Insufficient data for correlation calculation: {len(asset_returns)} < {self.config.min_data_points}")
        ...
        logger.info("Correlation matrix calculated",
                   ...
                   avg_correlation=np.mean(correlation_matrix.values[np.triu_indices_from(correlation_matrix.shape, k=1)]))
        
        return correlations
    
    def calculate_portfolio_volatility(self, returns: pd.Series, 
        ...
        logger.debug("Portfolio volatility calculated",
                   volatility=volatility,
    ...
    def calculate_max_drawdown(self, cumulative_returns: pd.Series) -> float:
        ...
        logger.debug("Max drawdown calculated",
    ...
    def calculate_concentration_risk(self, holdings: Dict[str, float], 
        ...
        logger.debug("Concentration risk calculated",
                   hhi=hhi,
    ...
    def calculate_total_exposure(self, holdings: Dict[str, float], 
        ...
        total_exposure = gross_exposure / (gross_exposure + cash) if (gross_exposure + cash) > 0 else 1.0
        
        logger.debug("Total exposure calculated",
    ...
    def calculate_portfolio_risk_metrics(self, portfolio_id: str,
                                        holdings: Dict[str, float],
                                        returns_data: pd.DataFrame,
                                        market_data: pd.Series = None) -> PortfolioRiskMetrics:
        """
    ...
    def _calculate_portfolio_returns(self, holdings: Dict[str, float],
    ...
    def run_stress_test(self, holdings: Dict[str, float],
                      returns_data: pd.DataFrame,
                      shock_scenarios: List[Dict[str, float]]) -> Dict[str, float]:
        """
