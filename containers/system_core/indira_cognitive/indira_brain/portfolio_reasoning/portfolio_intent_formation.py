...
class PortfolioIntentFormation:
    ...
    def form_rebalance_intent(self, current_portfolio: Dict[str, float],
                             target_portfolio: Dict[str, float],
                             risk_metrics: Dict[str, float],
                             performance_attribution: Dict[str, float]) -> PortfolioIntent:
        """
        ...
        if deviation < self.config.min_rebalance_threshold:
            logger.info("No rebalance needed", deviation=deviation, threshold=self.config.min_rebalance_threshold)
            return None
        
        # Calculate rebalance trades (real trade calculation)
        rebalance_trades = self._calculate_rebalance_trades(current_portfolio, target_portfolio, current_value)
        ...
        logger.info("Rebalance intent formed",
    ...
    def _calculate_portfolio_deviation(self, current: Dict[str, float],
    ...
    def _calculate_rebalance_trades(self, current_portfolio: Dict[str, float],
                                   target_portfolio: Dict[str, float],
        ...
        current_weights = {asset: value / current_value for asset, value in current_portfolio.items()}
        target_weights = {asset: value / sum(target_portfolio.values()) for asset, value in target_portfolio.items()}
        ...
        for asset in all_assets:
            ...
            if abs(weight_diff) > 0.01:  # > 1% difference
                ...
                if target_weight <= self.config.max_position_size:
                    ...
                    logger.warning("Position size exceeds limit", asset=asset, target_weight=target_weight)
        
        return trades
    
    def _calculate_expected_impact(self, trades: List[Dict[str, Any]],
        ...
        expected_volatility_change = turnover * 0.1  # Simplified model: 10% of turnover affects volatility
    ...
    def _calculate_rebalance_priority(self, deviation: float,
    ...
    def _calculate_rebalance_confidence(self, performance_attribution: Dict[str, float],
                                       risk_metrics: Dict[str, float]) -> float:
    ...
    def _generate_risk_mitigation(self, risk_metrics: Dict[str, float],
    ...
    def _determine_rebalance_action(self, deviation: float,
                                   risk_metrics: Dict[str, float]) -> PortfolioAction:
    ...
    def form_risk_adjustment_intent(self, risk_metrics: Dict[str, float],
                                   risk_limits: Dict[str, float],
                                   current_portfolio: Dict[str, float]) -> PortfolioIntent:
        """
        ...
        logger.info("Risk adjustment intent formed",
                   intent_id=intent.intent_id,
    ...
    def _check_risk_limits(self, risk_metrics: Dict[str, float],
    ...
    def _calculate_risk_adjustments(self, breaches: Dict[str, float],
    ...
    def _apply_adjustments(self, current_portfolio: Dict[str, float],
