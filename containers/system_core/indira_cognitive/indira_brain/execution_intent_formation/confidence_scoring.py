...
class ConfidenceScoring:
    ...
    def calculate_risk_management_confidence(self, trade_idea: TradeIdea) -> float:
        ...
        if trade_idea.stop_loss:
            stop_distance = (
                abs(trade_idea.entry_price - trade_idea.stop_loss) / trade_idea.entry_price
            )
    ...
    def calculate_historical_performance_confidence(
    ...
    def calculate_overall_confidence(
        ...
        if sentiment_data is not None:
            factor_scores["market_sentiment"] = self.calculate_market_sentiment_confidence(
                sentiment_data
            )
        else:
        ...
        if historical_trades is not None:
            factor_scores["historical_performance"] = (
                self.calculate_historical_performance_confidence(
                    trade_idea.symbol if trade_idea else "unknown", historical_trades
                )
            )
        else:
    ...
    def get_confidence_summary(self) -> Dict[str, Any]:
        ...
        avg_overall_confidence = np.mean(
            [score.overall_confidence for score in self.confidence_history]
        )
        avg_factor_scores = {}
        ...
        for factor in self.config.factor_weights.keys():
            factor_values = [
                score.factor_scores.get(factor, 0.5) for score in self.confidence_history
            ]
