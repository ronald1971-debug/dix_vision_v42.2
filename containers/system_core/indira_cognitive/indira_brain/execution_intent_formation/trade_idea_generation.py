...
class TradeIdeaGeneration:
    ...
    def generate_trade_from_opportunity(self, opportunity: MarketOpportunity,
                                      account_balance: float = 100000.0) -> Optional[TradeIdea]:
        """
        ...
        if opportunity.confidence < self.config.confidence_threshold:
            logger.info("Opportunity confidence below threshold",
        ...
        if opportunity.risk_reward_ratio < self.config.min_risk_reward_ratio:
            logger.info("Risk-reward ratio below threshold",
                       opportunity_id=opportunity.opportunity_id,
        ...
        logger.info("Trade idea generated from opportunity",
    ...
    def _determine_trade_type(self, opportunity: MarketOpportunity) -> TradeType:
        ...
        if opportunity.opportunity_type in [OpportunityType.PRICE_BREAKOUT, 
    ...
    def _calculate_take_profit(self, opportunity: MarketOpportunity,
    ...
    def generate_batch_trades(self, opportunities: List[MarketOpportunity],
                            account_balance: float = 100000.0) -> List[TradeIdea]:
        """
        ...
        for opportunity in sorted_opportunities:
            ...
            if total_position_risk + potential_position_size <= 0.30:  # Max 30% total exposure
                ...
                logger.info("Position risk limit reached, skipping opportunity",
                           opportunity_id=opportunity.opportunity_id,
                           total_position_risk=total_position_risk)
                continue
        
        logger.info("Batch trade generation completed",
    ...
    def get_trade_summary(self) -> Dict[str, Any]:
        ...
        total_position_value = sum(trade.metadata.get('position_value', 0) for trade in self.trade_ideas)
