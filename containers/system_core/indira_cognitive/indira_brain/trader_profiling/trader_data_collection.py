...
class TraderDataCollection:
    ...
    def __init__(self, config: DataCollectionConfig = None):
        ...
        self.trader_behaviors: Dict[str, deque] = defaultdict(
            lambda: deque(maxlen=self.config.max_behaviors_per_trader)
        )
    ...
    def extract_decision_patterns(
        self, trader_id: str, lookback_days: int = 30
        ...
        if len(recent_behaviors) < self.config.min_trades_for_profiling:
            logger.warning(
                "Insufficient behaviors for pattern extraction",
                trader_id=trader_id,
        ...
        logger.info(
            "Decision patterns extracted", trader_id=trader_id, patterns_found=len(patterns)
        )

        return patterns

    def _detect_trend_following_pattern(
        self, behaviors: List[TraderBehavior]
    ) -> Optional[DecisionPattern]:
        """Detect trend following pattern (real pattern detection)"""
        buy_signals = [
            b for b in behaviors if b.action in [TradeAction.MARKET_BUY, TradeAction.LIMIT_BUY]
        ]
        sell_signals = [
            b for b in behaviors if b.action in [TradeAction.MARKET_SELL, TradeAction.LIMIT_SELL]
        ]
    ...
    def _detect_scalping_pattern(
        ...
        for i in range(1, len(sorted_behaviors)):
            delta = (
                sorted_behaviors[i].timestamp - sorted_behaviors[i - 1].timestamp
            ).total_seconds()
        ...
        for i in range(1, len(sorted_behaviors)):
            if sorted_behaviors[i].action in [TradeAction.MARKET_SELL, TradeAction.LIMIT_SELL]:
                ...
                for j in range(i - 1, -1, -1):
                    if sorted_behaviors[j].action in [
                        TradeAction.MARKET_BUY,
                        TradeAction.LIMIT_BUY,
                    ]:
                ...
                if buy_behavior and buy_behavior.symbol == sorted_behaviors[i].symbol:
                    trade_return = (
                        sorted_behaviors[i].price - buy_behavior.price
                    ) / buy_behavior.price
        ...
        if confidence >= 0.6 and avg_hold_time < 3600:
            return DecisionPattern(
                ...
                avg_risk=avg_risk,
                metadata={
                    "avg_hold_time_hours": avg_hold_time / 3600,
    ...
    def calculate_performance_metrics(
        ...
        if len(recent_behaviors) < self.config.min_trades_for_profiling:
            raise ValueError(
                f"Insufficient data for performance calculation: {len(recent_behaviors)} < {self.config.min_trades_for_profiling}"
            )
        ...
        avg_win = (
            np.mean([t["return"] for t in trades if t["return"] > 0]) if winning_trades > 0 else 0.0
        )
        avg_loss = (
            np.mean([abs(t["return"]) for t in trades if t["return"] < 0])
            if losing_trades > 0
            else 0.0
        )
            ...
            for behavior in sorted_behaviors:
                if behavior.action in [TradeAction.MARKET_BUY, TradeAction.LIMIT_BUY]:
                    ...
                    if open_positions:
                        ...
                        trade_return = (behavior.price - position["buy_price"]) / position[
                            "buy_price"
                        ]
                        hold_time = (
                            behavior.timestamp - position["buy_time"]
    ...
    def cleanup_old_behaviors(self, trader_id: str = None, retention_days: int = None) -> int:
