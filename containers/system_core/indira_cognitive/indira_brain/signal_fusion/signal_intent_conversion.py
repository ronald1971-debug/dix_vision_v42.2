...
class SignalIntentConversion:
    ...
    def classify_intent_type(self, cluster: IntentCluster) -> IntentType:
        ...
        buy_signals = sum(
            ...
            if s.get("signal_type", "").lower() in ["buy", "long", "entry"]
        )
        sell_signals = sum(
            1
            for s in cluster.signals
            if s.get("signal_type", "").lower() in ["sell", "short", "exit"]
        )
    ...
    def convert_cluster_to_intent(self, cluster: IntentCluster) -> TradingIntent:
        ...
        logger.info(
            "Signal cluster converted to intent",
            intent_id=intent_id,
    ...
    def _determine_intent_action(
        self, intent_type: IntentType, cluster: IntentCluster
    ) -> IntentAction:
    ...
    def _calculate_intent_urgency(self, cluster: IntentCluster) -> float:
        ...
        if timestamps:
            avg_age = sum((datetime.now() - ts).total_seconds() for ts in timestamps) / len(
                timestamps
            )
    ...
    def validate_intent_governance(self, intent: TradingIntent) -> bool:
        ...
        if intent.confidence < self.config.confidence_threshold:
            logger.warning(
                "Intent rejected due to low confidence",
        ...
        if intent.position_size > 0.10:  # Max 10% position
            logger.warning(
                "Intent rejected due to excessive position size",
                intent_id=intent.intent_id,
    ...
    def process_signals_to_intents(self, signals: List[Dict[str, Any]]) -> List[TradingIntent]:
        ...
        logger.info(
            "Signal to intent conversion completed",
    ...
    def get_intent_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_type": {
                k: v for k, v in self.conversion_statistics.items() if k.startswith("intent_type_")
            },
            "conversion_rate": len(self.intent_history)
            / max(1, self.conversion_statistics.get("total_intents", 1)),
        }
