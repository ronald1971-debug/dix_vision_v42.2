...
class LeakyIntegrateFireNeuron:
    ...
    def __init__(self, neuron_id: str, tau_mem: float = 20.0, 
    ...
    def process_spike(self, spike_time: float, input_current: float = 1.0,
...
class TradingSpikingNetwork:
    ...
    def __init__(self, n_input_neurons: int = 50, 
        ...
        logger.info(f"[INDIRA_SNN] Trading Spiking Network initialized with {n_input_neurons} input, "
    ...
    def encode_market_data_to_spikes(self, market_data: Dict[str, Any], 
                                   encoding: SpikeEncoding = SpikeEncoding.TEMPORAL) -> List[TradingSpikeEvent]:
        """
        ...
        if encoding == SpikeEncoding.RATE:
            ...
            spike_events.append(TradingSpikeEvent(
    ...
    def process_spikes(self, spike_events: List[TradingSpikeEvent], 
                      dt: float = 1.0, duration_ms: float = 100.0) -> SNNResponse:
        """
    ...
    def _get_input_spikes_at_time(self, spike_events: List[TradingSpikeEvent], 
                                  current_time_ms: float) -> List[float]:
    ...
    def _propagate_spikes_to_hidden(self, input_idx: int, input_neuron: LeakyIntegrateFireNeuron,
                                    current_time_ms: float, dt: float) -> None:
    ...
    def _propagate_spikes_to_output(self, hidden_neuron: LeakyIntegrateFireNeuron,
                                    current_time_ms: float, dt: float) -> None:
    ...
    def _extract_temporal_features(self, output_spikes: Dict[str, List[float]]) -> Dict[str, float]:
        ...
        spike_rate = spike_count / (max(all_spikes) - min(all_spikes)) if len(all_spikes) > 1 else 0.0
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            total_spikes = sum(len(neuron.spike_history) for neuron in 
            ...
            return {
                ...
                "average_firing_rate": total_spikes / max(1, len(self.input_neurons + self.hidden_neurons + self.output_neurons)),
                "synaptic_weights_range": {
                    "input_to_hidden": (float(np.min(self.input_to_hidden_weights)), 
                                       float(np.max(self.input_to_hidden_weights))),
                    "hidden_to_output": (float(np.min(self.hidden_to_output_weights)), 
