...
class KnowledgeValidator:
    ...
    def validate_knowledge(
        ...
        logger.info(f"Validated knowledge from {source}: {level.value} (confidence: {base_confidence:.2f})")
        ...
        if "price" in knowledge and "current_prices" in context:
            ...
            if symbol in context["current_prices"]:
                ...
                if abs(new_price - existing_price) / existing_price > 0.10:  # 10% deviation
                    conflicts.append(f"Price conflict for {symbol}: {new_price} vs {existing_price}")
    ...
    def update_source_reliability(self, source: KnowledgeSource, delta: float) -> None:
        ...
        logger.info(f"Updated {source} reliability: {current:.2f} -> {self._source_reliability[source]:.2f}")
