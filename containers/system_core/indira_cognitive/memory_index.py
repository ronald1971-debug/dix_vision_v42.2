...
class MemoryIndexSystem:
    ...
    def __init__(self):
        ...
        self._access_stats = {"retrievals": 0, "hits": 0, "misses": 0, "total_latency_ms": 0}
    ...
    def index_memory(
        ...
        logger.info(
            f"Indexed memory {memory_id} of type {memory_type.value} with {len(index.tags)} tags"
        )
        return memory_id
    ...
    def link_memories(self, memory_id_1: str, memory_id_2: str, bidirectional: bool = True) -> None:
        ...
        logger.info(
            f"Linked memories: {memory_id_1} {'<->' if bidirectional else '->'} {memory_id_2}"
        )
    ...
    def optimize_access_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_access_count": (
                sum(idx.access_count for idx in self._index.values()) / len(self._index)
                if self._index
                else 0
            ),
            "cache_hit_rate": (
                self._access_stats["hits"] / self._access_stats["retrievals"]
                if self._access_stats["retrievals"] > 0
                else 0
            ),
            "average_latency_ms": (
                self._access_stats["total_latency_ms"] / self._access_stats["hits"]
                if self._access_stats["hits"] > 0
                else 0
            ),
        }
    ...
    def get_index_summary(self) -> Dict[str, Any]:
        ...
        type_distribution = {
            mem_type.value: len(mem_ids) for mem_type, mem_ids in self._type_index.items()
        }
        source_distribution = {
            source: len(mem_ids) for source, mem_ids in self._source_index.items()
        }
