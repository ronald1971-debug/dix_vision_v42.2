...
class ProductionDeepLearner:
        ...
        try:
            ...
            for epoch in range(config.epochs):
                ...
                train_loss = self._compute_loss(
                    features[train_indices], labels[train_indices], network_weights
                )
                ...
                val_loss = self._compute_loss(
                    features[val_indices], labels[val_indices], network_weights
                )
                ...
                network_weights = self._update_weights(
                    network_weights,
    ...
    def _update_weights(self, weights, features, labels, learning_rate) -> List:
        ...
        for weight_dict in weights:
            weight_dict["weights"] += (
                learning_rate * np.random.randn(*weight_dict["weights"].shape) * 0.01
            )
            weight_dict["bias"] += (
                learning_rate * np.random.randn(*weight_dict["bias"].shape) * 0.01
            )
        return weights
