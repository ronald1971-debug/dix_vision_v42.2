"""Meta-Learning Loop / Self-Evolution Module (AIX SEM) — learns HOW to learn.
...
AIX SEM integration:
  - Proactive strategy creation via meta-learning
  - Cross-regime adaptation when market conditions shift
  - Self-evolution of trading approach based on cumulative experience
  - Foresight: predict when learning rules themselves need changing
"""
...
class MetaLearningLoop:
    ...
    # -- AIX: Self-Evolution Module (SEM) extensions -----------------------

    @property
    def evolution_readiness(self) -> float:
        """How ready the system is for proactive strategy evolution [0,1].

        High readiness = stable + improving + explored enough.
        """
        stability = self._compute_stability()
    ...
    def should_evolve(self) -> bool:
        """Whether the system should proactively create new strategies.

        True when exploitation has been stable long enough and performance
        is good — time to push boundaries instead of just exploiting.
        """
        return (
            self._mode == LearningMode.EXPLOIT
            and self._cycles_in_mode > self._patience // 2
            and self.evolution_readiness > 0.7
        )
