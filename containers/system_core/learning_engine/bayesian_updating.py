...
class BayesianUpdating:
    ...
    def _update_belief(self, event: LearningEvent, belief_id: str) -> Optional[BayesianUpdate]:
        ...
        logger.info(
            f"Updated belief {belief_id}: {prior:.3f} -> {posterior:.3f} (Bayes factor: {bayes_factor:.3f})"
        )
        return update
    ...
    def calculate_performance_deltas(
        self, belief_id: str, time_window_hours: float = 24.0
    ) -> Dict[str, float]:
        """
        ...
        recent_updates = [
            u for u in self._updates if u.belief_id == belief_id and u.update_time > cutoff
        ]
        ...
        performance_delta = (
            recent_updates[-1].posterior_probability - recent_updates[0].prior_probability
        )
    ...
    def update_strategy_scores(self, strategy_id: str, performance_data: Dict[str, Any]) -> None:
        ...
        belief_id = list(self._beliefs.keys())[
            list(self._beliefs.values()).index(
                belief
                for belief in self._beliefs.values()
                if belief.topic == f"Strategy {strategy_id} Performance"
            )
        ][0]
    ...
    def update_execution_scores(self, execution_id: str, execution_data: Dict[str, Any]) -> None:
        ...
        matching_beliefs = [
            bid
            for bid in execution_beliefs
            if f"Execution {execution_id}" in self._beliefs[bid].topic
        ]
    ...
    def update_operator_models(self, operator_id: str, behavior_data: Dict[str, Any]) -> None:
        ...
        matching_beliefs = [
            bid for bid in operator_beliefs if f"Operator {operator_id}" in self._beliefs[bid].topic
        ]
    ...
    def get_learning_summary(self) -> Dict[str, Any]:
        ...
        avg_confidence_improvement = (
            statistics.mean(confidence_improvements) if confidence_improvements else 0.0
        )
        self._performance_metrics["average_confidence_improvement"] = avg_confidence_improvement
        self._performance_metrics["belief_convergence_rate"] = (
            convergent_beliefs / total_beliefs if total_beliefs > 0 else 0.0
        )
