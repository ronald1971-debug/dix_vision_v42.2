...
class SuccessPredictor:
    ...
    def predict_success(
        self, operation: str, attempt: int, world_context: Optional[WorldContext]
    ) -> float:
...
class ExponentialBackoffStrategy(RetryStrategy):
    ...
    def should_retry(self, attempt: int, error: Exception) -> bool:
        return attempt < self._config.max_attempts and isinstance(
            error, self._config.retryable_exceptions
        )
...
class AdaptiveRetryStrategy:
    ...
    def __init__(self, config: Optional[RetryConfig] = None):
        ...
        logger.info(
            f"[ADAPTIVE_RETRY] Initialized with policy: {self._config.policy}, world_aware: {self._config.world_aware}"
        )
    ...
    def _calculate_backoff_with_world_context(
        self, attempt: int, world_context: Optional[WorldContext], strategy: RetryStrategy
    ) -> float:
    ...
    def _update_performance(self, policy: RetryPolicy, success: bool) -> None:
        ...
        # Update success rate
        if success:
            perf["success_rate"] = (perf["success_rate"] * (perf["attempts"] - 1) + 1.0) / perf[
                "attempts"
            ]
        else:
            perf["success_rate"] = (perf["success_rate"] * (perf["attempts"] - 1)) / perf[
                "attempts"
            ]
    ...
    def adapt_strategy(self) -> None:
        ...
        with self._lock:
            ...
            if best_policy[0] != self._config.policy:
                logger.info(
                    f"[ADAPTIVE_RETRY] Adapting strategy from {self._config.policy} to {best_policy[0]}"
                )
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "policy_performance": {
                    policy.value: perf for policy, perf in self._policy_performance.items()
                },
                # Phase 12.1 enhancements
