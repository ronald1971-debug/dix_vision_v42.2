...
from .adaptive_retry import AdaptiveRetryStrategy, RetryConfig
...
class DistributedExecutionResilience:
    ...
    def __init__(self, service_name: str, checkpoint_dir: str = "checkpoints"):
        self._service_name = service_name
        self._lock = threading.Lock()

        # Resilience components
        self._circuit_breaker = get_circuit_breaker(
            f"{service_name}_circuit", CircuitBreakerConfig(failure_threshold=5, timeout_ms=30000)
        )
    ...
    def execute_with_resilience(
        ...
        if enable_circuit_breaker:
            ...
            circuit_result = self._circuit_breaker.execute(
                circuit_protected_func, fallback=self._fallback_function
            )
            ...
            if not circuit_result.success:
                ...
                logger.warning(
                    f"[DISTRIBUTED_RESILIENCE] Circuit breaker opened for {self._service_name}"
                )
        ...
        return ExecutionResult(
            ...
            execution_time_ms=execution_time_ms,
            circuit_state=(
                self._circuit_breaker.get_state().value if enable_circuit_breaker else None
            ),
        )
    ...
    def restore_from_checkpoint(self, component: str) -> bool:
        ...
        if restore_result.success:
            logger.info(
                f"[DISTRIBUTED_RESILIENCE] Restored from checkpoint {checkpoint.checkpoint_id}"
            )
            self._apply_restored_state(restore_result.restored_state)
            return True
        else:
            logger.error(
                f"[DISTRIBUTED_RESILIENCE] Checkpoint restore failed: {restore_result.error_message}"
            )
            return False
    ...
    def _apply_restored_state(self, state: Dict[str, Any]) -> None:
        ...
        logger.info(
            f"[DISTRIBUTED_RESILIENCE] Applied restored state from {state.get('timestamp')}"
        )
...
def get_distributed_resilience(
    service_name: str, checkpoint_dir: str = "checkpoints"
    ...
    if service_name not in _resilience_managers:
        with _resilience_managers_lock:
            if service_name not in _resilience_managers:
                _resilience_managers[service_name] = DistributedExecutionResilience(
                    service_name, checkpoint_dir
                )
