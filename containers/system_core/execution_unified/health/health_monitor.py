...
class AnomalyDetector:
    ...
    def detect_anomaly(
        self, component: str, metrics: Dict[str, float], confidence_threshold: float = 0.95
    ) -> bool:
...
class HealthMonitor:
    ...
    def _collect_system_metrics(self) -> Dict[str, float]:
        ...
        try:
            metrics = {
                ...
                "disk_percent": (
                    psutil.disk_usage("/").percent if hasattr(psutil, "disk_usage") else 0.0
                ),
    ...
    def register_provider(self, provider: HealthCheckProvider) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"[HEALTH_MONITOR] Registered health check provider: {provider.component_name}"
            )

    def check_component_health(
        self, component: str, world_context: Optional[WorldContext] = None
    ) -> HealthCheck:
        ...
        if provider:
            try:
                ...
                if health_check.metrics:
                    ...
                    health_check.confidence_interval = (
                        self._anomaly_detector.calculate_confidence_interval(metric_values)
                    )
    ...
    def _apply_world_aware_thresholds(
        self, health_check: HealthCheck, world_context: WorldContext
    ) -> HealthCheck:
        ...
        if world_context.volatility_regime == "high":
            ...
            if (
                ...
                and health_check.metrics.get("error_rate", 0) > 0.01
            ):
                health_check.status = HealthStatus.DEGRADED
    ...
    def check_system_health(self) -> SystemHealthReport:
        ...
        unhealthy = sum(
            1 for hc in component_checks.values() if hc.status == HealthStatus.UNHEALTHY
        )
        ...
        health_score = (
            (healthy + degraded * 0.5) / total_components if total_components > 0 else 0.0
        )
    ...
    def _predict_health_score(
        self, current_score: float, trend: str, world_context: Optional[WorldContext]
    ) -> float:
        ...
        if world_context:
            if world_context.volatility_regime == "high":
                ...
                world_context.market_trend == "stable" and world_context.volatility_regime == "low"
            ):
