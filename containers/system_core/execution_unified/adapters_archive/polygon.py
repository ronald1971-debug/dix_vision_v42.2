...
class PolygonAdapter:
    ...
    trade data. API key from system_unified_engine/credentials/.
    ...
    def __init__(self, *, api_key: str = "", in_memory: bool | None = None) -> None:
        ...
        # Auto-detect: live mode when API key is present, placeholder otherwise
        self._in_memory = in_memory if in_memory is not None else (not bool(api_key))
        self._placeholder_bars: list[OHLCVBar] = []
    ...
    def get_aggs(
        ...
        *,
        ...
        if self._in_memory:
            return self._placeholder_bars[:limit]
    ...
    def get_last_trade(self, ticker: str) -> OHLCVBar | None:
        ...
        if self._in_memory:
            return self._placeholder_bars[-1] if self._placeholder_bars else None
        return self._fetch_last_trade(ticker)

    def add_placeholder_bar(self, bar: OHLCVBar) -> None:
        """Add a mock bar for testing."""
        self._placeholder_bars.append(bar)
