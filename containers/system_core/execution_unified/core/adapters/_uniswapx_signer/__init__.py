...
def build_exclusive_dutch_order_typed_data(order_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        'orderType': 'EXCLUSIVE_DUTCH_ORDER',
        'data': order_data
    }
...
def sign_typed_data(typed_data: Dict[str, Any], private_key: str = "") -> Dict[str, Any]:
    ...
    return {
        'signature': 'mock_signature_' + str(__import__('hashlib').md5(str(typed_data).encode()).hexdigest()),
        'typed_data': typed_data
    }

__all__ = ['UniswapXSigner', 'build_exclusive_dutch_order_typed_data', 'intent_from_quote_payload', 'sign_typed_data']