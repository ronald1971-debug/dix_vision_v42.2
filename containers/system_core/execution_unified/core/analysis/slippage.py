...
class SlippageAnalyzer:
    ...
    def get_average_slippage(self) -> float:
        ...
        if not self._slippage_history:
            return 0.0
        return sum(s["slippage_percent"] for s in self._slippage_history) / len(self._slippage_history)
    
    def estimate(self, execution_data: Dict[str, Any] = None, qty: float = 0, adv_qty: float = 0, spread_bps: int = 0) -> SlippageResult:
