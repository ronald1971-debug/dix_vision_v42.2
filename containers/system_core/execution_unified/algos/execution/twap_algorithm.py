...
class TWAPAlgorithm:
    ...
    def create_execution(
        ...
        execution_id = (
            f"TWAP_{symbol}_{datetime.utcnow().timestamp()}:{datetime.utcnow().nanosecond()}"
        )

        # Create slices based on strategy
        slices = self._create_slices(
            total_quantity, start_time, end_time, num_slices, strategy, market_data
        )
        ...
        logger.info(
            f"Created TWAP execution: {execution_id} for {symbol} ({total_quantity} units in {num_slices} slices)"
        )
        return execution
    ...
    def _create_slices(
        ...
        slice_quantities = self._calculate_slice_quantities(
            total_quantity, num_slices, strategy, market_data
        )
    ...
    def _calculate_slice_quantities(
        ...
        if strategy == TWAPStrategy.STANDARD:
            ...
            if market_data and "expected_hourly_volume" in market_data:
                ...
                quantities = [
                    hourly_volume * participation_rate * slice_duration_hours
                ] * num_slices
    ...
    def execute_slice(
        ...
        execution_result = self._execute_market_order(
            slice_obj.target_quantity, current_price, market_conditions, order_type
        )
        ...
        logger.info(
            f"Executed slice {slice_id}: {slice_obj.executed_quantity} @ {slice_obj.average_price:.4f}"
        )
        return slice_obj
    ...
    def _calculate_slippage(
        self, quantity: float, current_price: float, market_conditions: Dict[str, Any]
    ) -> float:
    ...
    def modify_execution(
        ...
        executed_quantity = sum(
            s.executed_quantity for s in execution.slices if s.status == "completed"
        )
