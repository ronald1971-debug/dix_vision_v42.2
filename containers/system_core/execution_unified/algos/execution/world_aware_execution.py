...
class WorldAwareTWAP:
    ...
    def _create_world_aware_context(self, symbol: str, world_context: Dict[str, Any]) -> WorldAwareExecutionContext:
    ...
    def _adapt_execution_parameters(
        ...
        high_activity_agents = [
            agent for agent, level in world_context.agent_activity_snapshot.items()
            if level > 0.7
        ]
...
class WorldAwarePOV:
    ...
    def _adapt_execution_parameters(
        ...
        if world_context.world_regime == 'high_volatility':
            ...
            logger.info(f"[WORLD_AWARE_POV] High volatility: reduced target to {adapted_params['target_percentage']:.2%}")
            ...
            logger.info(f"[WORLD_AWARE_POV] Bullish trending: increased target to {adapted_params['target_percentage']:.2%}")
        
        # Adjust based on liquidity
        if world_context.world_liquidity == 'low':
            # Reduce target further in low liquidity
            adapted_params['target_percentage'] = adapted_params['target_percentage'] * 0.5
            logger.info(f"[WORLD_AWARE_POV] Low liquidity: further reduced target to {adapted_params['target_percentage']:.2%}")
        ...
        if any(level > 0.8 for level in world_context.agent_activity_snapshot.values()):
            ...
            logger.info("[WORLD_AWARE_POV] Very high agent activity: reduced target to avoid front-running")
        ...
        if world_context.prediction_confidence > 0.85:
            if 'liquidity_inflow' in world_context.causal_factors_active:
                ...
                adapted_params['target_percentage'] = min(adapted_params['target_percentage'] * 1.1, 0.2)
...
def create_world_aware_executor(algorithm_type: str, algorithm_instance) -> Any:
    ...
    if algorithm_type == 'twap':
        ...
        logger.warning(f"[WORLD_AWARE_EXECUTOR] Unknown algorithm type {algorithm_type}, returning original")
