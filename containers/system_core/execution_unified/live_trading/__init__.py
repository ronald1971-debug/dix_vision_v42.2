...
class LiveTradingManager:
    ...
    async def check_risk_limits(self) -> Tuple[bool, List[str]]:
        ...
        if self._metrics.max_drawdown > self._risk_constraints.max_drawdown:
            violations.append(f"Max drawdown {self._metrics.max_drawdown} exceeds limit {self._risk_constraints.max_drawdown}")
        
        # Check daily loss
        if self._metrics.daily_pnl < -self._risk_constraints.max_daily_loss:
            violations.append(f"Daily loss {self._metrics.daily_pnl} exceeds limit {self._risk_constraints.max_daily_loss}")
        ...
        if current_exposure > self._risk_constraints.max_exposure:
            violations.append(f"Current exposure {current_exposure} exceeds limit {self._risk_constraints.max_exposure}")
