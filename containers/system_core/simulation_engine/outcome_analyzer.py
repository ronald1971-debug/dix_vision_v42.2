...
class ProductionOutcomeAnalyzer:
    ...
    def analyze_outcome(self, simulation_run_id: str, results: Dict[str, Any]) -> OutcomeAnalysis:
        ...
        if trades:
            ...
            if trade_outcomes:
                ...
                metrics[AnalysisMetric.WIN_RATE] = (
                    len(wins) / len(trade_outcomes) if trade_outcomes else 0
                )
                metrics[AnalysisMetric.AVG_WIN] = (
                    sum(t.pnl for t in wins) / len(wins) if wins else 0
                )
                metrics[AnalysisMetric.AVG_LOSS] = (
                    sum(t.pnl for t in losses) / len(losses) if losses else 0
                )
                metrics[AnalysisMetric.PROFIT_FACTOR] = (
                    abs(sum(t.pnl for t in wins) / sum(t.pnl for t in losses)) if losses else 0
                )
        ...
        logger.info(
            f"[OUTCOME_ANALYZER] Analyzed outcome: {analysis_id} with {len(insights)} insights"
        )
        return analysis
    ...
    def _calculate_risk_reward(self, trade_outcomes: List[TradeOutcome]) -> float:
        ...
        total_risk = sum(
            abs(t.max_adverse_excursion) for t in trade_outcomes if t.max_adverse_excursion > 0
        )
    ...
    def _generate_recommendations(
        self, metrics: Dict[AnalysisMetric, float], risk_analysis: RiskAnalysis
    ) -> List[str]:
        ...
        if AnalysisMetric.PROFIT_FACTOR in metrics and metrics[AnalysisMetric.PROFIT_FACTOR] < 1.5:
            recommendations.append(
                "Improve risk-reward ratio by tightening stop losses or letting winners run"
            )

        if AnalysisMetric.SHARPE_RATIO in metrics and metrics[AnalysisMetric.SHARPE_RATIO] < 0.5:
            recommendations.append(
                "Improve risk-adjusted returns by reducing volatility or increasing consistency"
            )
