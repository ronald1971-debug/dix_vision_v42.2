...
class SyntheticMarketEngine:
    ...
    def tick(self, ts_ns: int) -> None:
        try:
            with self._lock:
                ...
                dv = self._kappa * (
                    self._sigma - self._current_vol
                ) * 0.01 + self._vol_of_vol * math.sqrt(
                    max(self._current_vol, 1e-6)
                ) * self._rng.gauss(
                    0.0, 0.1
                )
                ...
                log_ret = self._drift - 0.5 * self._current_vol**2 + self._current_vol * z
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        realized_vol = (
            math.sqrt(sum(r**2 for r in returns) / max(1, len(returns))) if returns else 0.0
        )
...
__all__ = ["SyntheticMarketEngine", "SyntheticBar", "SyntheticBook", "get_synthetic_market_engine"]
