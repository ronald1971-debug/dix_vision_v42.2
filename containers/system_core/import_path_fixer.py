...
class ImportPathFixer:
    ...
    def fix_import_paths(self, file_path):
        ...
        try:
            ...
            # Replace execution_engine.* with execution_unified.*
            if 'from execution_engine.' in content or 'import execution_engine.' in content:
                content = re.sub(r'from execution_engine\.', 'from execution_unified.', content)
                content = re.sub(r'import execution_engine\.', 'import execution_unified.', content)
                fixes_applied += 1
            ...
            # Replace cognitive_governance.* with governance_unified.domains.cognitive.*
            if 'from cognitive_governance.' in content or 'import cognitive_governance.' in content:
                content = re.sub(r'from cognitive_governance\.engine', 'from governance_unified.domains.cognitive.cognitive_engine', content)
                content = re.sub(r'import cognitive_governance', 'import governance_unified.domains.cognitive as cognitive_governance', content)
                fixes_applied += 1
            ...
            content = re.sub(r'from execution\.protections\.', 'from execution_unified.core.protections.', content)
            content = re.sub(r'from execution_unified\.protections\.', 'from execution_unified.core.protections.', content)
            content = re.sub(r'from governance\.domains\.', 'from governance_unified.domains.', content)
            content = re.sub(r'from execution_unified\.semi_auto\.', 'from execution_unified.core.semi_auto.', content)
            content = re.sub(r'from execution_unified\.adapters\.registry', 'from execution_unified.engine_archive.registry', content)
            content = re.sub(r'from execution_unified\.adapters\.', 'from execution_unified.adapters.', content)
            # Fix state imports
            content = re.sub(r'from state\.', 'from state.', content)  # Keep as-is if already correct
            # Fix core imports that should be from system_core
            content = re.sub(r'import core\.', 'import core.', content)  # Keep as-is if already correct
...
def main():
    ...
    # Fix intelligence_engine archival components
    print("\nFixing intelligence_engine archival components...")
    intelligence_files = fixer.fix_directory('intelligence_engine')
    print(f"Processed {fixer.files_processed} total files")
    print(f"Applied {fixer.import_fixes_applied} total import fixes")
    
    # Fix learning_engine archival components
    print("\nFixing learning_engine archival components...")
    learning_files = fixer.fix_directory('learning_engine')
    print(f"Processed {fixer.files_processed} total files")
    print(f"Applied {fixer.import_fixes_applied} total import fixes")
    
    # Fix evolution_engine archival components
    print("\nFixing evolution_engine archival components...")
    evolution_files = fixer.fix_directory('evolution_engine')
    print(f"Processed {fixer.files_processed} total files")
    print(f"Applied {fixer.import_fixes_applied} total import fixes")
    
    # Fix user_interfaces/dashboard_backend archival components
    print("\nFixing user_interfaces/dashboard_backend archival components...")
    dashboard_files = fixer.fix_directory('../user_interfaces/dashboard_backend')
    print(f"Processed {fixer.files_processed} total files")
    print(f"Applied {fixer.import_fixes_applied} total import fixes")
    
    # Fix user_interfaces/ui archival components
    print("\nFixing user_interfaces/ui archival components...")
    ui_files = fixer.fix_directory('../user_interfaces/ui')
    print(f"Processed {fixer.files_processed} total files")
    print(f"Applied {fixer.import_fixes_applied} total import fixes")
    ...
    all_files = execution_files + governance_files + intelligence_files + learning_files + evolution_files + dashboard_files + ui_files
    for file_path, fixes in all_files[:10]:
        if fixes > 0:
