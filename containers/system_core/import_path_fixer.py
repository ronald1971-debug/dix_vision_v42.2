    ...
    def fix_import_paths(self, file_path):
        ...
        try:
            ...
            if "from governance_engine." in content or "import governance_engine." in content:
                ...
                content = re.sub(
                    r"import governance_engine\.", "import governance_unified.", content
                )
                fixes_applied += 1
            ...
            if "from cognitive_governance." in content or "import cognitive_governance." in content:
                content = re.sub(
                    r"from cognitive_governance\.engine",
                    "from governance_unified.domains.cognitive.cognitive_engine",
                    content,
                )
                content = re.sub(
                    r"import cognitive_governance",
                    "import governance_unified.domains.cognitive as cognitive_governance",
                    content,
                )
                fixes_applied += 1
            ...
            content = re.sub(
                r"from execution\.adapters\.", "from execution_unified.adapters.", content
            )
            content = re.sub(r"from execution\.core\.", "from execution_unified.core.", content)
            content = re.sub(
                r"from execution\.protections\.",
                "from execution_unified.core.protections.",
                content,
            )
            content = re.sub(
                r"from execution_unified\.protections\.",
                "from execution_unified.core.protections.",
                content,
            )
            content = re.sub(
                r"from governance\.domains\.", "from governance_unified.domains.", content
            )
            content = re.sub(
                r"from execution_unified\.semi_auto\.",
                "from execution_unified.core.semi_auto.",
                content,
            )
            content = re.sub(
                r"from execution_unified\.adapters\.registry",
                "from execution_unified.engine_archive.registry",
                content,
            )
            content = re.sub(
                r"from execution_unified\.adapters\.", "from execution_unified.adapters.", content
            )
            # Fix state imports
            content = re.sub(
                r"from state\.", "from state.", content
            )  # Keep as-is if already correct
            # Fix core imports that should be from system_core
            content = re.sub(
                r"import core\.", "import core.", content
            )  # Keep as-is if already correct
