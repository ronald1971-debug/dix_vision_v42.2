...
class GovernedEvolutionPipeline:
    ...
    def _apply_to_registry(self, record: PipelineRecord, ts_ns: int) -> None:
        ...
        try:
            ...
            _logger.warning(f"Failed to promote strategy in registry: {e}")
            _logger.debug("Strategy promotion failed, but pipeline will continue")
    ...
    def _store_audit(self, record: PipelineRecord, ts_ns: int) -> None:
        ...
        try:
            ...
            _logger.warning(f"Failed to store audit record: {e}")
            _logger.debug("Audit storage failed, but pipeline will continue")
    ...
    def _emit_transition(self, proposal_id: str, stage: PipelineStage, ts_ns: int) -> None:
        ...
        try:
            ...
            _logger.warning(f"Failed to emit patch proposal event: {e}")
            _logger.debug("Patch proposal emission failed, but pipeline will continue")
        try:
            ...
            _logger.warning(f"Failed to emit DYON observability event: {e}")
            _logger.debug("DYON observability emission failed, but pipeline will continue")
        ...
        if repair_mapping is not None:
            ...
            try:
                ...
                _logger.warning(f"Failed to emit repair pipeline event: {e}")
                _logger.debug("Repair pipeline emission failed, but pipeline will continue")
