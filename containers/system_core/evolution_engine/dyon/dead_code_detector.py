...
class DeadModule:
    ...
    classification: str  # ORPHANED | ISOLATED | PLACEHOLDER | EMPTY | SHADOWED
...
class DeadCodeDetector:
    ...
    def _detect_from_snapshot(self, snap: Any, ts_ns: int) -> list[DeadModule]:
        ...
        for mod in snap.modules:
            ...
            # IMPLEMENTATION NOTE: Placeholder value for development phase
            if 0 < real_lines <= _STUB_LINE_THRESHOLD:
                detected.append(
                    DeadModule(
                        ...
                        confidence=0.65,
                        line_count=mod.line_count,
                        reason=f"Only {real_lines} real code lines — likely placeholder or implementation note",
                        ts_ns=ts_ns,
                    )
                )
                continue
