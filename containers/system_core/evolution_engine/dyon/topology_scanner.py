...
# ---------------------------------------------------------------------------
# Rule constants — loaded from ownership_registry.yaml via loader
# ---------------------------------------------------------------------------

# Lazy-load domain definitions from registry with fallback to defaults
_INDIRA_DOMAINS: tuple[str, ...] | None = None
_DYON_DOMAINS: tuple[str, ...] | None = None


def _load_indira_domains() -> tuple[str, ...]:
    """Load INDIRA owned domains from registry."""
    global _INDIRA_DOMAINS
    ...
    return _INDIRA_DOMAINS


def _load_dyon_domains() -> tuple[str, ...]:
    """Load DYON owned domains from registry."""
    global _DYON_DOMAINS
    ...
    return _DYON_DOMAINS


_INDIRA_DOMAINS: frozenset[str] = frozenset(_load_indira_domains())
...
class DyonTopologyScanner:
    ...
    def _emit(result: TopologyScanResult) -> None:
        ...
        try:
            ...
            pass  # IMPLEMENTATION NOTE: DYON implementation deferred for current development phase
