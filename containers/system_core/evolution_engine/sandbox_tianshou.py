...
def tianshou_policy_trainer(
    *,
    ...
    class _TianshouPolicyTrainer:
        ...
        def train(  # pragma: no cover - exercised when tianshou present
            self,
            env: DIXStrategyEnv,
            *,
            episode_config: EpisodeConfig,
            ...
            act_n = env.action_space_n  # 3: BUY, HOLD, SELL
            hidden = 128
            device = "cpu"

            # ------------------------------------------------------------------
            # Gymnasium wrapper — tianshou's Collector requires a gymnasium.Env.
            # We inline the wrapper here (rather than calling
            # gymnasium_dix_strategy_env) so we can pass episode_config and
            # derive per-episode seeds deterministically.
            # ------------------------------------------------------------------
            try:
                import gymnasium
                from gymnasium import spaces
            except ImportError:
                import gym as gymnasium  # type: ignore[no-redef]
                from gym import spaces  # type: ignore[no-redef]

            ep_count_box = [0]

            class _DIXGymEnv(gymnasium.Env):  # type: ignore[misc]
                ...
                def step(  # type: ignore[override]
                    ...
                    dix_obs, reward, terminated, truncated, info = env.step(
            ...
            # ------------------------------------------------------------------
            # Actor / critic networks (2-hidden-layer MLP, tanh activations)
            # ------------------------------------------------------------------
            ...
            # ------------------------------------------------------------------
            # Policy construction: PPO (on-policy) vs DQN fallback (off-policy)
            # ------------------------------------------------------------------
            is_ppo = arguments.policy_kind == TianshouPolicyKind.PPO

            _ppo_policy_kwargs: dict = dict(
                actor=_ActorMLP().to(device),
                critic=_CriticMLP().to(device),
                dist_fn=torch.distributions.Categorical,
                discount_factor=arguments.gamma,
            )
            # action_space added in tianshou ~0.5; safe to pass, silently
            # ignored by older builds.
            try:
                _ppo_policy_kwargs["action_space"] = gym_env.action_space
                PPOPolicy(**_ppo_policy_kwargs, optim=torch.optim.Adam([], lr=1e-4))
            except TypeError:
                _ppo_policy_kwargs.pop("action_space", None)

            if is_ppo:
                actor_net = _ActorMLP().to(device)
                critic_net = _CriticMLP().to(device)
                ...
                if "action_space" in _ppo_policy_kwargs:
                ...
                try:
                    dqn_kw["action_space"] = gym_env.action_space
                    DQNPolicy(**dqn_kw)
                except TypeError:
            ...
            # ------------------------------------------------------------------
            # Training loop: collect → update → record
            # ------------------------------------------------------------------
            ...
            for _epoch in range(arguments.max_epoch):
                try:
                    stats = collector.collect(n_step=arguments.step_per_collect)
                    if isinstance(stats, dict):
                        ...
                        lens = list(stats.get("lens", []))
                    else:
                        n_st = int(
                            getattr(
                                stats,
                                "n_collected_steps",
                                arguments.step_per_collect,
                            )
                        )
                        rews = list(getattr(stats, "returns", getattr(stats, "rews", [])))
                        lens = list(getattr(stats, "lens", []))
                except Exception:
                    n_st = 0
                    rews = []
                    lens = []

                total_steps_done += n_st
                ep_rewards.extend(float(r) for r in rews)
                ep_lengths.extend(float(l) for l in lens)

                update_result: dict = {}
                if is_ppo:
                    for _ in range(arguments.repeat_per_collect):
                        if len(buf) >= arguments.batch_size:
                            ur = policy.update(arguments.batch_size, buf)  # type: ignore[union-attr]
                ...
                cl = float(
                    update_result.get(
                        "loss/vf",
                        update_result.get("vf", update_result.get("critic", 0.0)),
                    )
                )
            ...
            # ------------------------------------------------------------------
            # Serialize policy weights via caller-supplied artifact sink
            # ------------------------------------------------------------------
            artifact_buf = _io.BytesIO()
            torch.save(policy.state_dict(), artifact_buf)  # type: ignore[union-attr]
            ...
            return TianshouSandboxMetrics(
                epochs_completed=arguments.max_epoch,
                total_steps_executed=max(total_steps_done, 0),
