...
class FastRiskCache:
    ...
    def __init__(self, default_ttl_ns: int = 60_000_000_000):  # 60 seconds default
        ...
        self._stats = {"hits": 0, "misses": 0, "sets": 0, "invalidations": 0, "evictions": 0}
    ...
    def set(self, key: str, data: RiskData, ttl_ns: Optional[int] = None) -> None:
        ...
        entry = RiskCacheEntry(
            key=key, data=data, expiry_ns=expiry_ns, last_access_ns=datetime.now().timestamp_ns()
        )
    ...
    def cleanup_expired(self) -> int:
        ...
        expired_keys = [
            key for key, entry in self._cache.items() if entry.expiry_ns < current_time_ns
        ]
