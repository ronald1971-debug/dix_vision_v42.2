...
class OperatorAuthorityResponse(BaseModel):
    """Projection of OperatorAuthority for the dashboard."""
    ...
    granted_ts_ns: int
    notes: str
    trading_mode: dict[str, str]
    semi_auto_policy: dict[str, dict[str, Any]]


class OperatorAuthorityRequest(BaseModel):
    """Body for ``POST /api/operator/authority``."""

    model_config = ConfigDict(extra="forbid")

    learning: str = Field(..., min_length=1, max_length=16)
    practice: str = Field(..., min_length=1, max_length=16)
    live_execution: str = Field(..., min_length=1, max_length=16)
    operator_id: str = Field(..., min_length=1, max_length=64)
    notes: str = Field("", max_length=256)


class TradingModeRequest(BaseModel):
    """Body for ``POST /api/operator/trading_mode``."""

    model_config = ConfigDict(extra="forbid")

    domain: str = Field(..., min_length=1, max_length=32)
    mode: str = Field(..., min_length=1, max_length=16)
    requestor: str = Field("dashboard", min_length=1, max_length=64)


class SemiAutoPolicyRequest(BaseModel):
    """Body for ``POST /api/operator/semi_auto_policy``."""
...
class PromoteBuildoutParamsRequest(BaseModel):
    """Body for ``POST /api/operator/promote_buildout_params``."""
...
class ExternalSourceRequest(BaseModel):
    """Body for ``POST /api/operator/external_source``."""
...
class SemiAutoApprovalRequest(BaseModel):
    """Body for ``POST /api/operator/semi_auto_approve``."""
...
class SemiAutoRejectRequest(BaseModel):
    """Body for ``POST /api/operator/semi_auto_reject``."""
...
class ManualOrderRequest(BaseModel):
    """Body for ``POST /api/operator/manual_order``."""
...
class LearningProgressResponse(BaseModel):
    ...
    class_a_updates_applied: int = 0
    paper_fills_today: int = 0
    backtest_runs_today: int = 0
    live_fills_today: int = 0
    confidence_calibration_score: float = 0.0
    validated_hypothesis_count: int = 0
    adaptation_velocity: float = 0.0


class SemiAutoQueueResponse(BaseModel):
    """Pending semi-auto approvals."""
