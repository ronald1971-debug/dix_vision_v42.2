...
class StrategyArbiter:
    ...
    def __init__(self, arbiter_id: str = "default_arbiter"):
        ...
        self._conflict_stats = {
            ...
            "resolutions": {"priority": 0, "vote": 0, "confidence": 0, "cancel": 0, "combine": 0},
        }
    ...
    def set_strategy_weight(self, strategy_id: str, weight: float, priority: int = 100) -> None:
        """Set weight and priority for a strategy."""
        self._strategy_weights[strategy_id] = StrategyWeight(
            strategy_id=strategy_id, weight=max(0.0, min(1.0, weight)), priority=priority
        )
        logger.info(
            f"[STRATEGY_ARBITER] Set weight for {strategy_id}: {weight:.2f}, priority: {priority}"
        )
    ...
    def _resolve_conflict(
        ...
        hold_signals: List[Dict[str, Any]],
    ) -> ArbitrationResult:
    ...
    def _resolve_by_priority(self, signals: List[Dict[str, Any]]) -> ArbitrationResult:
        ...
        sorted_signals = sorted(
            signals,
            key=lambda s: self._strategy_weights.get(
                s["strategy_id"], StrategyWeight(s["strategy_id"], 0.5, 100)
            ).priority,
        )
    ...
    def _resolve_by_combine(self, signals: List[Dict[str, Any]]) -> ArbitrationResult:
        ...
        for signal in signals:
            ...
            weight = self._strategy_weights.get(
                strategy_id, StrategyWeight(strategy_id, 0.5, 100)
            ).weight
        ...
        buy_weight = sum(
            weights.get(s["strategy_id"], 0.0) for s in signals if s["signal_type"] == "BUY"
        )
        sell_weight = sum(
            weights.get(s["strategy_id"], 0.0) for s in signals if s["signal_type"] == "SELL"
        )
        hold_weight = sum(
            weights.get(s["strategy_id"], 0.0) for s in signals if s["signal_type"] == "HOLD"
        )
    ...
    def get_arbitration_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "conflict_rate": (
                self._conflict_stats["conflicts_detected"]
                / self._conflict_stats["total_arbitrations"]
                if self._conflict_stats["total_arbitrations"] > 0
                else 0.0
            ),
    ...
    def arbitrate_signals_with_world_context(
        self, signals: List[Dict[str, Any]], world_context: Optional[WorldContext] = None
    ) -> ArbitrationResult:
        """
    ...
    def _enhance_arbitration_with_world_context(
        self, result: ArbitrationResult, signals: List[Dict[str, Any]], world_context: WorldContext
    ) -> ArbitrationResult:
    ...
    def _should_override_for_regime(
        self, signals: List[Dict[str, Any]], world_context: WorldContext
    ) -> bool:
