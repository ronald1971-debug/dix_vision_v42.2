...
from mind.order_manager import Order, OrderStatus, Position
...
class PortfolioManager:
    ...
    async def update_position(
        self, portfolio_id: str, symbol: str, quantity_delta: float, price: float
    ) -> bool:
    ...
    async def update_order_status(
        self, portfolio_id: str, order_id: str, status: OrderStatus
    ) -> bool:
    ...
    async def calculate_portfolio_value(self, portfolio_id: str) -> Tuple[float, float]:
        ...
        position_value = sum(
            abs(pos.quantity * pos.average_entry_price) for pos in portfolio.positions.values()
        )
    ...
    async def _update_portfolio_metrics(self, portfolio_id: str):
        ...
        portfolio.metrics.unrealized_pnl = (
            total_value - portfolio.initial_capital - portfolio.metrics.realized_pnl
        )
        portfolio.metrics.total_pnl = (
            portfolio.metrics.unrealized_pnl + portfolio.metrics.realized_pnl
        )

        # Update risk metrics
        portfolio.risk_metrics.portfolio_value = total_value
        portfolio.risk_metrics.exposure = exposure
        portfolio.risk_metrics.current_drawdown = (
            (portfolio.initial_capital - total_value) / portfolio.initial_capital
    ...
    async def check_risk_limits(self, portfolio_id: str) -> Tuple[bool, List[str]]:
        ...
        if exposure > portfolio.max_total_exposure:
            violations.append(
                f"Exposure {exposure:.2f} exceeds maximum {portfolio.max_total_exposure}"
            )
        ...
        for symbol, position in portfolio.positions.items():
            ...
            if position_ratio > portfolio.max_position_size:
                violations.append(
                    f"Position {symbol} size {position_ratio:.2f} exceeds maximum {portfolio.max_position_size}"
                )

        # Check stop loss
        if portfolio.risk_metrics.current_drawdown > portfolio.stop_loss_threshold:
            violations.append(
                f"Current drawdown {portfolio.risk_metrics.current_drawdown:.2f} exceeds stop loss threshold {portfolio.stop_loss_threshold}"
            )
    ...
    async def create_portfolio_with_world_context(
        self,
        portfolio_id: str,
        name: str,
        initial_capital: float = 100000.0,
    ...
    def _adjust_portfolio_params_with_world_context(
        self, initial_capital: float, risk_level: RiskLevel, world_context: Optional[WorldContext]
    ) -> Dict[str, Any]:
        """Adjust portfolio parameters based on world context."""
        adjustments = []
        adjusted_params = {
