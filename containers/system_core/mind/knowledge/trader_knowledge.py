...
class TraderKnowledge:
    ...
    def add_expertise(self, expertise: TradingExpertise) -> None:
        ...
        logger.info(
            f"[TRADER_KNOWLEDGE] Added expertise in {expertise.domain} (proficiency: {expertise.proficiency_level:.2f})"
        )
    ...
    def record_behavioral_pattern(self, pattern: BehavioralPattern) -> None:
        ...
        logger.info(
            f"[TRADER_KNOWLEDGE] Recorded pattern {pattern.pattern_type} (confidence: {pattern.confidence:.2f})"
        )
    ...
    def form_belief(self, belief: TradingBelief) -> None:
        ...
        logger.info(
            f"[TRADER_KNOWLEDGE] Formed belief: {belief.belief_statement[:50]}... (confidence: {belief.confidence:.2f})"
        )

    def update_belief_confidence(
        self, belief_id: str, evidence: str, supporting: bool, effectiveness: float = 0.0
    ) -> None:
        ...
        belief.last_updated = datetime.now()
        logger.debug(
            f"[TRADER_KNOWLEDGE] Updated belief {belief_id} confidence: {belief.confidence:.2f}"
        )
    ...
    def recommend_regime_strategy(self, current_regime: MarketRegime) -> Dict[str, Any]:
        ...
        return {
            ...
            "best_alternative_regimes": [
                r.value for r, p in best_regimes if p > regime_performance
            ],
            ...
            "confidence": (
                min(1.0, abs(regime_performance) * 10) if regime_performance != 0 else 0.5
            ),
        }
