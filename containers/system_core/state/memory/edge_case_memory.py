...
class EdgeCaseMemory:
    ...
    def _is_unusual_value(self, key: str, value: str) -> bool:
        ...
        try:
            ...
            # Not a numeric value
            LOG.debug(f"Value '{value}' is not numeric, skipping numeric edge case check")
            LOG.debug("Will check for other edge case patterns")
    ...
    def _is_unusual_system_state(self, key: str, value: str) -> bool:
        ...
        if any(resource in key.lower() for resource in resource_keys):
            try:
                ...
                LOG.debug(f"Failed to parse resource value '{value}': {e}")
                LOG.debug("Will check for other resource exhaustion patterns")
        ...
        if any(perf in key.lower() for perf in performance_keys):
            try:
                ...
                LOG.debug(f"Failed to parse performance value '{value}': {e}")
                LOG.debug("Will check for other performance anomaly patterns")
