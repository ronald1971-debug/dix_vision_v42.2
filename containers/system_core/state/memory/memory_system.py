...
class MemorySystem:
    ...
    def set(
        ...
        with self._lock:
            ...
            if self._stats.total_cache_entries > 0:
                self._stats.compression_ratio = self._stats.compression_savings_bytes / (self._memory_used_bytes + self._stats.compression_savings_bytes)
        
        return True
    
    def _adjust_ttl(self, ttl: Optional[float], world_context: Optional[WorldContext]) -> Optional[float]:
    ...
    def _evict_entries(self, required_bytes: int, world_context: Optional[WorldContext]) -> None:
        ...
        if self._cache_policy == CachePolicy.LRU:
            ...
            evicted_count = 0
            while self._memory_used_bytes + required_bytes > self._max_memory_bytes * 0.9 and self._access_order:
            ...
            while self._memory_used_bytes + required_bytes > self._max_memory_bytes * 0.9 and self._cache:
                # Find least frequently used key
                key_to_evict = min(self._cache.keys(), key=lambda k: self._access_frequency.get(k, 0))
                if key_to_evict in self._cache:
                    entry = self._cache.pop(key_to_evict)
                    self._memory_used_bytes -= entry.size_bytes
            ...
            if world_context and world_context.volatility_regime == "high":
                ...
                entries_to_evict = sorted(
                    self._cache.items(),
                    key=lambda x: (0 if x[1].world_context is None else 
                                    1 if x[1].world_context.volatility_regime == "low" else 0,
                                    x[1].last_access.timestamp())
                )
            else:
                # Default to LRU
                entries_to_evict = sorted(self._cache.items(), key=lambda x: x[1].last_access.timestamp())
