...
from state.memory.contracts import MemoryKind, MemoryQuery, MemoryRecord, MemorySearchResult
    ...
    def _route_to_domain(self, rec: MemoryRecord) -> None:
        try:
            if rec.kind == MemoryKind.STRATEGY and self._strategy:
                ...
                _logger.debug(f"Strategy memory record routed to timeline (no domain-specific routing needed)")
            elif rec.kind == MemoryKind.TRADER and self._trader:
                _logger.debug(f"Trader memory record routed to timeline (no domain-specific routing needed)")
            elif rec.kind == MemoryKind.GOVERNANCE and self._governance:
                _logger.debug(f"Governance memory record routed to timeline (no domain-specific routing needed)")
            elif rec.kind == MemoryKind.RUNTIME and self._runtime_ev:
                _logger.debug(f"Runtime memory record routed to timeline (no domain-specific routing needed)")
        except Exception as exc:
