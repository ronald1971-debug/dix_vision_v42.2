    ...
    def chain_stats(self) -> dict[str, Any]:
        ...
        if self._authority_db and self._authority_db.exists():
            try:
                ...
                LOG.debug(f"Failed to query authority ledger count: {e}")
                LOG.debug("Will use default authority count of 0")
        if self._event_db and self._event_db.exists():
            try:
                ...
                LOG.debug(f"Failed to query event ledger count: {e}")
                LOG.debug("Will use default event count of 0")
        return {
            "authority_chain": {
