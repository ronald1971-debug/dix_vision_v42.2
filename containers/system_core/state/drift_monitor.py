...
class DriftMonitor:
    ...
    def record_metric(self, metric_name: str, knowledge_id: str, value: float) -> DriftMetric:
        ...
        with self._lock:
            ...
            logger.debug(f"[DRIFT_MONITOR] Recorded metric {metric_name} for {knowledge_id}: {value:.3f} (drift: {drift_score:.3f})")
            
            return metric
    
    def establish_baseline(self, knowledge_id: str, metrics: Dict[str, float], valid_days: int = 30) -> DriftBaseline:
        """
        ...
        with self._lock:
            ...
            logger.info(f"[DRIFT_MONITOR] Established baseline for {knowledge_id} with {len(metrics)} metrics, valid until {valid_until}")
    ...
    def _check_drift_alerts(self, metric: DriftMetric) -> None:
        ...
        if metric.drift_score > self._thresholds["temporal_decay"]:
            alerts.append(DriftAlert(
                ...
                suggested_actions=["Refresh knowledge source", "Verify if knowledge is still relevant", "Update knowledge if possible"],
        ...
        if self._check_distributional_drift(metric):
            alerts.append(DriftAlert(
                ...
                affected_metrics=[metric],
                suggested_actions=["Recollect statistical baseline", "Analyze distribution shift causes", "Update models if needed"],
                triggered_at=str(datetime.utcnow())
            ))
        ...
        for alert in alerts:
            ...
            logger.warning(f"[DRIFT_MONITOR] Drift alert triggered: {alert.drift_type.value} for {metric.knowledge_id} - {alert.description}")
    ...
    def check_consistency_with_world_model(
        ...
        with self._lock:
            ...
            if "market_state" in world_model_state:
                ...
                consistency_issues += self._check_market_state_consistency(knowledge_id, market_state)
                total_checks += 1
            ...
            if len(self._consistency_scores[knowledge_id]) > 5:
                ...
                if recent_consistency < avg_consistency - self._thresholds["consistency_loss"]:
                    alerts.append(DriftAlert(
                        ...
                        affected_metrics=[],
                        suggested_actions=["Review knowledge for conflicts with current world state", "Update knowledge if inconsistencies found", "Mark knowledge for review"],
                        triggered_at=str(datetime.utcnow())
                    ))
            
            return consistency_score, alerts
    
    def _check_market_state_consistency(self, knowledge_id: str, market_state: Dict[str, Any]) -> int:
        """Check consistency with market state."""
        issues = 0
    ...
    def get_active_alerts(self, knowledge_id: Optional[str] = None) -> List[DriftAlert]:
        ...
        with self._lock:
            if knowledge_id:
                return [alert for alert in self._active_alerts if alert.knowledge_id == knowledge_id]
    ...
    def get_knowledge_drift_report(self, knowledge_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "last_updated": self._knowledge_timestamps.get(knowledge_id).isoformat() if knowledge_id in self._knowledge_timestamps else None
            }
