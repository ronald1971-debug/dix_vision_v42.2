...
class SourceConflictGraph:
    ...
    def __init__(self):
        ...
        self._edges: Dict[str, List[ConflictEdge]] = defaultdict(
            list
        )  # source_id -> List[ConflictEdge]
        self._conflict_groups: Dict[str, Set[str]] = defaultdict(
            set
        )  # group_id -> set of statement_ids
    ...
    def add_source_statement(
        ...
        with self._lock:
            ...
            logger.info(
                f"[SOURCE_CONFLICT_GRAPH] Added statement {statement_id} from {source_id}, detected {len(conflicts)} conflicts"
            )
    ...
    def _check_temporal_conflict(self, node1: ConflictNode, node2: ConflictNode) -> bool:
        ...
        try:
            ...
            LOG.debug(f"Timestamp parsing error in conflict detection: {e}")
            LOG.debug("Will use other conflict detection methods")
    ...
    def resolve_conflict(
        self, statement_id: str, resolution_strategy: str = "confidence"
        ...
        with self._lock:
            if statement_id not in self._edges:
                logger.warning(
                    f"[SOURCE_CONFLICT_GRAPH] No conflicts found for statement {statement_id}"
                )
                return None
            ...
            if resolution_strategy == "confidence":
                ...
                logger.error(
                    f"[SOURCE_CONFLICT_GRAPH] Unknown resolution strategy: {resolution_strategy}"
                )
                return None
            ...
            if resolution:
                ...
                logger.info(
                    f"[SOURCE_CONFLICT_GRAPH] Resolved {len(conflicts)} conflicts for {statement_id} using {resolution_strategy}"
                )
    ...
    def update_source_trust(self, source_id: str, trust_score: float) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"[SOURCE_CONFLICT_GRAPH] Updated source trust: {source_id} -> {trust_score}"
            )
