...
class ReplayValidator:
    ...
    def replay_events(
        ...
        for event in events:
            try:
                ...
                self._replay_event_with_world_context(
                    event, None, world_context, validation_strictness
                )
                successful_events += 1
            except Exception as e:
                failed_events += 1
        ...
        base_consistency_score = successful_events / total_events if total_events > 0 else 1.0
    ...
    def replay_events_with_historical_world_context(
        ...
        if historical_timestamp_ns:
            ...
            if events and hasattr(events[0], "timestamp"):
                world_context = self._get_historical_world_context(
                    getattr(events[0], "timestamp", self._get_timestamp())
                )
            else:
                world_context = self._get_world_context()
    ...
    def predict_replay_outcome(
        ...
        if not world_context:
            ...
            return {"success_probability": 0.95, "expected_consistency": 0.95, "confidence": 0.5}
        ...
        with self._lock:
            stats = {
                ...
                "successful_replays": sum(
                    1 for r in self._replay_history.values() if r.status == ReplayStatus.SUCCESS
                ),
                ...
                "world_aware_replays": sum(
                    1 for r in self._replay_history.values() if r.world_context_aware
                ),
            }
    ...
    def _has_prohibited_transitions(
        ...
        prohibited_transitions = {
            ...
            (
                "halted",
                "trading",
            ): True,  # Cannot go from halted to trading without explicit approval
    ...
    def _replay_event_with_world_context(
        ...
        try:
            ...
            if validation_strictness == "relaxed":
                event_id = getattr(event, "record_id", getattr(event, "id", "unknown"))
                _logger.warning(
                    f"[REPLAY] Event {event_id} validation failed but ignored due to relaxed strictness: {e}"
                )
            else:
                raise
    ...
    def _validate_event_with_world_context(
        ...
        if validation_strictness == "relaxed":
            ...
            if event_type in ["trade", "market_update"]:
                _logger.debug(
                    f"[REPLAY] Relaxed validation for event {event_id} in relaxed strictness mode"
                )
                return
        ...
        if world_context.volatility_regime == "high":
            ...
            if event_type == "trade":
                _logger.debug(
                    f"[REPLAY] Allowing trade variance in high volatility for event {event_id}"
                )
    ...
    def _validate_transition_with_world_context(
        ...
        if world_context.volatility_regime == "high":
            ...
            if "trade" in event_type.lower() or "market" in event_type.lower():
                _logger.debug(
                    f"[REPLAY] Relaxed transition validation for {event_type} in high volatility"
                )
                return True
