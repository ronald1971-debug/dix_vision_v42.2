...
from governance_unified.control_plane.promotion_gates import PromotionGates
...
class StateTransitionManager:
    ...
    def __init__(
        ...
        *,
        ...
        readiness_validator: Any | None = None,
        on_mode_change: Callable[[SystemMode], None] | None = None,
    ) -> None:
        self._policy = policy
        self._ledger = ledger
        ...
        # Operational readiness gate — blocks CANARY/LIVE/AUTO
        # transitions unless readiness validator confirms the
        # required level. Optional; when None the gate is skipped.
        self._readiness_validator = readiness_validator
        # Callback for notifying the SystemKernel of mode changes
    ...
    def propose(self, request: ModeTransitionRequest) -> ModeTransitionDecision:
        ...
        with self._lock:
            ...
            # Operational readiness gate — blocks CANARY/LIVE/AUTO unless
            # the runtime readiness validator confirms the system meets
            # minimum operational requirements for that mode.
            _READINESS_REQUIRED_MODES = {
                SystemMode.CANARY: "CANARY_READY",
                SystemMode.LIVE: "LIVE_READY",
                SystemMode.AUTO: "LIVE_READY",
            }
            required_level = _READINESS_REQUIRED_MODES.get(normalised.target_mode)
            if required_level and self._readiness_validator is not None:
                try:
                    report = self._readiness_validator.assess()
                    if report.level.value < required_level:
                        rejection_payload = {
                            "requestor": normalised.requestor,
                            "prev_mode": prev.name,
                            "target_mode": normalised.target_mode.name,
                            ...
                            "current_level": str(report.level),
            ...
            if self._on_mode_change is not None:
                self._on_mode_change(normalised.target_mode)
