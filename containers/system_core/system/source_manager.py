...
class SourceManager:
    ...
    def _add_remaining_sources(self) -> None:
        ...
        crypto_sources = [
            "SRC-CRYPTO-COINMARKETCAP-001",
            "SRC-CRYPTO-KRAKEN-001",
            "SRC-CRYPTO-COINBASE-001",
            "SRC-CRYPTO-COINPAPRIKA-001",
            "SRC-CRYPTO-CRYPTOCOMPARE-001",
            "SRC-CRYPTO-COINCAP-001",
            "SRC-CRYPTO-NOMICS-001",
            "SRC-CRYPTO-GLASSNODE-001",
        ...
        for source_id in self._sources:
            self._health[source_id] = SourceHealth(
                ...
                status=(
                    ...
                    if self._sources[source_id].enabled
    ...
    def enable_source(self, source_id: str, agent: str | None = None) -> bool:
        ...
        with self._lock:
            ...
            if agent:
                ...
                self._sources[source_id] = SourceConfig(**{**source.__dict__, "enabled": True})
                self._health[source_id] = SourceHealth(
    ...
    def disable_source(self, source_id: str, agent: str | None = None) -> bool:
        ...
        with self._lock:
            ...
            if agent:
                ...
                # Disable globally
                source = self._sources[source_id]
                self._sources[source_id] = SourceConfig(**{**source.__dict__, "enabled": False})
                self._health[source_id] = SourceHealth(
            ...
            for source_id, health in self._health.items():
                if health.status == SourceStatus.ENABLED:
                    ...
                    if health.failure_count > 0:
                        failure_rate = health.failure_count / (
                        ...
                        if failure_rate > 0.5:  # More than 50% failures
                            ...
                            LOG.warning(
                                f"Auto-disabled {source_id} due to high failure rate: {failure_rate:.2%}"
                            )
