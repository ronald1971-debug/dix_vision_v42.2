...
class ComponentConnectionManager:
    ...
    def connect_component(
        ...
        try:
            ...
            logger.info(
                f"[CONNECTION_MANAGER] Component {component_name} connected successfully"
            )
            ...
            for cb in self._on_connect_callbacks.get(component_name, []):
                try:
                    ...
                    logger.error(f"[CONNECTION_MANAGER] On-connect callback failed for {component_name}: {e}")
        ...
        try:
            ...
            for cb in self._on_disconnect_callbacks.get(component_name, []):
                try:
                    ...
                    logger.error(f"[CONNECTION_MANAGER] On-disconnect callback failed for {component_name}: {e}")
            
            return True
            
        except Exception as e:
            logger.error(f"[CONNECTION_MANAGER] Failed to disconnect component {component_name}: {e}")
            return False
        ...
        def health_check_loop():
            while not self._stop_event.is_set():
                try:
                    ...
                    logger.error(f"[CONNECTION_MANAGER] Health check error for {component_name}: {e}")
            ...
            if component:
                ...
                if healthy:
                    if status.state == ConnectionState.DEGRADED:
                        ...
                        logger.info(f"[CONNECTION_MANAGER] Component {component_name} recovered from degraded state")
                else:
        ...
        try:
            ...
            for cb in self._on_failure_callbacks.get(component_name, []):
                try:
                    ...
                    logger.error(f"[CONNECTION_MANAGER] On-failure callback failed for {component_name}: {e}")
        ...
        try:
            ...
            for cb in self._on_degrade_callbacks.get(component_name, []):
                try:
                    ...
                    logger.error(f"[CONNECTION_MANAGER] On-degrade callback failed for {component_name}: {e}")
