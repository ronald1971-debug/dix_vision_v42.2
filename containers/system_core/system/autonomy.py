...
_DEFAULT_BUDGETS: dict[AutonomyMode, AutonomyBudget] = {
    ...
    AutonomyMode.FULL_AUTO: AutonomyBudget(float("inf"), 1_000_000, True),
}
...
class AutonomyManager:
    ...
    # ---- write paths -------------------------------------------------
    def transition(
        self, new_mode: AutonomyMode, *, operator_id: str = "operator", reason: str = ""
    ) -> AutonomyStatus:
        ...
        try:
            ...
            LOG.warning(f"Failed to log autonomy override to event bus: {e}")
            LOG.debug("Will continue with autonomy snapshot")
        return snapshot

    def set_budget(
        self, mode: AutonomyMode, budget: AutonomyBudget, *, operator_id: str = "operator"
    ) -> None:
        with self._lock:
        ...
        try:
            ...
            LOG.warning(f"Failed to log autonomy budget change to event bus: {e}")
            LOG.debug("Will continue with budget setting")

    # ---- fast-path gate ----------------------------------------------
    def allows_auto(self, *, size_usd: float, asset_known: bool = True) -> tuple[bool, str]:
        """Return (ok, reason).
