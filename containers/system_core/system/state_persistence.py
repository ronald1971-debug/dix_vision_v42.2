    ...
    def save(self, state_dict: dict[str, Any]) -> None:
        with self._lock:
            try:
                ...
                LOG.warning(f"Failed to save state to {self._path}: {e}")
                LOG.debug("State persistence save failed, will continue with in-memory state")
    ...
    def clear(self) -> None:
        with self._lock:
            try:
                ...
                LOG.warning(f"Failed to clear state file {self._path}: {e}")
                LOG.debug("State persistence clear failed, will continue")
