...
class Scheduler:
    ...
    def tick(self, now_ns: int) -> list[str]:
        ...
        for task in due:
            try:
                task.fn()
            except Exception as e:  # noqa: BLE001 - never abort the tick loop
                LOG.warning(f"Task {task.name} failed to execute: {e}")
                LOG.debug("Will continue with other scheduled tasks")
            with self._lock:
