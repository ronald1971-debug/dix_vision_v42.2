...
class CapacityPlanningEngine:
    ...
    # Capacity plans
    _capacity_plans: dict[str, CapacityPlan] = field(
        default_factory=dict, init=False, repr=False
    )
    ...
    def forecast_load(
        ...
        historical = tuple(
            u.current_usage / u.max_capacity if u.max_capacity > 0 else 0.0
            for u in history[-64:]
        )
    ...
    def _fallback_forecast(
        ...
        vol = sqrt(sum((x - sum(historical) / len(historical)) ** 2 for x in historical) / len(historical))
        confidence_interval = (max(0.0, historical[-1] - 2 * vol), min(1.0, historical[-1] + 2 * vol))
    ...
    def generate_scaling_recommendations(
        ...
        # Calculate impacts
        cost_impact = self._calculate_cost_impact(action, recommended_capacity, current_usage.max_capacity)
    ...
    def _determine_scaling_action(
        ...
        if predicted_peak > self.scaling_threshold:
            ...
            if predicted_peak > 0.9:
                return (ScalingAction.SCALE_UP, f"Predicted peak usage {predicted_peak:.1%} exceeds critical threshold", required_capacity)
            else:
                return (ScalingAction.SCALE_OUT, f"Predicted peak usage {predicted_peak:.1%} exceeds threshold", required_capacity)
        ...
        if current_utilization < 0.3 and predicted_peak < 0.4:
            ...
            return (ScalingAction.SCALE_DOWN, f"Low utilization {current_utilization:.1%} with slack capacity", required_capacity)
        ...
        if 0.5 <= current_utilization <= 0.7 and predicted_peak <= 0.8:
            ...
            return (ScalingAction.OPTIMIZE, f"Moderate utilization with optimization opportunity", required_capacity)
    ...
    def identify_optimization_opportunities(
        ...
        for resource_type in ResourceType:
            ...
            if efficiency < 0.5:
                ...
                opportunities.append((component, f"optimize_{resource_type.value}", potential_savings))
