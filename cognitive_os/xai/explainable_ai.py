...
class ExplainableAI:
    ...
    def explain_decision(self, decision: Dict[str, Any], context: Dict[str, Any], 
                        ...
                        detail_level: ExplanationDetail = ExplanationDetail.DETAILED) -> DecisionExplanation:
        ...
        # Generate rationale
        rationale = self._explanation_formatter.format_rationale(decision, feature_importances, detail_level)
    ...
    def generate_counterfactual(self, decision: Dict[str, Any], context: Dict[str, Any], 
                              target_outcome: str) -> CounterfactualExplanation:
        """Generate counterfactual explanation."""
        logger.info(f"[XAI] Generating counterfactual explanation for target outcome: {target_outcome}")
        
        counterfactual = self._counterfactual_generator.generate(
            decision, context, target_outcome
        )
    ...
    def analyze_attention(self, decision: Dict[str, Any], context: Dict[str, Any]) -> AttentionVisualization:
    ...
    def get_explanation_history(self, decision_type: Optional[str] = None, 
                              limit: int = 10) -> List[DecisionExplanation]:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "explanations_by_type": {dt: len(exps) for dt, exps in self._decision_patterns.items()},
                "average_explanation_confidence": np.mean([exp.confidence for exp in self._explanation_history]) if self._explanation_history else 0.0
            }

    def _generate_primary_reason(self, decision: Dict[str, Any], 
                                feature_importances: List[FeatureImportance]) -> str:
        """Generate primary reason for decision."""
        if not feature_importances:
            return "No clear primary reason identified."
        
        # Get most important features
        top_features = sorted(feature_importances, key=lambda x: abs(x.importance_score), reverse=True)[:3]
    ...
    def _generate_alternative_decisions(self, decision: Dict[str, Any], 
        ...
        if decision_type == "trading":
            alternatives = [
                ...
                {"decision_type": "reverse_position", "reason": "Contrarian approach based on market signals"}
            ]
        
        return alternatives

    def _generate_supporting_evidence(self, decision: Dict[str, Any], 
    ...
    def _generate_potential_concerns(self, decision: Dict[str, Any], 
                                   context: Dict[str, Any]) -> List[str]:
        """Generate potential concerns."""
        concerns = []
...
class FeatureExplainer:
    ...
    def explain_features(self, decision: Dict[str, Any], context: Dict[str, Any]) -> List[FeatureImportance]:
    ...
    def _extract_features(self, decision: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _calculate_importance(self, feature_name: str, value: float, 
                           decision: Dict[str, Any], context: Dict[str, Any]) -> float:
    ...
    def _generate_feature_explanation(self, feature_name: str, importance: float, 
                                    direction: str) -> str:
        ...
        return explanations.get(feature_name, f"Feature {feature_name} {direction_text} decision {direction_text}")


class CounterfactualGenerator:
    """Generate counterfactual explanations."""
    
    def generate(self, decision: Dict[str, Any], context: Dict[str, Any], 
                target_outcome: str) -> CounterfactualExplanation:
        """Generate counterfactual explanation."""
    ...
    def _generate_required_changes(self, decision: Dict[str, Any], context: Dict[str, Any], 
...
class ExplanationFormatter:
    ...
    def format_rationale(self, decision: Dict[str, Any], 
                       feature_importances: List[FeatureImportance],
                       detail_level: ExplanationDetail) -> str:
    ...
    def _detailed_rationale(self, feature_importances: List[FeatureImportance]) -> str:
        ...
        reasons = []
        for feature in top_features:
            reasons.append(f"{feature.feature_name} {feature.direction} decision ({feature.importance_score:.2f} importance)")
        
        return "Decision based on: " + ", ".join(reasons)
    
    def _technical_rationale(self, decision: Dict, feature_importances: List[FeatureImportance]) -> str:
        ...
        for feature in feature_importances[:5]:
            rationale_parts.append(f"  {feature.feature_name}: {feature.importance_score:.3f} ({feature.direction})")
        
        return "\n".join(rationale_parts)
    
    def _comprehensive_rationale(self, decision: Dict, feature_importances: List[FeatureImportance]) -> str:
