...
class AgentCommunication:
    ...
    def send_message(self, message: AgentMessage) -> None:
        ...
        with self._lock:
            ...
            logger.debug(f"[AGENT_COMMUNICATION] Message sent: {message.sender_id} -> {message.receiver_id}")
...
class AgentOrchestrator:
    ...
    def _decompose_task(self, task: Task) -> List[str]:
        ...
        if len(task.required_capabilities) > 3:
            ...
            return [f"{task.task_id}_subtask_{i}" for i in range(len(task.required_capabilities) // 2 + 1)]
        return [task.task_id]
