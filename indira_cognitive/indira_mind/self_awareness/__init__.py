...
class RiskSelfAwareness:
    ...
    def is_within_risk_tolerance(self) -> bool:
        return (
            self.current_position_risk <= self.max_position_risk_allowed and
            self.current_portfolio_risk <= self.max_portfolio_risk_allowed
        )
...
class INDIRASelfAwarenessInterface(ABC):
    ...
    @abstractmethod
    def calibrate_self_assessment(self, actual_outcome: float, self_assessed_outcome: float) -> float:
...
{', '.join(awareness.performance_awareness.self_generated_recommendations[:2]) if awareness.performance_awareness else 'None'}
    ...
    def calibrate_self_assessment(self, actual_outcome: float, self_assessed_outcome: float) -> float:
        ...
        if error > 0.3:
            ...
            self._self_awareness_state.overall_confidence = max(0.0, self._self_awareness_state.overall_confidence - 0.1)
        elif error < 0.1:
            # Increase confidence in self-assessment
            self._self_awareness_state.overall_confidence = min(1.0, self._self_awareness_state.overall_confidence + 0.05)
