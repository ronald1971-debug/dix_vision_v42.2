...
from system_unified_unified_unified_unified.fast_risk_cache import get_risk_cache
from system_unified_unified_unified_unified.logger import get_logger
from system_unified_unified_unified_unified.state import get_state_manager
...
class GovernanceDecision:
    ...
    def __post_init__(self) -> None:
        self.allowed = self.outcome in {GovernanceOutcome.APPROVED, GovernanceOutcome.MODIFIED}
...
class GovernanceKernel:
    ...
    def _evaluate_market(self, request: ActionRequest) -> GovernanceDecision:
        ...
        ok, reason = constraints.allows_trade(
            ...
            portfolio_usd=float(
                kwargs.get("portfolio_usd", payload.get("portfolio_usd", 100_000.0))
            ),
        )
        if not ok:
            ...
            raw_size = kwargs.get("trade_size_pct", payload.get("trade_size_pct", None))
    ...
    def _evaluate_system(self, request: ActionRequest) -> GovernanceDecision:
        ...
        if action not in allowed_system_actions:
            return GovernanceDecision(GovernanceOutcome.REJECTED, f"unknown_system_action:{action}")
    ...
    def evaluate_boot(self, state: Any) -> GovernanceDecision:
        ...
        _mode = getattr(state, "mode", "INIT")  # noqa: F841
        if health < 0.3:
            return GovernanceDecision(GovernanceOutcome.REJECTED, f"health_too_low:{health:.2f}")
