...
#: Composite threshold alias (same semantics, used by the extended oracle).
DEFAULT_COMPOSITE_THRESHOLD: float = DEFAULT_DOWNGRADE_THRESHOLD

#: Default axis weights — equal weighting across the four canonical axes.
...
#: Minimum number of samples before the oracle considers a drift reading valid.
DEFAULT_MIN_SAMPLES: int = 30

#: Sliding window size (number of ticks) for EWMA drift computation.
DEFAULT_WINDOW_SIZE: int = 100

#: Ledger event kind for drift observations.
LEDGER_KIND_DRIFT_OBSERVATION: str = "GOV/DRIFT_OBSERVATION"
...
# Alias for the extended oracle interface expected by __init__.py
DriftOracle = DriftCompositeOracle
