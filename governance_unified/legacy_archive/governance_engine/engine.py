...
from governance_unified.control_plane import (
    ComplianceValidator,
    EventClassifier,
...
from governance_unified.control_plane.decision_signer import DecisionSigner
from governance_unified.control_plane.drift_oracle import DriftCompositeOracle
from governance_unified.control_plane.event_classifier import PipelineStage
from governance_unified.control_plane.exposure_store import ExposureStore
from governance_unified.control_plane.policy_engine import install_policy_table
from governance_unified.control_plane.risk_evaluator import ExposureBook
from governance_unified.control_plane.update_applier import UpdateApplier
from governance_unified.control_plane.update_validator import (
    ProposedUpdate,
    UpdateValidator,
    UpdateVerdict,
)
    ...
    def __init__(
        ...
        *,
        ...
        self.state_transitions = StateTransitionManager(
            ...
            on_mode_change=self._transition_mode_to_kernel,
        )
        ...
        self._kernel = kernel  # type: ignore[assignment]

    def _transition_mode_to_kernel(self, new_mode: SystemMode) -> None:
        """Sync mode change to SystemKernel if we have one."""
        if self._kernel is not None:
            self._kernel.transition_mode(new_mode, reason="fsm_transition")
