...
GOVERNANCE_CHARTER = Charter(
    ...
    how=[
        ...
        "Adaptively re-derive risk constraints from rolling-window outcomes via governance.self_optimizer \u2192 write to state.fast_risk_cache.",  # noqa: E501
        "Gate every strategy promotion (7d realized-PnL positive + outperforms shadow) and every DYON patch (authority_lint + policy + 24h shadow).",  # noqa: E501
    ...
    accountability=[
        "GOVERNANCE/MODE_TRANSITION",
        "GOVERNANCE/CONSTRAINT_SET_UPDATE",
        "GOVERNANCE/STRATEGY_APPROVED",
        "GOVERNANCE/STRATEGY_REJECTED",
        "GOVERNANCE/PATCH_APPROVED",
        "GOVERNANCE/PATCH_REJECTED",
        "GOVERNANCE/KILL_SWITCH_ARMED",
        "GOVERNANCE/KILL_SWITCH_DISARMED",
    ],
    tools=[
        "governance.kernel",
        "governance.self_optimizer",
        "state.ledger (CONTROL stream)",
        "state.fast_risk_cache (writer)",
        "security.authentication (cockpit auth)",
        "tools.authority_lint (CI)",
    ],
)
