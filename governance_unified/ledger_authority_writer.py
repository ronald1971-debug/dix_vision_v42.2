...
DUAL-LEDGER NOTE
----------------
This is the *authority ledger* — one of two complementary hash-chained
SQLite ledgers in DIX VISION:

  1. **This file** (authority ledger) — governance authority decisions
     only (mode transitions, strategy lifecycle, HMAC-signed operator
     approvals, policy hash anchors). Written exclusively by
     ``GovernanceEngine`` (GOV-CP-05). Low frequency.

  2. **state/ledger/event_store.py** (event store) — all runtime
     events (every tick, signal, hazard, wallet cap change). Written
     via ``state/ledger/writer.get_writer()``. High frequency.

These two chains are NOT duplicates. A unified read-only query surface
that merges both chains lives in ``state/ledger/bridge.LedgerBridge``.
``state/ledger/reader.LedgerReader`` exposes the authority surface via
``authority_entries()``; ``bridge.py`` adds the event-store side.
"""
...
#   ts_ns       INTEGER NOT NULL     — wall-ns from system_unified_unified_unified_unified.time_source
