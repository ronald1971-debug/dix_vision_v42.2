...
class Checkpoint:
    ...
    def __post_init__(self):
        ...
        state_hash = hashlib.sha256(json.dumps(self.state_data, sort_keys=True).encode()).hexdigest()
...
class CheckpointManager:
    ...
    def __init__(self, checkpoint_dir: str = "checkpoints", max_checkpoints: int = 10):
        ...
        logger.info(f"[CHECKPOINT_MANAGER] Initialized with {len(self._checkpoints)} existing checkpoints")
    ...
    def restore_checkpoint(self, checkpoint_id: str) -> CheckpointRestoreResult:
        ...
        with self._lock:
            ...
            if not verification_passed:
                logger.error(f"[CHECKPOINT_MANAGER] Checkpoint {checkpoint_id} checksum verification failed")
                return CheckpointRestoreResult(
                    success=False,
                    checkpoint_id=checkpoint_id,
            ...
            logger.info(f"[CHECKPOINT_MANAGER] Restored checkpoint {checkpoint_id} in {restoration_time_ms:.2f}ms")
    ...
    def _persist_checkpoint(self, checkpoint: Checkpoint) -> None:
        ...
        try:
            ...
            logger.error(f"[CHECKPOINT_MANAGER] Failed to persist checkpoint {checkpoint.checkpoint_id}: {e}")
    ...
    def get_checkpoint_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            for checkpoint in self._checkpoints.values():
                component_counts[checkpoint.component] = component_counts.get(checkpoint.component, 0) + 1
            ...
            return {
                ...
                "oldest_checkpoint": min(cp.timestamp for cp in self._checkpoints.values()) if self._checkpoints else None,
                "newest_checkpoint": max(cp.timestamp for cp in self._checkpoints.values()) if self._checkpoints else None
            }
...
def get_checkpoint_manager(checkpoint_dir: str = "checkpoints", max_checkpoints: int = 10) -> CheckpointManager:
