...
from execution_unified.core.events import (
    ExecutionEvent,
    ExecutionStatus,
    SignalEvent,
)
...
class LiveAdapterBase:
    ...
    def halt(self, reason: str, *, canonical_halted: bool | None = None) -> None:
        if canonical_halted is not None:
            self.reconcile_adapter_halt(canonical_halted)
        else:
            self._state = AdapterState.HALTED
            self._detail = f"halted: {reason}"

    def reconcile_adapter_halt(self, canonical_halted: bool) -> bool:
        """P2 authority reconciliation.

        Reconcile adapter-local halted state with the canonical system-mode
        halt. When ``canonical_halted`` is True the system is in emergency
        halt (``ModeManager.halt`` / kill switch); we mirror that into the
        adapter state so adapter health reports are consistent. Returns
        True if a state change was made, False if already consistent.
        """
        if canonical_halted and self._state is not AdapterState.HALTED:
            self._state = AdapterState.HALTED
            self._detail = "halted: canonical system halt"
            return True
        if not canonical_halted and self._state is AdapterState.HALTED:
            self._state = AdapterState.DISCONNECTED
            self._detail = "resumed: canonical system halt released"
            return True
        return False
...
from execution_unified.core.adapters.base import BrokerAdapter as _BrokerAdapter  # noqa: E402
