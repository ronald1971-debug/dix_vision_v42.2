...
from system_unified_unified_unified_unified_engine.authority.matrix import (
    AuthorityMatrix,
...
# ---------------------------------------------------------------------------
# BUILD-DIRECTIVE extension — OperatorAuthority-aware execution routing.
# ---------------------------------------------------------------------------


class ExecutionRouteDecision:
    """Result of operator-authority routing decision.

    Determines whether an intent should be:
    - BLOCKED (LiveExecution=BLOCKED)
    - PAPER (Practice=ON, LiveExecution=BLOCKED)
    - SEMI_AUTO_QUEUE (requires approval)
    - EXECUTE (approved for execution)
    """

    __slots__ = ("route", "reason")

    BLOCKED: Final[str] = "BLOCKED"
    PAPER: Final[str] = "PAPER"
    SEMI_AUTO_QUEUE: Final[str] = "SEMI_AUTO_QUEUE"
...
def route_with_authority(
    *,
    live_execution: str,
    practice: str,
    trading_mode: str,
    is_exit: bool = False,
    is_risk_reduce: bool = False,
) -> ExecutionRouteDecision:
    """Route an ExecutionIntent based on OperatorAuthority state.

    Args:
        live_execution: "BLOCKED" or "ARMED"
        practice: "ON" or "OFF"
        trading_mode: "MANUAL", "SEMI_AUTO", or "FULL_AUTO"
        is_exit: Whether the intent is an exit
        is_risk_reduce: Whether the intent reduces risk

    Returns:
        ExecutionRouteDecision with route and reason.
    """
    # LiveExecution=BLOCKED → always block live, may paper
    if live_execution == "BLOCKED":
        if practice == "ON":
            return ExecutionRouteDecision(
                ExecutionRouteDecision.PAPER,
                "LiveExecution=BLOCKED, Practice=ON → paper execution",
            )
        return ExecutionRouteDecision(
            ExecutionRouteDecision.BLOCKED,
            "LiveExecution=BLOCKED, Practice=OFF → fully blocked",
        )
    ...
    if trading_mode == "MANUAL":
        return ExecutionRouteDecision(
            ExecutionRouteDecision.BLOCKED,
            "MANUAL mode — only dashboard order-ticket may produce intents",
        )

    if trading_mode == "SEMI_AUTO":
        # Exits/risk-reductions always auto-fire in semi-auto
        if is_exit or is_risk_reduce:
            return ExecutionRouteDecision(
                ExecutionRouteDecision.EXECUTE,
                "SEMI_AUTO exit/risk-reduce → auto-fire",
            )
        return ExecutionRouteDecision(
            ExecutionRouteDecision.SEMI_AUTO_QUEUE,
            "SEMI_AUTO entry → requires operator approval",
        )
