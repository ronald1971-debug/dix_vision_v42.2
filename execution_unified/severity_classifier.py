"""
execution_engine.hazard.severity_classifier
DIX VISION v42.2 — Hazard Severity Classifier

Migrated from execution/hazard/severity_classifier.py
"""

from __future__ import annotations

from execution_unified.core.hazard.async_bus import HazardEvent, HazardSeverity, HazardType


def should_halt_trading(event: HazardEvent) -> bool:
    """Return True if this hazard should trigger trading halt."""
    return event.severity == HazardSeverity.CRITICAL or event.hazard_type in {
...
def should_enter_safe_mode(event: HazardEvent) -> bool:
    ...
    return event.severity in {
        HazardSeverity.HIGH,
        HazardSeverity.CRITICAL,
    } or event.hazard_type in {HazardType.FEED_SILENCE, HazardType.EXCHANGE_TIMEOUT}
