...
def enable_solana_native_factory() -> Callable[..., SolanaTransport]:
    ...
    def _factory(*args: object, **kwargs: object) -> SolanaTransport:
        ...
        try:
            ...
            raise ImportError(
                "enable_solana_native_factory requires the optional 'solana', "
                "'solders', 'anchorpy', and 'base58' packages — install with "
                "'pip install solana solders anchorpy base58' "
                "(NEW_PIP_DEPENDENCIES in "
                "execution_engine/adapters/solana_native.py flags this)."
            ) from exc

        endpoint = str(
            kwargs.get("endpoint", args[0] if args else "https://api.mainnet-beta.solana.com")
        )
        _rpc_client = Client(endpoint)

        class _LiveSolanaTransport:
            def get_recent_blockhash(self) -> str:
                resp = _rpc_client.get_latest_blockhash()
                return str(resp.value.blockhash)

            def send_transaction(self, signed: SignedTransaction) -> TxResult:
                try:
                    wire = signed.to_wire_bytes()
                    resp = _rpc_client.send_raw_transaction(wire)
                    sig_b58 = b58encode(signed.signature)
                    return TxResult(
                        signature=sig_b58,
                        status="CONFIRMED",
                        slot=0,
                        detail=str(resp.value),
                    )
