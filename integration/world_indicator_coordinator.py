...
This is the core component that implements the architectural principle that world understanding
and indicator processing are equally important in the cognitive trading system.
"""
...
class WorldIndicatorCoordinator:
    ...
    def analyze_market_integrated(
        ...
        try:
            ...
            if self._integration_mode == IntegrationMode.WORLD_ENHANCED_INDICATORS:
                integrated_analysis = self._world_enhanced_indicators(world_analysis, indicator_analysis)
            elif self._integration_mode == IntegrationMode.INDICATOR_VALIDATED_WORLD:
                integrated_analysis = self._indicator_validated_world(world_analysis, indicator_analysis)
            elif self._integration_mode == IntegrationMode.HYBRID_DECISION_FUSION:
                integrated_analysis = self._hybrid_decision_fusion(world_analysis, indicator_analysis, market_data)
            else:
            ...
            logger.info(f"[WORLD_INDICATOR_COORDINATOR] Integrated analysis completed: {integrated_analysis.integrated_decision} (confidence: {integrated_analysis.integrated_confidence:.2f})")
    ...
    def _world_enhanced_indicators(
        ...
        composite_signal = sum(enhanced_signals.values()) / len(enhanced_signals) if enhanced_signals else 0.0
    ...
    def _indicator_validated_world(
        ...
        # Check alignment
        alignment = self._check_prediction_indicator_alignment(world_prediction, indicator_composite)
        ...
        integrated_confidence = (validated_world_confidence * 0.7) + (indicator_analysis["confidence"] * 0.3)
    ...
    def _adaptive_weighting(
        ...
        indicator_weight = indicator_performance / total_performance if total_performance > 0 else 0.5
    ...
    def _check_prediction_indicator_alignment(self, prediction: str, indicator_signal: float) -> float:
        """Check alignment between world prediction and indicator signal."""
        prediction_direction = 1.0 if prediction.lower() in ["bullish", "buy", "positive"] else (-1.0 if prediction.lower() in ["bearish", "sell", "negative"] else 0.0)
    ...
    def _get_indicator_performance_metric(self) -> float:
        ...
        with self._lock:
            ...
            if indicator_contributions:
                avg_indicator_contribution = sum(indicator_contributions) / len(indicator_contributions)
                # Higher contribution = better performance
        ...
        with self._lock:
            ...
            if self._metrics.total_integrations == 1:
                ...
                self._metrics.average_confidence = (
                    0.95 * self._metrics.average_confidence + 
                    0.05 * analysis.integrated_confidence
                )
            ...
            if self._metrics.total_integrations == 1:
                ...
                self._metrics.average_processing_time_ms = (
                    0.9 * self._metrics.average_processing_time_ms + 
    ...
    def get_analysis_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "average_confidence": sum(analysis.integrated_confidence for analysis in recent_analyses) / len(recent_analyses),
                ...
                "validation_status_distribution": self._count_validation_status_distribution(recent_analyses),
                "timestamp": datetime.now().isoformat(),
            }
    
    def _count_decision_distribution(self, analyses: List[IntegratedMarketAnalysis]) -> Dict[str, int]:
    ...
    def _count_validation_status_distribution(self, analyses: List[IntegratedMarketAnalysis]) -> Dict[str, int]:
