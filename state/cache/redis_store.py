...
def redis_client_factory(config: RedisConfig) -> object:
    ...
    try:
        import hiredis  # noqa: F401, PLC0415
        import redis  # noqa: PLC0415
    except ImportError as exc:
        raise ImportError(
            "redis_client_factory requires the optional 'redis' and 'hiredis' packages "
            "— install with 'pip install redis hiredis' "
            "(NEW_PIP_DEPENDENCIES in state/cache/redis_store.py flags this)."
        ) from exc
    return redis.Redis(
        host=config.host,
        port=config.port,
        db=config.db,
        socket_timeout=config.socket_timeout_ns / 1_000_000_000,
...
def async_redis_client_factory(config: RedisConfig) -> object:
    ...
    try:
        ...
        raise ImportError(
            "async_redis_client_factory requires the optional 'redis' and 'hiredis' packages "
            "— install with 'pip install redis hiredis' "
            "(NEW_PIP_DEPENDENCIES in state/cache/redis_store.py flags this)."
        ) from exc
