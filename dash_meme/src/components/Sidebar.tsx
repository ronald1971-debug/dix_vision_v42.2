...
const MEME_NAV: Record<MemeRoute, NavItem<MemeRoute>> = {
  explorer: { key: "explorer", label: "Explorer", href: "#/explorer", icon: Compass },
  pools: { key: "pools", label: "Pools", href: "#/pools", icon: Layers },
  bigswap: { key: "bigswap", label: "Big Swap", href: "#/bigswap", icon: TrendingUp },
  multichart: { key: "multichart", label: "Multi Chart", href: "#/multichart", icon: BarChart3 },
  trade: { key: "trade", label: "Trade", href: "#/trade", icon: Target },
  copy: { key: "copy", label: "Copy Trading", href: "#/copy", icon: Copy },
  sniper: { key: "sniper", label: "Sniper", href: "#/sniper", icon: Zap },
  multiswap: { key: "multiswap", label: "Multi Swap", href: "#/multiswap", icon: Layers },
  wallet: { key: "wallet", label: "Wallet", href: "#/wallet", icon: Wallet },
  stats: { key: "stats", label: "Stats", href: "#/stats", icon: Activity },
};
...
export function Sidebar({ active, collapsed, onToggle, onRouteChange }: SidebarProps) {
  return (
    <aside
    ...
    >
      ...
      <SidebarSection title="MEME Trading" collapsed={collapsed}>
        {MEME_ROUTE_LIST.map((key) => (
          <SidebarLink
            key={key}
            item={MEME_NAV[key]}
            isActive={active === key}
