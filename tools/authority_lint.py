...
EXECUTION_EVENT_ALLOWED_PREFIXES: tuple[str, ...] = (
    "execution_engine",
    # mind.engine (IndiraEngine) is the unified fast-path that produces
    # ExecutionEvents directly as part of the tick→signal→execute pipeline.
    "mind",
)
...
COGNITIVE_ALLOWED_PREFIXES: tuple[str, ...] = (
    ...
    "integrations.langgraph_adapter",
    "tests",
)
...
# ---------------------------------------------------------------------------
# B-DIX-03 — INDIRA domain ownership (INV-DIX-03)
# B-DIX-04 — DYON domain ownership (INV-DIX-04)
# B-DIX-05 — Strategy cognition exclusivity (INV-DIX-05)
# ---------------------------------------------------------------------------
# Domain ownership is loaded from contracts/ownership_registry.yaml via
# ownership_registry_loader.py to ensure consistent enforcement across tools.

INDIRA_OWNED_DOMAINS_CACHE: tuple[str, ...] | None = None
DYON_OWNED_DOMAINS_CACHE: tuple[str, ...] | None = None


def _get_indira_domains() -> tuple[str, ...]:
    """Get INDIRA owned domains from registry with fallback."""
    global INDIRA_OWNED_DOMAINS_CACHE
    if INDIRA_OWNED_DOMAINS_CACHE is None:
        try:
            from tools.ownership_registry_loader import indira_owned_domains
            INDIRA_OWNED_DOMAINS_CACHE = indira_owned_domains()
        except Exception:
            INDIRA_OWNED_DOMAINS_CACHE = (
                "market_intelligence",
                "trader_intelligence",
    ...
    return INDIRA_OWNED_DOMAINS_CACHE


def _get_dyon_domains() -> tuple[str, ...]:
    """Get DYON owned domains from registry with fallback."""
    global DYON_OWNED_DOMAINS_CACHE
    if DYON_OWNED_DOMAINS_CACHE is None:
        try:
            from tools.ownership_registry_loader import dyon_owned_domains
            DYON_OWNED_DOMAINS_CACHE = dyon_owned_domains()
        except Exception:
            DYON_OWNED_DOMAINS_CACHE = (
                "repository_intelligence",
                "architecture_intelligence",
                "runtime_intelligence",
                "infrastructure_intelligence",
            )
    return DYON_OWNED_DOMAINS_CACHE


def _check_indira_domain(importer: str, target: str, file: Path, line: int) -> Violation | None:
    """B-DIX-03 — DYON may not import INDIRA-owned domains."""
    if not _starts_with_any(importer, ("system_engine", "evolution_engine.dyon")):
        return None
    for domain in _get_indira_domains():
        if target == domain or target.startswith(domain + "."):
            return Violation(
                ...
                importer,
                target,
                f"DYON may not import INDIRA-owned domain {domain} (INV-DIX-03)",
            )
    return None


def _check_dyon_domain(importer: str, target: str, file: Path, line: int) -> Violation | None:
    """B-DIX-04 — INDIRA may not import DYON-owned domains."""
    if not _starts_with_any(importer, ("intelligence_engine",)):
        return None
    for domain in _get_dyon_domains():
        if target == domain or target.startswith(domain + "."):
            return Violation(
                "B-DIX-04",
                file,
                ...
                importer,
                target,
                f"INDIRA may not import DYON-owned domain {domain} (INV-DIX-04)",
            )
    return None


def _check_strategy_exclusivity(importer: str, target: str, file: Path, line: int) -> Violation | None:
    """B-DIX-05 — Only INDIRA may access strategy cognition."""
    if target == "strategy_engine" or target.startswith("strategy_engine."):
        if _starts_with_any(importer, ("evolution_engine.dyon", "system_engine.architecture_intelligence")):
            return Violation(
                "B-DIX-05",
                file,
                line,
                importer,
                target,
                "Strategy cognition belongs exclusively to INDIRA (INV-DIX-05)",
            )
    return None
...
RULE_CHECKS = (
    ...
    _check_b_polars,
    _check_b_torch,
    _check_indira_domain,
    _check_dyon_domain,
    _check_strategy_exclusivity,
)
...
B31_ALLOWED_PREFIXES: tuple[str, ...] = (
    ...
    "system.kill_switch",
    # bootstrap_kernel is the system entrypoint that routes initial mode
    # from config into the governance layer; it must name SystemMode.
    "bootstrap_kernel",
    # governance.mode.* and governance.mode_manager are the mode FSM
...
B23_PYTHON_EXEMPT_MODULES: frozenset[str] = frozenset(
    {
        ...
        # C-47 — local inference transport adapted from ollama-python.
        # Vendor name "ollama"/"llama" necessarily appears as the
        # backend identifier and in protocol documentation.
        "intelligence_engine.cognitive.chat.local_transport",
        # C-48 — vLLM OpenAI-compatible inference transport.
        "intelligence_engine.cognitive.chat.vllm_transport",
        # C-49 — llama-cpp-python local CPU inference transport.
        "intelligence_engine.cognitive.chat.llama_transport",
        # C-50 — TensorRT-LLM inference transport.
        "intelligence_engine.cognitive.chat.tensorrt_transport",
    }
)
...
# ---------------------------------------------------------------------------
# B-FETCH — external adapters expose only fetch_* methods (BUILD-DIRECTIVE §3).
# ---------------------------------------------------------------------------
#
# Adapters under ``execution_engine/adapters/external/`` and
# ``data_sources/external/`` are read-only data sources. They may only
# expose public methods named ``fetch_*``. Any ``submit*``, ``execute*``,
# ``place*``, ``trade*``, ``swap*``, or ``modify_strategy*`` method is
# forbidden. External platforms (TradingView, MT5, QuantConnect, etc.) may
# not submit orders — they provide data only.

B_FETCH_EXTERNAL_PREFIXES: tuple[str, ...] = (
    "execution_engine.adapters.external.",
    "data_sources.external.",
)

B_FETCH_FORBIDDEN_METHOD_PREFIXES: tuple[str, ...] = (
    "submit",
    "execute",
    "place",
    "trade",
...
def _check_b_fetch(importer: str, file: Path, repo_root: Path, tree: ast.AST) -> list[Violation]:
    """B-FETCH — external adapters may only expose fetch_* methods."""
    if not any(importer.startswith(p) for p in B_FETCH_EXTERNAL_PREFIXES):
        return []
    out: list[Violation] = []
    for node in ast.walk(tree):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            for prefix in B_FETCH_FORBIDDEN_METHOD_PREFIXES:
                if node.name.startswith(prefix):
                    out.append(
                        Violation(
                            ...
                            importer,
                            node.name,
                            f"external adapter method '{node.name}' is forbidden"
                            " — external adapters may only expose fetch_* methods"
                            " (BUILD-DIRECTIVE §3, invariant 3).",
                        )
                    )
                    break
    return out
...
# ---------------------------------------------------------------------------
# B-OPAUTH — only OperatorInterfaceBridge writes operator_authority state
# (BUILD-DIRECTIVE §new-lint-rules).
# ---------------------------------------------------------------------------
#
# The ``operator_authority`` field in server state may only be mutated by
# ``governance_engine.control_plane.operator_interface_bridge``. No other
# module may assign to ``operator_authority`` or ``_operator_authority``.

B_OPAUTH_ALLOWED_PREFIXES: tuple[str, ...] = (
    "governance_engine.control_plane.operator_interface_bridge",
    "tests.",
    "tools.",
)

B_OPAUTH_FORBIDDEN_ATTRS: frozenset[str] = frozenset({"operator_authority", "_operator_authority"})


def _check_b_opauth(importer: str, file: Path, repo_root: Path, tree: ast.AST) -> list[Violation]:
    """B-OPAUTH — only OperatorInterfaceBridge writes operator_authority."""
    if any(importer.startswith(p) for p in B_OPAUTH_ALLOWED_PREFIXES):
        return []
    out: list[Violation] = []
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign):
            for target in node.targets:
                if isinstance(target, ast.Attribute):
                    if target.attr in B_OPAUTH_FORBIDDEN_ATTRS:
                        out.append(
                            Violation(
                                "B-OPAUTH",
                                file,
                                ...
                                importer,
                                target.attr,
                                "only OperatorInterfaceBridge may write"
                                " operator_authority (BUILD-DIRECTIVE §lint).",
                            )
                        )
    return out
...
# ---------------------------------------------------------------------------
# B-COMPOSER — only strategy_composer constructs ComposedStrategy
# (BUILD-DIRECTIVE §new-lint-rules).
# ---------------------------------------------------------------------------
#
# The ``ComposedStrategy`` class may only be instantiated inside
# ``intelligence_engine.strategy_composer.composer``. This prevents
# unvalidated strategy compositions from bypassing the sandbox and
# promotion gates.

B_COMPOSER_ALLOWED_PREFIXES: tuple[str, ...] = (
    "intelligence_engine.strategy_composer.composer",
    "intelligence_engine.strategy_library.composition",
    "tests.",
    "tools.",
)


def _check_b_composer(importer: str, file: Path, repo_root: Path, tree: ast.AST) -> list[Violation]:
    """B-COMPOSER — only strategy_composer constructs ComposedStrategy."""
    if any(importer.startswith(p) for p in B_COMPOSER_ALLOWED_PREFIXES):
        return []
    out: list[Violation] = []
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            func = node.func
            if isinstance(func, ast.Name) and func.id == "ComposedStrategy":
                out.append(
                    Violation(
                        "B-COMPOSER",
                        file,
                        ...
                        importer,
                        "ComposedStrategy",
                        "only intelligence_engine.strategy_composer.composer"
                        " may construct ComposedStrategy"
                        " (BUILD-DIRECTIVE §lint).",
                    )
                )
            elif isinstance(func, ast.Attribute) and func.attr == "ComposedStrategy":
                out.append(
                    Violation(
                        "B-COMPOSER",
                        file,
...
# ---------------------------------------------------------------------------
# B-MANUAL — only dashboard order-ticket may produce ExecutionIntent in
# MANUAL mode (BUILD-DIRECTIVE §new-lint-rules).
# ---------------------------------------------------------------------------
#
# When a domain is in MANUAL trading mode, Indira does NOT produce
# ExecutionIntent. Only the dashboard's order-ticket (registered in
# ``registry/authority_matrix.yaml`` as manual-mode producer) may
# construct ExecutionIntent for manual-mode domains. This is a runtime
# check more than an AST check, but we enforce that
# ``intelligence_engine`` modules never reference ``ManualOrderTicket``
# construction.

B_MANUAL_FORBIDDEN_PREFIXES: tuple[str, ...] = ("intelligence_engine.",)

B_MANUAL_ALLOWED_PREFIXES: tuple[str, ...] = (
    "intelligence_engine.strategy_composer.",
    "tests.",
    "tools.",
)


def _check_b_manual(importer: str, file: Path, repo_root: Path, tree: ast.AST) -> list[Violation]:
    """B-MANUAL — intelligence_engine may not construct ManualOrderTicket."""
    if not any(importer.startswith(p) for p in B_MANUAL_FORBIDDEN_PREFIXES):
        return []
    if any(importer.startswith(p) for p in B_MANUAL_ALLOWED_PREFIXES):
        return []
    out: list[Violation] = []
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            func = node.func
            if isinstance(func, ast.Name) and func.id == "ManualOrderTicket":
                out.append(
                    Violation(
                        "B-MANUAL",
                        file,
                        ...
                        importer,
                        "ManualOrderTicket",
                        "intelligence_engine may not construct"
                        " ManualOrderTicket — MANUAL-mode ExecutionIntent"
                        " comes only from the dashboard order ticket"
                        " (BUILD-DIRECTIVE §lint).",
                    )
                )
    return out
...
def lint_repo(repo_root: Path) -> list[Violation]:
    ...
    for path in _iter_python_files(repo_root):
        ...
        # B-FETCH — external adapters expose only fetch_* (BUILD-DIRECTIVE §3).
        violations.extend(_check_b_fetch(importer, path, repo_root, tree))
        # B-OPAUTH — only OperatorInterfaceBridge writes operator_authority.
        violations.extend(_check_b_opauth(importer, path, repo_root, tree))
        # B-COMPOSER — only strategy_composer constructs ComposedStrategy.
        violations.extend(_check_b_composer(importer, path, repo_root, tree))
        # B-MANUAL — intelligence_engine cannot produce ManualOrderTicket.
        violations.extend(_check_b_manual(importer, path, repo_root, tree))
