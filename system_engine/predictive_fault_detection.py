...
class PredictiveFaultDetector:
    ...
    def _check_failure_signal(
        ...
        if failure_type == FailureType.RESOURCE_EXHAUSTION:
            ...
            for i, pred in enumerate(predicted):
                if pred > threshold:
                    confidence = min(1.0, (pred - current_value) / (threshold - current_value + 1e-6))
                    return (int(history[-1].timestamp_ns + i * 1_000_000_000), confidence)
            ...
            if trend_slope > 0 and all(predicted[i] > predicted[i-1] for i in range(1, len(predicted))):
    ...
    def _generate_recommendations(self, failure_type: FailureType, confidence: float) -> tuple[str, ...]:
