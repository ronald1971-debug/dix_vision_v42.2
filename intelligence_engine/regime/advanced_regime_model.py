...
class AdvancedRegimeModel:
    ...
    _transition_history: list[RegimeTransition] = field(default_factory=list, init=False, repr=False)
    ...
    def _detect_hmm_regime(self, ts_ns: int) -> RegimeState | None:
        ...
        vol = sqrt(sum((x - sum(self._observation_window) / len(self._observation_window)) ** 2 
                      for x in self._observation_window) / len(self._observation_window))
        drift = sum(self._observation_window[-8:]) / 8 if len(self._observation_window) >= 8 else 0.0
    ...
    def _fallback_regime_detection(self) -> tuple[tuple[int, ...], tuple[tuple[float, ...], ...], float]:
    ...
    def _detect_bayesian_change(self) -> tuple[int, float, float]:
        ...
        prior_std = sqrt(sum((x - prior_mean) ** 2 for x in observations[:n//2]) / (n//2)) + 1e-6
        ...
        for cp in range(n//4, 3*n//4):
            ...
            if pre_std > 0 and post_std > 0:
                bayes_factor = exp(
                    -0.5 * (sum((x - pre_mean) ** 2 for x in pre_data) / pre_std ** 2 +
    ...
    def _record_transition(self, change_point: int, change_prob: float, bayes_factor: float) -> None:
        ...
        if post_change_data:
            ...
            vol = sqrt(sum((x - sum(post_change_data) / len(post_change_data)) ** 2 
                          for x in post_change_data) / len(post_change_data))
...
class MultiTimescaleRegimeModel:
    ...
    def process_observation(self, value: float, ts_ns: int) -> tuple[RegimeState | None, RegimeState | None, RegimeState | None]:
