...
class MarketImpactEstimator:
    ...
    def calibrate_model(
        ...
        avg_volume = sum(t.get("volume", 0) for t in historical_trades[-50:]) / 50 if historical_trades else 1000000.0
        avg_spread = sum(t.get("spread", 0.001) for t in historical_trades[-50:]) / 50 if historical_trades else 0.001
    ...
    def estimate_impact(
        ...
        normalized_quantity = quantity / model.avg_daily_volume if model.avg_daily_volume > 0 else 0.0
...
class OptimalExecutionPlanner:
    ...
    def __init__(
        self,
        default_execution_bars: int = 20,
        max_execution_bars: int = 100
    ) -> None:
    ...
    def plan_execution(
        ...
        execution_time_options = [
            ...
            min(self._max_execution_bars, int(self._default_execution_bars * (1.0 + urgency) * 2.0))
        ]
        
        impact_estimates = {}
        for time_bars in execution_time_options:
            impact = self._impact_estimator.estimate_impact(
                symbol, quantity, time_bars,
                market_conditions.get("volatility", 1.0)
            )
        ...
        optimal_schedule = self._generate_schedule(
            side, quantity, optimal_time, optimal_algorithm
        )
