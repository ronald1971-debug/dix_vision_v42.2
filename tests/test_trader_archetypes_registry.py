...
def test_known_archetype_shape() -> None:
    registry = load_trader_archetypes()
    a = registry.get("TA-MACRO-001")
    assert a is not None
    assert a.name == "George Soros — scalp/conservative/trending"
    assert a.state is ArchetypeState.ACTIVE
    assert a.risk_attitude is RiskAttitude.CONSERVATIVE
    assert a.time_horizon is TimeHorizon.SCALP
    assert a.conviction_style is ConvictionStyle.REACTIVE
    assert a.group == "MACRO"
    assert a.philosophy == "reflexivity"
    assert 0.0 <= a.decay_rate <= 1.0
    assert -1.0 <= a.performance_score <= 1.0
    assert pytest.approx(a.belief_system["reflexivity"]) == 0.95
...
def test_contains_supports_both_id_and_archetype() -> None:
    ...
    assert "TA-MACRO-001" in registry
