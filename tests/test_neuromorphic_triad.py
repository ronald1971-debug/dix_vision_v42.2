...
def test_detector_emits_latency_drift() -> None:
    ...
    with patch("execution.monitoring.neuromorphic_detector.append_event") as ae:
        event = d.on_telemetry({"component": "binance_ws", "latency_ms_p99": 1200.0})
...
def test_detector_emits_silent_data_failure() -> None:
    ...
    with patch("execution.monitoring.neuromorphic_detector.append_event"):
        event = d.on_telemetry(
            {"component": "binance_ws", "event_rate_hz": 0.5, "expected_rate_hz": 100.0}
        )
...
def test_signal_plugin_deadman_survives_calm_market(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "mind.plugins.neuromorphic_signal.time.monotonic",
...
def test_risk_sensor_deadman_survives_calm_conditions(monkeypatch) -> None:
    ...
    monkeypatch.setattr(nr.time, "monotonic", lambda: t0 + 10.0 * r.heartbeat_interval)
    r.evaluate({"drawdown_velocity": 0.0})  # no threshold
    assert r.check_self() is True, "N5 regression: calm risk eval() must keep dead-man alive"
    ...
    for rel in targets:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom) and node.module:
                assert not any(
                    node.module == b or node.module.startswith(b + ".") for b in forbidden
                ), f"{rel} imports forbidden decision surface: {node.module}"
            if isinstance(node, ast.Import):
                for alias in node.names:
                    assert not any(
                        alias.name == b or alias.name.startswith(b + ".") for b in forbidden
