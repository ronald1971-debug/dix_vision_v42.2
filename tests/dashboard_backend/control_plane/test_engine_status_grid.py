...
def test_grid_buckets_health_states():
    engines = {
        "execution": _FakeEngine(HealthStatus(engine_name="execution", state=HealthState.OK, detail="all good")),
        "intelligence": _FakeEngine(HealthStatus(engine_name="intelligence", state=HealthState.DEGRADED, detail="lag")),
        "system": _FakeEngine(HealthStatus(engine_name="system", state=HealthState.FAIL, detail="halted")),
    }
...
def test_grid_renders_plugin_state_rows():
    ...
    assert ("alpha", "plug_a", "alive") in plugins  # OK == "alive" per enum
