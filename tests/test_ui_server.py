...
def test_health_returns_all_six_engines(client):
    ...
    # Core six engines must be present
    for expected in ("intelligence", "execution", "system", "governance", "learning", "evolution"):
        assert expected in engines, f"Missing engine: {expected}"
    # All six core engines should report healthy or have state OK
    for name in ("intelligence", "execution", "system", "governance", "learning", "evolution"):
        eng = engines[name]
        # Check either 'healthy' (kernel-provided) or 'state' (DirectStatus)
        if "healthy" in eng:
            assert eng["healthy"], f"Engine {name} not healthy: {eng['detail']}"
        elif "state" in eng:
            assert eng["state"] == "OK", f"Engine {name} not OK: {eng['detail']}"
