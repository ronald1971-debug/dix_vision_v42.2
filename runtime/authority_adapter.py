...
class SystemMode:
    """Canonical system modes — delegates to core.contracts.governance.SystemMode.

    The canonical enum is an IntEnum; this wrapper preserves string-
    based access for callers that previously used the local StrEnum
    while guaranteeing a single source of truth.
    """
    ...
    @classmethod
    def from_canonical(cls, mode) -> SystemMode:
        return cls(mode.name if hasattr(mode, "name") else str(mode))

    @classmethod
    def to_canonical(cls, mode: SystemMode):
        from core.contracts.governance import SystemMode as FsmMode
        try:
            return FsmMode[mode.value]
        except KeyError:
            return FsmMode.SAFE
