    ...
    for name, mod_path, factory_name in _TIER_SERVICE_FACTORIES:
        try:
            ...
            if fn is None:
                continue
            instance = fn()
            validate_runtime_contract(instance, name)
...
def validate_runtime_contract(instance: object, name: str) -> None:
    """Best-effort protocol check for RuntimeComponent.

    Logs a warning on mismatch; never raises because tier registration
    must remain best-effort. The check only fires if ``runtime_checkable``
    protocol matching is available (Python 3.8+).
    """
    try:
        from runtime.contracts import RuntimeComponent, RuntimeService
        if not isinstance(instance, (RuntimeComponent, RuntimeService)):
            _logger.warning(
                "service_registry: %s does not satisfy RuntimeComponent/RuntimeService", name
            )
    except Exception as exc:
        _logger.debug("service_registry: contract check skipped for %s: %s", name, exc)
