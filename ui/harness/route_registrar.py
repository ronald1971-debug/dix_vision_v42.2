...
_OPERATOR_ROUTES: frozenset[RouteKey] = frozenset(
    {
        ...
        ("POST", "/api/operator/authority"),
        ("POST", "/api/operator/trading_mode"),
        ("POST", "/api/operator/semi_auto_policy"),
        ("POST", "/api/operator/promote_buildout_params"),
        ("POST", "/api/operator/external_source"),
        ("POST", "/api/operator/manual_order"),
        ("GET", "/api/operator/learning_progress"),
        ("GET", "/api/operator/semi_auto_queue"),
        ("POST", "/api/operator/semi_auto_approve"),
        ("POST", "/api/operator/semi_auto_reject"),
    }
)
...
# P3/P4 — cognitive governance status panel + SL/TP bracket calculator
        ...
        ("GET", "/api/cognitive/knowledge_lifecycle"),
        ("GET", "/api/cognitive/cognitive_physics"),
        # P4 — cognitive research queue
        ...
        # Stage 2/3 — DYON extended engineering surfaces
        ...
        # COGNITIVE ACTIVATION PHASE — real-time stream projection
        ("GET", "/api/cognitive/stream"),
        ("GET", "/api/cognitive/snapshot"),
        # Agent orchestrator introspection
        ("GET", "/api/cognitive/orchestrator/state"),
        ("GET", "/api/cognitive/orchestrator/agents"),
        ("GET", "/api/cognitive/orchestrator/recent"),
    }
)

_MEMORY_ROUTES: frozenset[RouteKey] = frozenset(
    {
...
_FABRIC_ROUTES: frozenset[RouteKey] = frozenset(
    {
...
_COCKPIT_ROUTES: frozenset[RouteKey] = frozenset(
    {
        ...
        # Phase 11.1 Dashboard Update – DYON signals domain
        ("GET", "/api/signals"),
        ("GET", "/api/adapters"),
        ("GET", "/api/hazards"),
        # Phase 11.1 Dashboard Update – INDIRA execution domain
        ("GET", "/api/forms"),
        ("GET", "/api/orders/open"),
        ("GET", "/api/fills"),
        ...
        # Phase 11.1 Dashboard Update – GOVERNANCE mode timeline domain
        ("GET", "/api/mode/timeline"),
        ("GET", "/api/security/events"),
        # Phase 11.1 Dashboard Update – EVENT-SOURCED LEDGER domain
        ("GET", "/api/ledger/tail"),
...
_DOMAIN_INVENTORY: Mapping[str, frozenset[RouteKey]] = {
    ...
    "memory": _MEMORY_ROUTES,
    "fabric": _FABRIC_ROUTES,
}
