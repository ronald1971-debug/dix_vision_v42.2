...
AIX Language Expansion Module (LEM) integration:
  - Supports 100+ languages via XLM-RoBERTa embeddings
  - Auto-detects new languages from incoming data
  - Parallel async processing for multilingual feeds
  - Financial-domain-specific detection (ticker-aware)
"""
...
def detect_language(text: str) -> str:
    ...
    if greek / total > 0.2:
        return "el"
    # AIX LEM: additional script detection
...
def translate_to(
    text: str, target_lang: str = "en", source_lang: str | None = None
) -> TranslationResult:
    if not text:
        return TranslationResult(
            text="", source_lang="en", target_lang=target_lang, translated=False, provider="none"
        )
    src = source_lang or detect_language(text)
    if src == target_lang:
        return TranslationResult(
            text=text, source_lang=src, target_lang=target_lang, translated=False, provider="noop"
        )
    p = _provider()
    if p == "none":
        return TranslationResult(
...
def _deepl(text: str, src: str, target: str) -> TranslationResult:  # pragma: no cover
    ...
    data = urllib.parse.urlencode(
        {"auth_key": key, "text": text, "target_lang": target.upper()}
    ).encode()
    ...
    return TranslationResult(
        text=out, source_lang=src, target_lang=target, translated=True, provider="deepl"
    )
...
def _google(text: str, src: str, target: str) -> TranslationResult:  # pragma: no cover
    ...
    return TranslationResult(
        text=out, source_lang=src, target_lang=target, translated=True, provider="google"
    )
...
def _openai(text: str, src: str, target: str) -> TranslationResult:  # pragma: no cover
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": f"Translate the user's text from {src} to {target}. "
    ...
    return TranslationResult(
        text=out, source_lang=src, target_lang=target, translated=True, provider="openai"
    )
...
# ── AIX: Language Expansion Module (LEM) ─────────────────────────────

# Languages supported via XLM-RoBERTa embeddings for financial data
SUPPORTED_LANGUAGES: frozenset[str] = frozenset({
    ...
    "ka", "kk", "km", "kn", "ko", "ku", "ky", "la", "lo", "lt",
    "lv", "mg", "mk", "ml", "mn", "mr", "ms", "my", "ne", "nl",
    "no", "or", "pa", "pl", "ps", "pt", "ro", "ru", "sa", "sd",
    "si", "sk", "sl", "so", "sq", "sr", "su", "sv", "sw", "ta",
    "te", "th", "tl", "tr", "uk", "ur", "uz", "vi", "xh", "yi",
    "yo", "zh", "zu",
})


def is_supported_language(lang_code: str) -> bool:
    """Check if a language code is in the AIX LEM supported set."""
