...
from execution_engine.adapters.simple_router import get_adapter_router
...
from system.time_source import monotonic_ns as _perf_ns
...
def fast_execute_trade(
    ...
    with scope(Domain.MARKET):
        ...
        if not rc.trading_allowed:
            ...
            return FastExecuteResult(False, "trading_disallowed", "", None, _perf_ns() - t0)
        if rc.safe_mode:
            _audit_reject(asset, side, size_usd, "safe_mode")
            return FastExecuteResult(False, "safe_mode", "", None, _perf_ns() - t0)
        ...
        if portfolio_usd <= 0:
            ...
            return FastExecuteResult(False, "portfolio_usd_required", "", None, _perf_ns() - t0)

        ok, reason = rc.allows_trade(size_usd=size_usd, portfolio_usd=portfolio_usd)
        if not ok:
            _audit_reject(asset, side, size_usd, reason)
            return FastExecuteResult(False, reason, "", None, _perf_ns() - t0)
        ...
        if not auto_ok:
            ...
            return FastExecuteResult(
                False, f"awaiting_operator:{auto_reason}", "", None, _perf_ns() - t0
            )

        adapter = get_adapter_router().route(asset)
        if adapter is None:
            _audit_reject(asset, side, size_usd, "no_adapter")
            return FastExecuteResult(False, "no_adapter", "", None, _perf_ns() - t0)

        order = adapter.place_order(symbol=asset, side=side, size=size_usd, order_type=order_type)

        latency_ns = _perf_ns() - t0
...
def _stage_pending_operator(asset: str, side: str, size_usd: float, reason: str) -> None:
