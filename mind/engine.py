...
class IndiraEngine:
    ...
    def __init__(self, on_execution: Callable[[ExecutionEvent], None] | None = None) -> None:
    ...
    def process_tick(self, market_data: dict[str, Any]) -> ExecutionEvent:
        ...
        if intent.intent_type == IntentType.DELEGATE:
            ...
            ev = self._make_event(
                "HOLD", asset, "NONE", "NONE", 0.0, price, strategy, intent.confidence, True
            )
        else:
            # Determine side
        ...
        # Step 4: Callback for downstream (exchange adapters etc.)
    ...
    def evaluate(self, market_data: dict[str, Any]) -> "IndiraDecision":  # noqa: UP037, F821
