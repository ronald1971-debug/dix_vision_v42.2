...
class SignalPerformanceTracker(Protocol):
    ...
    def update_performance(self, modality: SignalModality, signal_id: str, 
                         actual_outcome: float, predicted_outcome: float) -> float:
...
class MultiModalSignalFusion:
    ...
    def add_signal(self, signal: ModalitySignal) -> None:
        ...
        if len(self._recent_signals[modality]) > self.performance_window:
            self._recent_signals[modality] = self._recent_signals[modality][-self.performance_window:]
    
    def fuse_signals(self, signals: tuple[ModalitySignal, ...], 
                     method: FusionMethod | None = None,
                     timestamp_ns: int = 0) -> FusionResult | None:
    ...
    def _detect_conflicts(
        ...
        if buy_modalities and sell_modalities:
            ...
            conflict_magnitude = 2.0 * min(buy_weight, sell_weight) / total_weight if total_weight > 0 else 0.0
    ...
    def _weighted_average_fusion(
        ...
        if consensus_score < self.consensus_threshold and self.default_method == FusionMethod.CONSENSUS:
            return None
