...
class MemeRejectionReason(StrEnum):
    ...
    # AIX rug-pull analyzer (RPA) reasons
    LIQUIDITY_TOO_LOW = "LIQUIDITY_TOO_LOW"
    RUG_PULL_PATTERN = "RUG_PULL_PATTERN"
    CONCENTRATED_SUPPLY = "CONCENTRATED_SUPPLY"
    SOCIAL_SCORE_LOW = "SOCIAL_SCORE_LOW"


# ── AIX thresholds (from vision spec) ────────────────────────────────
AIX_MIN_LIQUIDITY_USD: float = 10_000.0
AIX_MAX_SLIPPAGE_PCT: float = 0.5
AIX_MAX_TOP_HOLDER_PCT: float = 20.0
...
class MemeRiskPolicy:
    ...
    # -- AIX: Rug-Pull Analyzer (RPA) extensions ---------------------------
    ...
    ) -> MemeRejectionReason | None:
        """Run AIX rug-pull heuristics. Returns rejection reason or None.

        Checks:
        - Liquidity must be >= $10K (AIX_MIN_LIQUIDITY_USD)
        - Top holder concentration < 20%
        - Social score must meet minimum
        - Contract must be verified
        """
        if liquidity_usd < AIX_MIN_LIQUIDITY_USD:
            return MemeRejectionReason.LIQUIDITY_TOO_LOW
        if top_holder_pct > AIX_MAX_TOP_HOLDER_PCT:
            return MemeRejectionReason.CONCENTRATED_SUPPLY
        if social_score < 0.3:
            return MemeRejectionReason.SOCIAL_SCORE_LOW
        if not contract_verified:
            return MemeRejectionReason.RUG_PULL_PATTERN
        return None
