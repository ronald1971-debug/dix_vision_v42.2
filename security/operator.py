...
* Single-operator authority: all actions require only one operator
  approval.  Ronald is the sole operator — no multi-person gates.
"""
...
class ApprovalKind(StrEnum):
    ...
    # AIX vision: additional HITL approval gates
    MEMECOIN_SNIPE = "MEMECOIN_SNIPE"
    STRATEGY_DEPLOY = "STRATEGY_DEPLOY"
    NEW_ASSET_ONBOARD = "NEW_ASSET_ONBOARD"
    PLATFORM_ONBOARD = "PLATFORM_ONBOARD"
    LARGE_TRADE_EXECUTE = "LARGE_TRADE_EXECUTE"
    EVOLUTION_PROMOTE = "EVOLUTION_PROMOTE"
...
# Single-operator mode: all actions require exactly 1 approval.
# The two-person gate has been removed — Ronald operates solo.
...
def approve(request_id: str, *, operator_id: str) -> ApprovalRequest:
    ...
    Single-operator: one approval transitions to GRANTED.
    """
    import json

    with _lock:
        c = _connect()
        r = c.execute("SELECT * FROM approvals WHERE request_id=?", (request_id,)).fetchone()
        if r is None:
        ...
        new_state = ApprovalState.GRANTED if len(approvers) >= required else ApprovalState.PENDING
        c.execute(
            "UPDATE approvals SET state=?, approvers=? WHERE request_id=?",
            (new_state.value, json.dumps(approvers), request_id),
        )
        c.commit()
        r2 = c.execute("SELECT * FROM approvals WHERE request_id=?", (request_id,)).fetchone()
    try:
        get_writer().write(
            "OPERATOR",
            "APPROVAL_GRANTED" if new_state is ApprovalState.GRANTED else "APPROVAL_CAST",
            "security.operator",
            {
                "request_id": request_id,
                "kind": str(r["kind"]),
...
def deny(request_id: str, *, operator_id: str, reason: str = "") -> ApprovalRequest:
    with _lock:
        c = _connect()
        r = c.execute("SELECT * FROM approvals WHERE request_id=?", (request_id,)).fetchone()
        if r is None:
...
def pending(kind: ApprovalKind | None = None) -> list[ApprovalRequest]:
    with _lock:
        ...
        if kind is None:
            rows = c.execute(
                "SELECT * FROM approvals WHERE state=? ORDER BY created_utc DESC",
                (ApprovalState.PENDING.value,),
            ).fetchall()
        else:
            rows = c.execute(
                "SELECT * FROM approvals WHERE state=? AND kind=? ORDER BY created_utc DESC",
                (ApprovalState.PENDING.value, kind.value),
            ).fetchall()
        rows = [_maybe_expire_locked(c, r) for r in rows]
    return [_row_to_request(r) for r in rows if str(r["state"]) == ApprovalState.PENDING.value]
