"""HARDEN-04 — CognitiveDevelopment policy + ExecutionGate (INV-70).
...
Two entirely independent gates govern two entirely independent concerns:
    ...
    ┌─────────────────────────────────────────────────────────────┐
    │  GATE 1 — CognitiveDevelopmentPolicy                        │
    │  Controls: cognition, research, evolution, system mutation  │
    │  Default : OPEN (operator_override = True)                  │
    │  Indira  : full trader discovery, market cognition, BeliefState│
    │  Dyon    : engineering cognition, architecture evolution, runtime analysis│
    ├─────────────────────────────────────────────────────────────┤
...
* All cognition, evolution, and decisions are logged with full
  provenance regardless of whether trading is enabled.

BeliefState
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Indira continuously updates the dixvision.42.2 BeliefState.
BeliefState remains the authoritative market worldview.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  v42.2-DEV-MODE       Gate 1 open by default (cognition/evolution free).
                        ...
                        dixvision.42.2-aligned: BeliefState, Trader Cognition, Dyon System Cognition.
"""
...
_BOOT_LEARNING_OVERRIDE: bool = _env_bool("DIXVISION_LEARNING_OVERRIDE", default=True)

# Gate 2 — execution / trading: LOCKED by default.
_BOOT_TRADING_ALLOWED: bool = _env_bool("DIXVISION_TRADING_ALLOWED", default=False)
...
# ══════════════════════════════════════════════════════════════════════════════
# GATE 1 — Cognitive Development (Indira cognition + Dyon system cognition)
# ══════════════════════════════════════════════════════════════════════════════
...
class LearningEvolutionFreezePolicy:
    """Gate 1 — controls cognition, evolution, and system mutation.
    ...
    What Gate 1 enables when open:
        ...
        Indira — Market Cognition
            * Full trader discovery, observation, profiling
            * Trader archetype modeling and memory
            * BeliefState updates and alpha generation
            * Strategy research and model mutation

        Dyon — System Cognition
            * Engineering learning cycles and self-reflection
            * Repository understanding and architecture analysis
            * Runtime analysis and infrastructure evolution
            * Structural mutation

        Learning Engine
            * Replay, attribution, model improvement

        Research
            * Continuous evolution, experimental modeling
    ...
    # BUILD-DIRECTIVE §3: optional OperatorAuthority supersedes mode-based freeze.
    # When present, learning state from OperatorAuthority takes priority.
    operator_authority: object | None = field(default=None)
    ...
    def is_unfrozen(self) -> bool:
        ...
        When ``operator_authority`` is set, its ``learning`` field supersedes
        ``operator_override``:
          - learning=FULL  → unfrozen (True)
          - learning=OFF   → frozen (False)
          - learning=SHADOW → frozen for apply, but shadow-eligible

        Otherwise falls back to single-gate predicate: ``operator_override``.
        """
        if self.operator_authority is not None:
            from core.contracts.operator_authority import LearningAuthority

            auth = self.operator_authority
            return getattr(auth, "learning", None) == LearningAuthority.FULL
        return self.operator_override is True

    def is_shadow_eligible(self) -> bool:
        """Return ``True`` if shadow-mode learning ticks are permitted.

        Shadow ticks compute updates but do not apply them. Produces
        LEARNING_SHADOW_TICK events for offline analysis.
        """
        if self.operator_authority is not None:
            from core.contracts.operator_authority import LearningAuthority

            auth = self.operator_authority
            learning = getattr(auth, "learning", None)
            return learning in (LearningAuthority.FULL, LearningAuthority.SHADOW)
        return self.operator_override is True
...
# ══════════════════════════════════════════════════════════════════════════════
# GATE 2 — Capital Deployment (locked until operator decides system is ready)
# ══════════════════════════════════════════════════════════════════════════════
...
@dataclass(frozen=True, slots=True)
class ExecutionGate:
    """Gate 2 — controls real order dispatch and capital deployment.
    ...
    can run at full cognitive and evolution capacity while this gate is
...
class OperatorMasterDevelopmentMode:
    ...
    Default state:
        ...
        Gate 1 (cognition/evolution) -> OPEN
        Gate 2 (execution/trading)   -> LOCKED

    This is the intended startup posture. Indira and Dyon run at full
    cognitive capacity; no real capital is placed until the operator decides
    the system is ready and explicitly unlocks Gate 2.

    dixvision.42.2 Gate Separation:

        Gate 1 — Cognitive Development (OPEN)
            Indira: Market Cognition, BeliefState, Trader Cognition
            Dyon: System Cognition, Architecture Evolution
            Learning Engine: Continuous research and modeling

        Gate 2 — Capital Deployment (LOCKED)
            Broker Access, Exchange Access, Order Dispatch
            Real Capital Exposure — only operator may unlock
    ...
    def unlock_trading(self, mode: SystemMode) -> OperatorMasterDevelopmentMode:
    ...
    def freeze_learning(self, mode: SystemMode) -> OperatorMasterDevelopmentMode:
        ...
        """
        return OperatorMasterDevelopmentMode(
            learning_gate=LearningEvolutionFreezePolicy(mode=mode, operator_override=False),
